...
void ExprNode::print_expr_info() {
    if (_node_type == proto::FUNCTION_CALL) {
        DB_WARNING("node_type: %s fn %s", proto::ExprNodeType_Name(_node_type).c_str(), static_cast<ScalarFnCall*>(this)->fn().DebugString().c_str());
    } else {
        DB_WARNING("node_type: %s tuple_id:%d slot_id:%d", proto::ExprNodeType_Name(_node_type).c_str(),
            _tuple_id, _slot_id);
    }
    for (auto& child : _children) {
...
bool ExprNode::is_logical_and_or_not() {
    if (_node_type == proto::NOT_PREDICATE || _node_type == proto::AND_PREDICATE || _node_type == proto::OR_PREDICATE) {
        return true;
    }
    return false;
}
