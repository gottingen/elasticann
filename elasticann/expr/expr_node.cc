...
#include "elasticann/expr/agg_fn_call.h"
...
namespace EA {
    ...
    void ExprNode::const_pre_calc() {
        if (_children.size() == 0 || _node_type == proto::AGG_EXPR) {
            return;
        }
        //标量函数和RowExpr会走到这
        _is_constant = true;
        ...
        if (_node_type == proto::IS_NULL_PREDICATE) {
            _has_null = false;
        }
        //const表达式等着父节点来替换
        //root是const表达式则外部替换
    ...
    bool ExprNode::contains_null_function() {
        if (_node_type == proto::IS_NULL_PREDICATE) {
            return true;
        } else if (_node_type == proto::FUNCTION_CALL) {
    ...
    bool ExprNode::is_function_eq() {
        if (_node_type == proto::FUNCTION_CALL
    ...
    bool ExprNode::is_children_all_and() {
        ...
        for (auto &child: _children) {
            ...
            if (child->node_type() != proto::AND_PREDICATE) {
                return false;
            }
        }
        return true;
    ...
    void ExprNode::replace_slot_ref_to_literal(const std::set<int64_t> &sign_set,
        ...
        for (size_t i = 0; i < _children.size(); i++) {
            if (_children[i]->node_type() == proto::SLOT_REF) {
    ...
    void ExprNode::set_slot_col_type(int32_t tuple_id, int32_t slot_id, proto::PrimitiveType col_type) {
        if (_node_type == proto::SLOT_REF && _tuple_id == tuple_id && _slot_id == slot_id) {
            _col_type = col_type;
        }
        for (auto &child: _children) {
    ...
    proto::PrimitiveType ExprNode::get_slot_col_type(int32_t slot_id) {
        if (_node_type == proto::SLOT_REF && _slot_id == slot_id) {
            return _col_type;
        }
        for (auto &child: _children) {
            proto::PrimitiveType type = child->get_slot_col_type(slot_id);
            if (type != proto::INVALID_TYPE) {
                return type;
            }
        }
    ...
    void ExprNode::transfer_pb(proto::ExprNode *pb_node) {
    ...
    void ExprNode::create_pb_expr(proto::Expr *expr, ExprNode *root) {
        proto::ExprNode *pb_node = expr->add_nodes();
    ...
    int ExprNode::create_tree(const proto::Expr &expr, ExprNode **root) {
        int ret = 0;
        int idx = 0;
    ...
    int ExprNode::create_tree(const proto::Expr &expr, int *idx, ExprNode *parent, ExprNode **root) {
        if (*idx >= expr.nodes_size()) {
            TLOG_ERROR("idx {} > size {}", *idx, expr.nodes_size());
            return -1;
        }
        ...
        if (ret < 0) {
            TLOG_ERROR("create_expr_node fail");
            return ret;
        }
        if (parent != nullptr) {
    ...
    void ExprNode::get_pb_expr(const proto::Expr &from, int *idx, proto::Expr *to) {
        if (*idx >= from.nodes_size()) {
            TLOG_WARN("idx: {} is out of range: {}", *idx, from.nodes_size());
            return;
        }
        const proto::ExprNode &node = from.nodes((*idx)++);
        if (to != nullptr) {
    ...
    void ExprNode::or_node_optimize(ExprNode **root) {
        ...
        if ((*root)->node_type() == proto::OR_PREDICATE
    ...
    int ExprNode::create_expr_node(const proto::ExprNode &node, ExprNode **expr_node) {
        switch (node.node_type()) {
            ...
            case proto::PLACE_HOLDER_LITERAL:
                *expr_node = new Literal;
                (*expr_node)->init(node);
                return 0;
            case proto::NOT_PREDICATE:
                *expr_node = new NotPredicate;
                (*expr_node)->init(node);
                return 0;
            case proto::AND_PREDICATE:
                *expr_node = new AndPredicate;
                (*expr_node)->init(node);
                return 0;
            case proto::OR_PREDICATE:
                *expr_node = new OrPredicate;
                (*expr_node)->init(node);
                return 0;
            case proto::XOR_PREDICATE:
            ...
            case proto::IN_PREDICATE:
                *expr_node = new InPredicate;
                (*expr_node)->init(node);
                return 0;
            case proto::IS_NULL_PREDICATE:
                *expr_node = new IsNullPredicate;
                (*expr_node)->init(node);
                return 0;
            case proto::IS_TRUE_PREDICATE:
                *expr_node = new IsTruePredicate;
                (*expr_node)->init(node);
                return 0;
            case proto::LIKE_PREDICATE:
                *expr_node = new LikePredicate;
                (*expr_node)->init(node);
                return 0;
            case proto::REGEXP_PREDICATE:
            ...
            default:
                //unsupport expr
                TLOG_ERROR("unsupport node type: {}", node.node_type());
                return -1;
        }
        return -1;
    }
    ...
    void ExprNode::print_expr_info() {
        if (_node_type == proto::FUNCTION_CALL) {
            TLOG_WARN("node_type: {} fn {}", proto::ExprNodeType_Name(_node_type).c_str(),
                       static_cast<ScalarFnCall *>(this)->fn().DebugString().c_str());
        } else {
            TLOG_WARN("node_type: {} tuple_id:{} slot_id:{}", proto::ExprNodeType_Name(_node_type).c_str(),
                       _tuple_id, _slot_id);
        }
        for (auto &child: _children) {
    ...
    bool ExprNode::is_logical_and_or_not() {
        if (_node_type == proto::NOT_PREDICATE || _node_type == proto::AND_PREDICATE ||
            _node_type == proto::OR_PREDICATE) {
            return true;
        }
        return false;
    }
    ...
    bool ExprNode::is_vaild_or_optimize_tree(int32_t level, std::unordered_set<int32_t> *tuple_set) {
        if (tuple_set == nullptr) {
            TLOG_ERROR("tuple_set is nullptr");
            return false;
        }
        if (node_type() == proto::SLOT_REF) {
        ...
        switch (node_type()) {
            case proto::AND_PREDICATE:
                ...
                break;
            case proto::OR_PREDICATE:
                if (_children[0]->node_type() == proto::FUNCTION_CALL && !is_children_all_eq()) {
                    return false;
                } else if (_children[0]->node_type() == proto::AND_PREDICATE && !is_children_all_and()) {
                    return false;
                }
                break;
            case proto::FUNCTION_CALL:
    ...
    bool ExprNode::has_same_children() {
        ...
        for (int idx = 0; idx < children_size(); idx++) {
            if (_children[idx] == nullptr) {
                TLOG_ERROR("_children:{} is nullptr", idx);
                return false;
            }
    ...
    int ExprNode::change_or_node_to_in(ExprNode **expr_node) {
        ...
        if ((*expr_node)->children(0) != nullptr && (*expr_node)->children(0)->node_type() == proto::AND_PREDICATE) {
            typedef std::pair<ExprNode *, ExprNode *> ExprNodePair;
            proto::ExprNode slot_ref_row_expr_pb_node;
            slot_ref_row_expr_pb_node.set_node_type(proto::ROW_EXPR);
            slot_ref_row_expr_pb_node.set_col_type(proto::INVALID_TYPE);
            ...
            for (int i = 0; i < (*expr_node)->children_size(); i++) {
                ...
                if (and_node == nullptr) {
                    TLOG_ERROR("and_node is nullptr");
                    return -1;
                }
                proto::ExprNode literal_row_expr_pb_node;
                literal_row_expr_pb_node.set_node_type(proto::ROW_EXPR);
                literal_row_expr_pb_node.set_col_type(proto::INVALID_TYPE);
                ...
                for (int j = 0; j < and_node->children_size(); j++) {
                    if (and_node->children(j) == nullptr) {
                        TLOG_ERROR("and_node children is nullptr");
                        return -1;
                    }
        ...
        if (slot_ref != nullptr && literal_list.size() > 0) {
            proto::ExprNode in_node;
            ExprNode *delete_expr_node = *expr_node;
            (*expr_node) = new InPredicate;
            in_node.set_col_type(proto::BOOL);
            in_node.set_node_type(proto::IN_PREDICATE);
            proto::Function *func = in_node.mutable_fn();
            func->set_name("in"); // in
    ...
    int ExprNode::serialize_tree(uint64_t &serialize_slot_id) {
        if (node_type() == proto::AND_PREDICATE) {
            for (auto &child: _children) {
                if (child == nullptr) {
                    TLOG_ERROR("child is nullptr");
                    return -1;
