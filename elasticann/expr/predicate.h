...
namespace EA {
    ...
    private:
        ...
        proto::PrimitiveType _map_type;
        std::vector<proto::PrimitiveType> _row_expr_types;
        size_t _col_size;
    ...
    public:
        ...
        std::optional<bool> like(const std::string &target, const std::string &pattern);

        bool like_one(const std::string &target, const std::string &pattern, proto::Charset charset);
    ...
    public:
        ...
        bool always_null_or_false() const {
            if (_children[0]->node_type() == proto::IN_PREDICATE) {
