...
namespace EA {
    ...
    int AggFnCall::type_inferer() {
        ...
        if (ret < 0) {
            TLOG_ERROR("ExprNode::type_inferer error");
            return ret;
        }
        switch (_agg_type) {
            case COUNT_STAR:
            case COUNT:
                _col_type = proto::INT64;
                return 0;
            case AVG:
                _col_type = proto::DOUBLE;
                return 0;
            case SUM:
                if (_children.size() == 0) {
                    return -1;
            ...
            case GROUP_CONCAT: {
                ...
                _col_type = proto::STRING;
                return 0;
            }
            default:
                TLOG_WARN("un-support agg type:{}", _agg_type);
                return -1;
        }
    }

    int AggFnCall::type_inferer(proto::TupleDescriptor *tuple_desc) {
    ...
    void AggFnCall::transfer_pb(proto::ExprNode *pb_node) {
    ...
    int AggFnCall::open() {
        ...
        switch (_agg_type) {
            ...
            case GROUP_CONCAT: {
                if (_children.size() == 0) {
                    TLOG_WARN("_agg_type:{} , _children.size() == 0", _agg_type);
                    return -1;
                }
                break;
            }
        ...
        if (_agg_type == GROUP_CONCAT) {
            ...
            if (children_size < 2) {
                TLOG_WARN("children_size {} less than 2", children_size);
                return -1;
            }
            ...
            if (children_size == 4) {
                if (!_children[2]->is_row_expr() || !_children[3]->is_row_expr() ||
                    ...
                    TLOG_WARN("children_size not equal");
                    return -1;
                }
                ...
                for (size_t i = 0; i < _children[2]->children_size(); i++) {
                    ...
                    proto::SlotDescriptor *slot = order_tuple.add_slots();
                ...
                slot->set_tuple_id(_order_tuple_id);
                slot->set_slot_type(proto::STRING);
    ...
    int AggFnCall::initialize(const std::string &key, MemRow *dst, int64_t &used_size, bool only_count) {
        ...
        if (only_count) {
            if (_agg_type == COUNT_STAR || _agg_type == COUNT) {
                if (dst_val.is_null()) {
                    dst->set_value(_tuple_id, _intermediate_slot_id, ExprValue(proto::INT64));
                }
            }
            return 0;
        }
            ...
            case GROUP_CONCAT: {
                ...
                if (_mem_row_compare != nullptr) {
                    if (!all_is_null) {
                        ...
                        ExprValue value = ExprValue(proto::STRING);
                        value.str_val = val;
                ...
                if (!all_is_null) {
                    ExprValue result = dst->get_value(_tuple_id, _intermediate_slot_id);
                    if (result.type != proto::STRING) { // is_null
                        result = ExprValue(proto::STRING);
                    } else {
                        result.str_val += _sep;
                    }
                    result.str_val += val;
    ...
    int AggFnCall::merge(const std::string &key, MemRow *src, MemRow *dst, int64_t &used_size) {
        ...
        if (src == dst) {
            ...
            if (is_bitmap_agg() || is_tdigest_agg() || is_hll_agg()) {
                if (is_bitmap_agg()) {
                    dst_value.cast_to(proto::BITMAP);
                } else if (is_tdigest_agg()) {
                    dst_value.cast_to(proto::TDIGEST);
                }
                auto &intermediate_val = _intermediate_val_map[key];
                intermediate_val.is_assign = true;
                if (is_hll_agg()) {
        ...
        switch (_agg_type) {
            ...
            case AVG: {
                ExprValue value = src->get_value(_tuple_id, _intermediate_slot_id);
                if (!value.is_null() && value.type == proto::STRING) {
                    ExprValue result = dst->get_value(_tuple_id, _intermediate_slot_id);
            ...
            case RB_BUILD_AGG: {
                ...
                if (!src_value.is_null()) {
                    src_value.cast_to(proto::BITMAP);
                    auto &intermediate_val = _intermediate_val_map[key];
                    int64_t old_used_size = intermediate_val.val.size();
