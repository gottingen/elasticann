...
namespace EA {
    ...
    int ScalarFnCall::init(const proto::ExprNode &node) {
        ...
        _fn = node.fn();
        // rand不是const
        if (node_type() == proto::FUNCTION_CALL && _fn.name() == "rand") {
            _is_constant = false;
        }
        return 0;
    }
    ...
    int ScalarFnCall::type_inferer() {
                ...
                if (_children[0]->is_slot_ref() && _children[1]->is_constant()) {
                    ...
                    for (uint32_t i = 1; i < children_size(); i++) {
                        for (auto &pair: slots) {
                            ...
                            proto::PrimitiveType tp = pair.second->col_type();
        ...
        for (auto c: _children) {
            if (c->col_type() == proto::INVALID_TYPE && !c->is_row_expr()) {
                TLOG_WARN("_children is proto::INVALID_TYPE, node:{}", c->node_type());
                return -1;
            }
            if (is_logical_and_or_not()) {
                //类型推导过程中分析表达式节点类型是否为bool型
                if (c->col_type() != proto::BOOL) {
                    TLOG_WARN(
                            "_children is not bool type, ScalarFnCall ExprNode type is [{}], children node_type_is [{}]",
                            proto::ExprNodeType_Name(_node_type).c_str(),
                            proto::ExprNodeType_Name(c->node_type()).c_str());
        ...
        if (_col_type == proto::INVALID_TYPE) {
...
// (11,22) < (a,b) => (a,b) > (11,22)
    ...
    int ScalarFnCall::open() {
        ...
        if ((int) _children.size() < _fn.arg_types_size()) {
            TLOG_WARN("_children.size:{} < _fn.arg_types_size:{}",
        ...
        if (children_size() > 0 && children(0)->is_row_expr()) {
            if (_fn.fn_op() != parser::FT_EQ &&
                ...
                TLOG_ERROR("Operand should contain 1 column(s)");
                return -1;
            }
            size_t col_size = children(0)->children_size();
            if (_fn.fn_op() == parser::FT_MATCH_AGAINST) {
                if (col_size > 1) {
                    TLOG_ERROR("MATCH_AGAINST column list support only 1, size:{}", col_size);
                    return -1;
                }
                return 0;
            }
            ...
            for (size_t i = 1; i < children_size(); i++) {
                if (!children(i)->is_row_expr() ||
                    ...
                    TLOG_ERROR("Operand should contain {} column(s)", col_size);
                    return -1;
                }
            }
        }
        ...
        if (_fn.return_type() != _col_type) {
            TLOG_WARN("_fn.return_type:{} != _col_type:{}", _fn.return_type(), _col_type);
            return -1;
        }
        for (int i = 0; i < _fn.arg_types_size(); i++) {
        ...
        if (node_type() == proto::FUNCTION_CALL && _fn_call == nullptr) {
            TLOG_WARN("fn call is null, name:{}", _fn.name().c_str());
        }
        return 0;
    }
