...
namespace EA {
    ...
    private:
        ...
        bool _send_store_raft_control_request(const SmartSocket &client, proto::RaftControlRequest &req,
                                              proto::RegionInfo &info);

        bool _make_response_packet(const SmartSocket &client, const std::string &response);

        void _make_handle_region_result_rows(const proto::MetaManagerRequest &request,
        ...
        void update_unhealthy_learners_schema(const proto::QueryRequest &query_req,
                                              proto::QueryResponse &query_res,
                                              std::vector<std::vector<std::string>> &rows);
