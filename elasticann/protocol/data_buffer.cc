...
namespace EA {
    DataBuffer::DataBuffer(size_t capacity) {
        ...
        if (_data == nullptr) {
            TLOG_ERROR("Failed to malloc memory.size:[{}]", capacity);
            return;
        }
        _capacity = capacity;
        return;
    ...
    void DataBuffer::byte_array_clear() {
        if (_capacity > DFT_ALLOC_BUF_SIZE) {
            ...
            if (nullptr == (p = (uint8_t *) malloc(DFT_ALLOC_BUF_SIZE))) {
                TLOG_WARN("byte_array_clear resize buf error.");
            } else {
                if (nullptr != _data) {
                    free(_data);
                }
    ...
    bool DataBuffer::byte_array_append_size(size_t len, int is_pow) {
        ...
        if (want_alloc > MAX_ALLOC_BUF_SIZE) {
            TLOG_ERROR("want_alloc size[{}] bigger than max[{}]",
        ...
        if (nullptr == _data) {
            TLOG_ERROR("malloc want_alloc={} append len={} failed", want_alloc, len);
            return false;
        }
...
// packet大于16M时插入4字节包头，导致内存移动，需要后续优化
    bool DataBuffer::byte_array_insert_len(const uint8_t *data, size_t start_pos, size_t len) {
        if (data == nullptr) {
            TLOG_ERROR("data buffer is null");
            return false;
        }
        if (len <= 0) {
            return true;
        ...
        if (!byte_array_append_size(len, 1)) {
            TLOG_ERROR("byte_array_append_size fail");
            return false;
        }
    ...
    bool DataBuffer::append_text_value(const ExprValue &value) {
        do {
            ...
            if (_capacity < _size) {
                TLOG_ERROR("_capacity:{} < size:{}", _capacity, _size);
                return false;
            }
            ...
            if (ret == STMPS_SUCCESS) {
                ...
                TLOG_ERROR("serialize_to_mysql_packet ret:{}", static_cast<int>(ret));
                return false;
            }
        } while (true);
    }
...
// TODO: support other mysql types
    bool DataBuffer::append_binary_value(const ExprValue &value, uint8_t type, uint8_t *null_map, int field_idx,
                                         int null_offset) {
        // TLOG_WARN("result value is: {} type: {}", value.get_string().c_str(), type);
        if (value.is_null()) {
        ...
        if (type == MYSQL_TYPE_LONGLONG) {
            ...
            TLOG_WARN("unsupported binary type: {}", type);
            return false;
        }
        return true;
    }
    ...
    bool DataBuffer::pack_length_coded_string(const std::string &str, bool is_null) {
        ...
        if (is_null) {
            ...
            if (!byte_array_append_len(&zero_byte, 1)) {
                TLOG_ERROR("Failed to append len.value:[{}],len:[1]", zero_byte);
                return false;
            }
        } else {
            ...
            if (!byte_array_append_len((const uint8_t *) str.c_str(), length)) {
                TLOG_ERROR("Failed to append len. value:[{}],len:[{}]", str.c_str(), length);
                return false;
            }
        }
        return true;
    ...
    bool DataBuffer::network_queue_send_append(
        ...
        if (len <= 0 || (data == nullptr && append_data_later == 0)) {
            TLOG_ERROR("send_buf==nullptr||len={}<= 0 ||(data == null&& append_data_later={} == 0)",
        ...
        if (!byte_array_append_len(header, 4)) {
            TLOG_ERROR("Failed to append length.length:[4]");
            return false;
        }
        if (append_data_later == 0) {
            if (!byte_array_append_len(data, len)) {
                TLOG_ERROR("Failed to append length.length:[{}]", len);
                return false;
            }
        }
        return true;
