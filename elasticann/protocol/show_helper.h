...
namespace EA {
    ...
    private:
        ...
        bool
        _handle_client_query_template_dispatch(const SmartSocket &client, const std::vector<std::string> &split_vec);
        ...
        inline ResultField
        make_result_field(const std::string &name, const uint8_t &type, const uint32_t &length) const {
            ResultField field;
            field.name = name;
            field.type = type;
            field.length = length;
