...
class Fetcher {
public:
    Fetcher(EA::client::Service* baikaldb, const std::map<std::string, EA::client::Service*>& baikaldb_map) :
            _baikaldb(baikaldb), _baikaldb_map(baikaldb_map) {
        _fs = nullptr;
    }
    ~Fetcher();
    int init();
    int querybaikaldb(std::string sql, EA::client::ResultSet& result_set);
    ...
    int select_new_task(EA::client::ResultSet& result_set, bool is_fast_importer);
...
private:
    ...
    EA::client::Service* _baikaldb_user;
...
class SqlAgg {
public:
    SqlAgg(EA::client::Service* s1, EA::client::Service* s2) : _baikaldb_info(s1), _baikaldb_task(s2) {}
...
private:
    ...
    Bthread _timer_bth;
    EA::client::Service* _baikaldb_info;
    EA::client::Service* _baikaldb_task;
};
