...
CaptureStatus FetchBinlog::run(int32_t fetch_num) {
    ...
    while (!_is_finish) {
        ...
        std::map<int64_t, proto::RegionInfo> region_map;
        ...
        for (auto& region_info : region_map) {
            ...
            auto request_binlog_proc = [this, region_info, commit_ts, fetch_num, &ret]() -> int {
                ...
                int no_data_num = 0;
                std::shared_ptr<proto::StoreRes> response(new proto::StoreRes);
...
CaptureStatus FetchBinlog::read_binlog(const proto::RegionInfo& region_info, int64_t commit_ts, int fetch_num_per_region, proto::StoreRes& response, const std::string& peer) {
    proto::StoreReq request;
    request.set_op_type(proto::OP_READ_BINLOG);
    ...
    if (ret != 0) {
        if (response.errcode() == proto::LESS_THAN_OLDEST_TS) {
...
CaptureStatus MergeBinlog::run() {
    ...
    for (auto& res_info : _fetcher_result) {
        ...
        for (const auto& binlog : res_info.second->binlogs()) {
            ...
            StoreReqPtr store_req_ptr(new proto::StoreReq);
...
int BinLogTransfer::init(const std::map<int64_t, SubTableIds>& sub_table_ids) {
    ...
    for (const auto& ids : sub_table_ids) {
        ...
        cap_info.table_info = EA::SchemaFactory::get_instance()->get_table_info_ptr(table_id);
        ...
        for (int64_t index_id : cap_info.table_info->indices) {
            SmartIndex info_ptr = EA::SchemaFactory::get_instance()->get_index_info_ptr(index_id);
            if (info_ptr == nullptr) {
        ...
        const DatabaseInfo& db_info = EA::SchemaFactory::get_instance()->get_database_info(cap_info.table_info->db_id);
        if (db_info.id == -1) {
...
void print_event(int64_t commit_ts, const std::shared_ptr<mysub::Event>& event) {
    ...
    diff_old_values += ")";
    std::string ts = EA::timestamp_to_str(event->timestamp());
...
int BinLogTransfer::single_record_to_event(const std::pair<TableRecord*, TableRecord*>& delete_insert_records, 
    ...
    event->set_charset(cap_info.table_info->charset == proto::UTF8 ? "utf8" : "gbk");
...
int BinLogTransfer::transfer_mutation(const proto::TableMutation& mutation, RecordCollection& records) {
    auto& insert_records = records.insert_records;
    auto& delete_records = records.delete_records;
...
int CapturerSingleton::init(const std::string& namespace_, const std::map<std::string, SubTableNames>& table_infos) {
    ...
    EA::BinlogNetworkServer* server = BinlogNetworkServer::get_instance();
...
int Capturer::init() {
    ...
    std::vector<std::string> split_vec = turbo::StrSplit(FLAGS_capture_tables,';', turbo::SkipEmpty());
...
int Capturer::get_binlog_regions(std::map<int64_t, proto::RegionInfo>& region_map, std::map<int64_t, int64_t>& region_id_ts_map, int64_t commit_ts) {
    region_map.clear();
