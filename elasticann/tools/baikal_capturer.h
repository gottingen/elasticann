...
#include "elasticann/proto/event.pb.h"
...
using StoreReqPtr = std::shared_ptr<EA::proto::StoreReq>;
...
public:
    ...
    int get_binlog_regions(std::map<int64_t, proto::RegionInfo>& region_map, std::map<int64_t, int64_t>& region_id_ts_map, int64_t commit_ts);
    ...
    void clear() {
        ...
        _two_way_sync.reset();
        EA::BinlogNetworkServer::get_instance()->close_schema_heartbeat();
    }
...
private:
    ...
    EA::SchemaFactory* _schema_factory {nullptr};
...
public:
    ...
    CaptureStatus read_binlog(const proto::RegionInfo& region_info, int64_t commit_ts, int fetch_num_per_region, proto::StoreRes& response, const std::string& peer);
    const std::map<int64_t, std::shared_ptr<proto::StoreRes>>& get_result() const {
        return _region_res;
    } 
...
private:
    ...
    std::map<int64_t, std::shared_ptr<proto::StoreRes>> _region_res;
...
class MergeBinlog {
public:
    MergeBinlog(Capturer* capturer, const std::map<int64_t, std::shared_ptr<proto::StoreRes>>& fetcher_result, uint64_t log_id)
...
private:
    Capturer* _capturer;
    const std::map<int64_t, std::shared_ptr<proto::StoreRes>>& _fetcher_result;
...
private:
    ...
    int deserialization(const Repeated& repeat, RecordMap& records_map, int64_t table_id) {
        ...
        for (const auto& str : repeat) {
            auto new_record = EA::SchemaFactory::get_instance()->new_record(*cap_info.table_info);
    ...
    int transfer_mutation(const proto::TableMutation& mutation, RecordCollection& records);
