...
int fun_level(std::map<int64_t, const proto::QueryRegion*>& region_map, const proto::QueryRegion& info) {
    if (info.parent() == 0) {
        return 0;
    } else {
        /*
...
int create_table(const std::string& namespace_name, const std::string& database, 
    ...
    request.set_op_type(proto::QUERY_REGION_FLATTEN);
    ...
    std::map<int64_t, const proto::QueryRegion*> region_map;
