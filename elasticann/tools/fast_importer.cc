...
int FastImporterCtrl::query_task_baikaldb(std::string sql, EA::client::ResultSet &result_set) {
    int ret = 0;
    int retry = 0;
...
int FastImporterCtrl::select_new_task(EA::client::ResultSet& result_set) {
...
int FastImporterCtrl::fetch_task() {
    ...
    while (result_set.next()) {
        ...
        std::vector<std::string> sub_paths = turbo::StrSplit(_task.file_path, ';'));
...
int FastImporterCtrl::get_table_schema() {
    ...
    req.set_op_type(proto::QUERY_SCHEMA);
    ...
    for (const auto& idx : _schema_info.indexs()) {
        if (idx.index_type() == proto::I_FULLTEXT) {
...
int FastImporterCtrl::get_store_rocks_statistic(const std::string& store,
    ...
    do {
        ...
        proto::RocksStatisticReq request;
        ...
        proto::StoreService_Stub(&channel).get_rocks_statistic(&cntl, &request, &response, NULL);
        if (cntl.Failed()) {
        ...
        if (response.errcode() == proto::SUCCESS) {
            ...
            DB_WARNING("addr: %s, errcode: %s", store.c_str(), proto::ErrCode_Name(response.errcode()).c_str());
            return -1;
        }
    } while (retry_times < 3);
...
int FastImporterImpl::init(proto::SchemaInfo& schema_info,
                           ::google::protobuf::RepeatedPtrField< ::EA::proto::RegionInfo >& region_infos) {
...
void FastImporterImpl::handle_fields(const std::string& path, const std::vector<std::vector<std::string>>& fields_vec) {
    ...
    for (int32_t i = 0; i < records.size(); ++i) {
        ...
        for (auto& idx : _indexes) {
            ...
            else if (idx->type != proto::I_PRIMARY) {
