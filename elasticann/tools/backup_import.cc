...
int BackUpImport::gen_backup_task() {
    ...
    while (result_set.next()) {
        ...
        EA::client::ResultSet tmp_result_set;
...
int BackUpImport::fetch_new_task() {
    ...
    while (result_set.next()) {
        ...
        EA::BackUp bp(meta_server_bns, resource_tag, interval_days, backup_times);
        std::unordered_set<int64_t> table_ids;
        table_ids.insert(table_id);

        int ret = bp.run_backup(table_ids, std::bind(&EA::BackUp::backup_region_streaming_with_retry, &bp,
