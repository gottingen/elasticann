...
int create_table() {
    ...
    request.set_op_type(proto::QUERY_SCHEMA);
    ...
    const proto::SchemaInfo& schema_info = response.schema_infos(0);

    //根据返回的结果创建新的建表请求
    proto::MetaManagerRequest create_table_request;
    std::cout << FLAGS_database2 << " " << FLAGS_table_name2 << "\n";
    create_table_request.set_op_type(proto::OP_CREATE_TABLE);
    ...
    for (auto& index_info : schema_info.indexs()) {
        ...
        *add_index = index_info;
        ...
        /*
        if (add_index->index_type() == proto::I_FULLTEXT) {
            add_index->set_segment_type(proto::S_UNIGRAMS);
        }
        */
    }
    ...
    */
    ...
    add_index->set_index_type(proto::I_KEY);
    */
    ...
    proto::MetaManagerResponse create_table_response;
