...
public:
    FastImporterCtrl(EA::client::Service* baikaldb,
                     const std::map<std::string, EA::client::Service*>& baikaldb_map,
                     EA::client::Service* baikaldb_user = nullptr) :
    ...
    int query_task_baikaldb(std::string sql, EA::client::ResultSet &result_set);
    ...
    int select_new_task(EA::client::ResultSet& result_set);
...
private:
    ...
    // 表和region信息
    proto::SchemaInfo _schema_info;
    ::google::protobuf::RepeatedPtrField< ::EA::proto::RegionInfo > _region_infos;

    EA::client::Service* _task_baikaldb;
    EA::client::Service* _user_baikaldb;
    const std::map<std::string, EA::client::Service*>& _baikaldb_map;
...
public:
    ...
    FastImporterImpl(const FastImportTaskDesc& task, EA::client::Service* baikaldb) : _task(task), _baikaldb(baikaldb) {};

    ~FastImporterImpl() {};

    int init(proto::SchemaInfo& schema_info,
    ...
    bool split(std::string& line, std::vector<std::string>& split_vec) {
        ...
        split_vec = turbo::StrSplit(line, turbo::ByAnyChar(_task.delim));
        return true;
    }
