...
public:
    ImporterHandle(OpType type, EA::client::Service* db) : _type(type), _baikaldb(db), _done_path("") {
        char buf[100];
    ...
    ImporterHandle(OpType type, EA::client::Service* db, std::string done_path) : _type(type), _baikaldb(db), _done_path(done_path) {
        char buf[100];
        time_t now = time(NULL);
        strftime(buf, 100, "%F_%T", localtime(&now));
        _err_name = "err_sql.";
    ...
    virtual ~ImporterHandle() {
        _err_fs.close();
        _err_fs_retry.close();
        turbo::filesystem::remove_all(_err_name);
        if (_err_cnt_retry == 0) {
            turbo::filesystem::remove_all(_err_name_retry);
        }
    }
    ...
    int query(std::string sql, int quota, EA::client::SmartConnection& connection, bool is_retry = false);

    int query(std::string sql, bool is_retry = false);

    int query_dm_baikaldb(std::string sql, EA::client::ResultSet& result_set);
    ...
    std::string _mysql_escape_string(EA::client::SmartConnection& connection, const std::string& value);

    static ImporterHandle* new_handle(OpType type,
            EA::client::Service* baikaldb,
            EA::client::Service* backup_db,
...
protected:
    virtual bool split(std::string& line, std::vector<std::string>& split_vec) {
        ...
        split_vec = turbo::StrSplit(line, turbo::ByAnyChar(_delim), turbo::SkipEmpty());
        return true;
    }
    ...
    std::string gen_insert_sql(EA::client::SmartConnection& connection, const std::string& database_name, const std::string& table_name,
    ...
    std::string gen_delete_sql(EA::client::SmartConnection& connection, const std::string& database_name, const std::string& table_name,
    ...
    std::string gen_update_sql(EA::client::SmartConnection& connection, const std::string& database_name, const std::string& table_name,
    ...
    // for upload err sqls
    EA::client::Service* _dm_baikaldb = nullptr;
...
public:
    ImporterTestHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {
        bthread_mutex_init(&_mutex, NULL);
    }

    ImporterTestHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {
        bthread_mutex_init(&_mutex, NULL);
    }
    ...
    virtual void handle_fields(const std::string& path, 
        ...
        std::vector<std::string> split_vec = turbo::StrSplit(path, '/', turbo::SkipEmpty());
...
public:
    ImporterAtomHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db),
                                        _insert_fial_file("insert_fial_file") {
        bthread_mutex_init(&_mutex, NULL);
    }

    ImporterAtomHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path),
                                        _insert_fial_file("insert_fial_file") {
        bthread_mutex_init(&_mutex, NULL);
    }
    ...
    void handle_fields_insert(const std::string& path, const std::vector<std::vector<std::string>>& fields_vec) {
        ...
        EA::client::SmartConnection connection = _baikaldb->fetch_connection();
    ...
    bool split(std::string& s, std::vector<std::string>& split_vec) override {
        std::vector<std::string> elems = turbo::StrSplit(s, turbo::ByAnyChar(_delim), turbo::SkipEmpty());
...
public:
    ImporterBigtreeDiffHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db),
    ...
    int db_query(EA::client::Service* db, std::vector<std::string>& fields,
            std::string& sql, RowsBatch& row_batch);
    int db_query(EA::client::Service* db, const std::string& field, std::string& sql, int64_t& count);
    ...
    int do_begin(EA::client::SmartConnection& conn);
    int do_commit(EA::client::SmartConnection& conn);
    int do_rollback(EA::client::SmartConnection& conn);

    bool split(std::string& s, std::vector<std::string>& split_vec) override {
        split_vec = turbo::StrSplit(s, boost::is_any_of(_delim), turbo::SkipEmpty());
        return true;
    }
...
public:
    ImporterAtomCheckHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db),
    ...
    ImporterAtomCheckHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path),
                                        _id_fial_file("id_fail_file"), _literal_fial_file("literal_fial_file"), 
                                        _id_notfound_file("id_notfound_file"), _literal_notfound_file("literal_notfound_file")  {
        bthread_mutex_init(&_mutex, NULL);
    }
...
public:
    ImporterRepHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {}
    
    ImporterRepHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {}
...
public:
    ImporterUpHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {}
    
    ImporterUpHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {}
...
public:
    ImporterTxnUpHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {}
    
    ImporterTxnUpHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {}
...
public:
    ImporterSelHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {}

    ImporterSelHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {}
...
public:
    ImporterSelpreHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {}

    ImporterSelpreHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {}
...
public:
    ImporterDelHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {}

    ImporterDelHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {}
...
public:
    ImporterSqlupHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {}

    ImporterSqlupHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {}
...
public:
    ImporterDupupHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {}

    ImporterDupupHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {}
...
public:
    ImporterBaseupHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {}

    ImporterBaseupHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {}
...
public:
    ImporterRecoveryHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {
    }

    ImporterRecoveryHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {
    }
...
public:
    ImporterDiffLineHandle(OpType type, EA::client::Service* db) : ImporterHandle(type, db) {}

    ImporterDiffLineHandle(OpType type, EA::client::Service* db, std::string done_path) : ImporterHandle(type, db, done_path) {}
