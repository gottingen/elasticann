...
class LocalImporter {
public:
    int atom_test(EA::client::Service* baikaldb);
    int import_to_baikaldb();

private:
    int handle(const Json::Value& node, OpType type,
            EA::client::Service* baikaldb, EA::client::Service* backup_db);
...
int LocalImporter::atom_test(EA::client::Service* baikaldb) {
    TimeCost time_cost;
...
int main(int argc, char **argv) {
    if (turbo::filesystem::exists("conf/gflags.conf")) {
    ...
    if (FLAGS_is_backup || FLAGS_is_send) {
        EA::BackUp bp(EA::FLAGS_meta_server_bns,
                            EA::FLAGS_backup_peer_resource_tag,
                            EA::FLAGS_interval_days,
                            EA::FLAGS_backup_times);
        bp.run();
        return 0;
    }
    int ret = EA::flash_back();
    if (ret == 1) {
        return 0;
    } else if (ret == -1) {
