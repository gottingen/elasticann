...
int ImporterHandle::init(const Json::Value& node, const std::string& path_prefix) {
    if (BASE_UP != _type && RECOVERY != _type) {
        ...
        for (auto& field : node["fields"]) {
            ...
            std::string name = field.asString();
            vec =  turbo::StrSplit(name, turbo::ByAnyChar("@"));
            name = "`" + vec[0] + "`";
    ...
    if (_done_path.empty() && !_is_local_done_json) {
        if (tmp_path[0] == '/') {
            std::vector<std::string> split_vec =  turbo::StrSplit(tmp_path, turbo::ByAnyChar("/"));
...
int ImporterHandle::query_dm_baikaldb(std::string sql, EA::client::ResultSet& result_set) {
    int ret = 0;
    int retry = 0;
...
int ImporterHandle::query(std::string sql, int quota, EA::client::SmartConnection& connection, bool is_retry) {
    if (_tps > 0) {
...
std::string ImporterHandle::_mysql_escape_string(EA::client::SmartConnection& connection, const std::string& value) {
...
void ImporterSelpreHandle::handle_fields(const std::string& path, const std::vector<std::vector<std::string>>& fields_vec, BlockHandleResult*) {
    ...
    std::string insert_values;
    int cnt = 0;
    EA::client::SmartConnection connection;
    MYSQL* conn;
...
int ImporterBigtreeDiffHandle::do_begin(EA::client::SmartConnection& conn) {
    return conn->execute("begin", true, NULL);
}

int ImporterBigtreeDiffHandle::do_commit(EA::client::SmartConnection& conn ) {
    return conn->execute("commit", true, NULL);
}

int ImporterBigtreeDiffHandle::do_rollback(EA::client::SmartConnection& conn) {
...
int ImporterBigtreeDiffHandle::do_insert(const std::string& table_name, RowsBatch& old_row_batch) {
    TimeCost step_time;
    auto& table_fields = table_fields_map[table_name];
    EA::client::SmartConnection conn = _baikaldb->fetch_connection(0);
    if (!conn) {
...
void ImporterBigtreeDiffHandle::gen_count_rows(const std::string& table_name, const RowsBatch& select_rows, std::map<int, RowsBatch>& count_rows_map) {
    ...
    for (auto& iter : count_map) {
        DataRow row;
        row.reserve(4);
     row =  turbo::StrSplit(iter.first, turbo::ByAnyChar("_"));
...
int ImporterBigtreeDiffHandle::db_query(EA::client::Service* db, const std::string& field, std::string& sql, int64_t& count) {
    int ret = 0;
    int retry = 0;
    do {
        EA::client::ResultSet result_set;
...
int ImporterBigtreeDiffHandle::db_query(EA::client::Service* db, std::vector<std::string>& fields, std::string& sql, RowsBatch& row_batch) {
    int ret = 0;
    int retry = 0;
    do {
        EA::client::ResultSet result_set;
...
void ImporterRecoveryHandle::handle_fields(const std::string& path, const std::vector<std::vector<std::string>>& fields_vec, BlockHandleResult*) {
    ...
    std::vector<std::string> vec =  turbo::StrSplit( path, turbo::ByAnyChar("/"));
    if (vec.empty()) {
