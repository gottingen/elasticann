...
namespace bfs = turbo::filesystem;
namespace EA {
    ...
    for (auto tid : tids) {
        ...
        for (auto i = bfs::directory_iterator(tpath); i!=bfs::directory_iterator(); ++i) {
            if (!bfs::is_directory(i->path())) {
                ...
                std::vector<std::string> seps = turbo::StrSplit(filename, '_');
                if (seps.size() == 3) {                        
...
std::vector<RegionFile> get_region_files(std::string path, int64_t region_id, std::string upload_date) {
    ...
    for (auto i = bfs::directory_iterator(tpath); i!=bfs::directory_iterator(); ++i) {
        if (!bfs::is_directory(i->path())) {
            auto filename = i->path().filename().string();
            std::vector<std::string> seps = turbo::StrSplit(filename, '_', turbo::SkipEmpty());
...
int BackUp::get_meta_info(const proto::QueryRequest& request, proto::QueryResponse& response) {
...
int BackUp::backup_region_streaming_with_retry(int64_t table_id, int64_t region_id, 
    ...
    do {
        ...
        request.set_op_type(proto::QUERY_REGION);
        request.add_region_ids(region_id);

        if (get_meta_info(request, response) != 0) {
            if (response.errcode() == proto::REGION_NOT_EXIST) {
                // 被merge了
...
void BackUp::gen_backup_task(std::unordered_map<std::string, std::vector<TaskInfo>>& tasks, 
    ...
    const proto::QueryResponse& response,
...
int BackUp::run_backup(std::unordered_set<int64_t>& table_ids, 
    ...
    if (!_pefered_peer_resource_tag.empty()) {
        ...
        proto::QueryRequest request;
        proto::QueryResponse response;
        request.set_op_type(proto::QUERY_INSTANCE_FLATTEN);
...
void BackUp::run_upload_from_region_id_files(const std::string& meta_server, int64_t table_id, std::unordered_set<int64_t> region_ids) {
    ...
    for (auto region_id : region_ids) {
        gen_upload_task(task, response, [region_id, table_id](const proto::RegionInfo& ri) -> bool {
...
void BackUp::run_upload_from_meta_info(const std::string& meta_server, int64_t table_id) {
    ...
    UploadTask task;
    gen_upload_task(task, response, [table_id](const proto::RegionInfo& ri) -> bool {
...
void BackUp::run_upload_from_meta_json_file(std::string& meta_server, std::string meta_json_file) {
    ...
    if (recover_response.isMember("inited_regions") && recover_response["inited_regions"].isArray()) {
        ...
        gen_upload_task(task, response, [&region_ids](const proto::RegionInfo& ri) -> bool {
...
void BackUp::request_sst_streaming(std::string url, int64_t table_id, int64_t region_id, 
    ...
    ON_SCOPE_EXIT(([&status, receiver_handle, region_id, table_id, stream, &outfile_name](){
        ...
        if (receiver_handle->get_status() == proto::StreamState::SS_FAIL ||
            receiver_handle->get_status() == proto::StreamState::SS_PROCESSING) {
        ...
        if (receiver_handle->get_status() != proto::StreamState::SS_SUCCESS
            && !outfile_name.empty()){
    ...
    proto::StoreService_Stub stub(&channel);
    proto::BackupRequest request;
    proto::BackupResponse response;
    request.set_region_id(region_id);
    request.set_log_index(log_index);
    request.set_backup_op(proto::BACKUP_DOWNLOAD);
    
    stub.backup(&cntl, &request, &response, NULL);
    if (cntl.Failed()) {
        auto errcode = response.errcode();
        if (errcode == proto::BACKUP_SAME_LOG_INDEX) {
    ...
    while (receiver_handle->get_status() == proto::StreamState::SS_INIT ||
        receiver_handle->get_status() == proto::StreamState::SS_PROCESSING) {
        if (_shutdown) {
            DB_NOTICE("shutdown...");
            butil::DeleteFile(butil::FilePath(outfile_name), false); 
...
Status BackUp::send_sst_streaming(std::string peer, std::string infile, int64_t table_id, int64_t region_id,
    ...
    stub.backup(&cntl, &request, &response, NULL);
    if (cntl.Failed()) {
        if (response.errcode() == proto::RETRY_LATER) {
    ...
    while (wait_time.get_time() < 10 * 60 * 1000 * 1000LL) {
        proto::StreamState state = get_streaming_result_from_store(peer, response.streaming_id(), region_id);
        if (state == proto::StreamState::SS_SUCCESS) {
            return Status::Succss;
        } else if (state == proto::StreamState::SS_FAIL) {
            return Status::Error;
        }
...
Status BackUp::get_region_peers_from_leader(const std::string& peer, int64_t region_id, 
    ...
    do {
        ...
        proto::BackupResponse response;
        request.set_region_id(region_id);
        request.set_backup_op(proto::BACKUP_QUERY_PEERS);
        proto::StoreService_Stub(&channel).backup(&cntl, &request, &response, NULL);
        if (cntl.Failed()) {
        ...
        if (response.errcode() == proto::NOT_LEADER) {
            ...
            DB_WARNING("region_id: %ld, addr: %s, errcode: %s", 
                region_id, leader.c_str(), proto::ErrCode_Name(response.errcode()).c_str());
            return Status::Error;
        }
...
proto::StreamState BackUp::get_streaming_result_from_store(const std::string& peer, brpc::StreamId server_id, int64_t region_id) {
    int retry_times = 0;
    TimeCost time_cost;
    std::string leader = peer;
    ...
    do {
        ...
        request.set_backup_op(proto::BACKUP_QUERY_STREAMING);
        proto::StoreService_Stub(&channel).backup(&cntl, &request, &response, NULL);
        if (cntl.Failed()) {
            DB_WARNING("cntl failed, region_id: %ld, addr: %s, err: %s",
                       region_id, leader.c_str(), cntl.ErrorText().c_str());
        ...
        if (response.errcode() == proto::SUCCESS) {
            ...
            DB_WARNING("region_id: %ld, addr: %s, stream_id: %lu, errcode: %s",
                       region_id, leader.c_str(), server_id, proto::ErrCode_Name(response.errcode()).c_str());
            return proto::StreamState::SS_FAIL;
        }

    } while (retry_times < 3);

    if (retry_times >= 3) {
        return proto::StreamState::SS_INIT;
    }
    return proto::StreamState::SS_INIT;
}
    ...
    if (FLAGS_is_backup) {
        ...
        if (FLAGS_table_file == "") {
            ...
            if (FLAGS_is_backup_all_tables) {
                ...
                int ret = run_backup(table_ids, std::bind(&EA::BackUp::backup_region_streaming_with_retry, this,
