...
int main(int argc, char** argv) {
    ...
    int64_t start_ts = EA::timestamp_to_ts((uint32_t)now - FLAGS_start_before_now_h * 3600);
    if (FLAGS_start_ts != 0) {
    ...
    if (!FLAGS_skip_region_ids.empty()) {
        std::vector<std::string> split_vec = turbo::StrSplit(FLAGS_skip_region_ids,',');
    ...
    EA::ConcurrencyBthread req_threads {FLAGS_concurrency, &BTHREAD_ATTR_NORMAL};
