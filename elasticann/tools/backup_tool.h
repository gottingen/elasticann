...
public:
    virtual int on_received_messages(baidu::rpc::StreamId id, 
        ...
        switch (_status) {
        case proto::StreamState::SS_INIT: {
            ...
            if (_to_process_size == 0) {
                ...
                _status = proto::StreamState::SS_PROCESSING;
            } else {
                break;
            }
        }
        default: {
            ...
            if (_all_size == _recived_size) {
                ...
                _status = _ofs.bad() ? proto::StreamState::SS_FAIL : proto::StreamState::SS_SUCCESS;
...
public:
    ...
    static proto::StreamState get_streaming_result_from_store(const std::string& peer, brpc::StreamId server_id, int64_t region_id);
private:
    ...
    void gen_upload_task(UploadTask& task, 
        const proto::QueryResponse& response, std::function<bool(const proto::RegionInfo& ri)> predicate) {
...
private:
    int get_meta_info(const proto::QueryRequest& request, proto::QueryResponse& response);
