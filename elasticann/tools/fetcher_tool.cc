...
#include "elasticann/proto/fc.pb.h"
#include "time.h"
...
int Fetcher::querybaikaldb(std::string sql, EA::client::ResultSet& result_set) {
    int ret = 0;
    int retry = 0;
    do {
...
void Fetcher::prepare() {
    ...
    std::vector<std::string> split_info = turbo::StrSplit(_table_info, '.');
    _import_db = split_info.front();
    _import_tbl = split_info.back();
    if (!_is_local_done_json) {
        std::vector<std::string> split_vec = turbo::StrSplit(_done_file, '/');
...
int Fetcher::import_to_baikaldb() {
    ...
    int64_t start_ts = EA::timestamp_to_ts((uint32_t)now);
            ...
            if (done_root.isMember(name_type.name)) {
                ...
                for (auto& node : done_root[name_type.name]) {
                    ...
                    if (node.isMember("mail") && node.isString()) {
                        ...
                        std::vector<std::string> users = turbo::StrSplit(mail, turbo::ByAnyChar(" ;,\n\t\r"));
                        for (auto& user : users) {
                            mails += user;
                    ...
                    if (_is_fast_importer) {
                        ...
                        if (_done_real_path.empty() && !_is_local_done_json) {
                            if (tmp_path[0] == '/') {
                                std::vector<std::string> split_vec = turbo::StrSplit(tmp_path, '/'));
                        ...
                        EA::FastImporterCtrl fast_importer_main_worker(_baikaldb,
                                                                             _baikaldb_map,
                                                                             _baikaldb_user);
...
int Fetcher::select_new_task(EA::client::ResultSet& result_set, bool is_fast_importer) {
...
int Fetcher::exec_user_sql() {
    ...
    int ret = 0;
    //执行用户sql
    std::vector<std::string> split_sql = turbo::StrSplit(user_sql, ';');
...
int SqlAgg::alarm(int date, const AlarmRule& rule) {
    ...
    while (result_set.next()) {
        ...
        std::vector<std::string> table_split_vec = turbo::StrSplit(tbl, ',');
        std::vector<std::string> family_split_vec = turbo::StrSplit(family, ',');
        std::vector<std::string> sign_split_vec= turbo::StrSplit(sign, ',');
        ...
        for (auto& db : family_split_vec) {
            for (auto& t : table_split_vec) {
                if (t == "ALL") {
                    for (auto& iter : rule) {
                        std::vector<std::string> split_vec2 = turbo::StrSplit(iter.first, '.');
                        if (db != split_vec2[0]) {
                            continue;
                        }
...
int main(int argc, char **argv) {
    ...
    signal(SIGINT, (sighandler_t)EA::handle_dm_signal);
    signal(SIGTERM, (sighandler_t)EA::handle_dm_signal);
    ...
    std::vector<std::string> split_info = turbo::StrSplit(EA::FLAGS_baikaldb_resource, ',', turbo::SkipEmpty());
    DB_WARNING("baikaldb_resource:%s", EA::FLAGS_baikaldb_resource.c_str());

    std::map<std::string, EA::client::Service*> baikaldb_map;
    ...
    if (EA::DMRocksWrapper::get_instance()->init(EA::FLAGS_db_path) != 0) {
        DB_FATAL("DMRocksWrapper init failed");
        return -1;
    }
    DB_WARNING("int baikaldb service success");

    if (EA::FLAGS_is_agg_sql) {
    ...
    // 重启之后的一些初始化操作
    if (EA::FLAGS_is_hdfs_import || EA::FLAGS_is_fast_importer) {
        EA::Fetcher fetcher(baikaldb, baikaldb_map);
        // 1. 更新残留任务的result table信息
        fetcher.update_result_task_fail();
    }

    EA::SqlAgg sqlagg(baikaldb, baikaldb_trace);
    if (EA::FLAGS_is_agg_sql) {
        DB_WARNING("sql agg bthread run");
        sqlagg.run();
    }

    EA::Bthread backup_fetch_worker {&BTHREAD_ATTR_NORMAL};
    backup_fetch_worker.run([&baikaldb](){
        EA::BackUpImport backup(baikaldb);
        while (!EA::Fetcher::is_shutdown()) {
            if (EA::FLAGS_is_sst_backup) {
                // 产生新任务
    ...
    EA::Bthread fast_main_worker {&BTHREAD_ATTR_NORMAL};
    fast_main_worker.run([&baikaldb, &baikaldb_map](){
        while (!EA::Fetcher::is_shutdown()) {
            // 快速导入主任务，生成子任务，等待子任务执行完
            if (EA::FLAGS_is_fast_importer) {
                EA::Fetcher fetcher(baikaldb, baikaldb_map);
    ...
    importer_worker.run([&baikaldb, &baikaldb_map](){
        ...
        while (!EA::Fetcher::is_shutdown()) {
            ...
            // 处理快速导入子任务
            if (EA::FLAGS_is_fast_importer) {
                EA::FastImporterCtrl fast_importer_sub_worker(baikaldb, baikaldb_map);
    ...
    EA::GenericRateLimiter::get_instance()->stop();
    EA::DMRocksWrapper::get_instance()->close();
