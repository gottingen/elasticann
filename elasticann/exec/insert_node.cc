...
namespace EA {
    int InsertNode::init(const proto::PlanNode &node) {
        int ret = 0;
        ...
        const proto::InsertNode &insert_node = node.derive_node().insert_node();
        ...
        TLOG_DEBUG("_row_ttl_duration:{}", _row_ttl_duration);
        ...
        if (!_insert_values.empty()) {
            proto::DerivePlanNode *derive = _pb_node.mutable_derive_node();
    ...
    int InsertNode::open(RuntimeState *state) {
        ...
        for (auto expr: _update_exprs) {
            ret = expr->open();
            if (ret < 0) {
                TLOG_WARN("{}, expr open fail, ret:{}", *state, ret);
                return ret;
            }
        }
        ...
        // cstore下只更新涉及列
        if (_is_replace && _table_info->engine == proto::ROCKSDB_CSTORE) {
        ...
        //TLOG_WARN("{}, insert_size:{}", *state, cnt);
    ...
    void InsertNode::transfer_pb(int64_t region_id, proto::PlanNode *pb_node) {
        ...
        for (const auto index_id: table_info->indices) {
            ...
            if (!info_ptr->is_global &&
                (info_ptr->state == proto::IS_WRITE_ONLY || info_ptr->state == proto::IS_WRITE_LOCAL)) {
    ...
    int InsertNode::insert_values_for_prepared_stmt(std::vector<SmartRecord> &insert_records) {
        if (_selected_field_ids.size() == 0) {
            TLOG_WARN("not execute a prepared stmt");
            return 0;
        }
        if ((_insert_values.size() % _selected_field_ids.size()) != 0) {
            TLOG_WARN("_selected_field_ids should not be empty()");
            return -1;
        }
        ...
        for (size_t row_idx = 0; row_idx < row_size; ++row_idx) {
            ...
            for (size_t col_idx = 0; col_idx < _selected_field_ids.size(); ++col_idx) {
                ...
                //TLOG_WARN("expr type:{} field type: {}", expr->node_type(), insert_fields[col_idx].type);
                expr->close();
            }
            ...
            //TLOG_WARN("DEBUG row: {}", row->to_string().c_str());
