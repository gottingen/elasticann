...
namespace EA {
    ...
    static int predicate_cut(SlotPredicate &preds, std::set<ExprNode *> &cut_preds) {
        ...
        auto replace_slot = [&get_slot_ref_idx](ExprValue eq, ExprNode *expr) {
            ...
            if (expr->children(0)->is_row_expr()) {
                ...
                if (idx < 0) {
                    TLOG_WARN("get_slot_ref_idx fail:{}", idx);
                    return;
                }
        ...
        if (preds.in_preds.size() > 0) {
            ...
            if (preds.in_preds[0]->children(0)->is_row_expr()) {
                ...
                if (idx0 < 0) {
                    TLOG_WARN("get_slot_ref_idx fail:{}", idx0);
                    return 0;
                }
            ...
            TLOG_DEBUG("in size:{}", preds.in_preds[0]->children_size() - 1);
    ...
    int FilterNode::expr_optimize(QueryContext *ctx) {
        ...
        for (auto &expr: _conjuncts) {
            ...
            //非bool型表达式判断
            if (expr->col_type() != proto::BOOL) {
                ExprNode::_s_non_boolean_sql_cnts << 1;
                TLOG_WARN("current sql [{}] is not bool type, current exprnode_type is [{}]",
                           ctx->sql.c_str(), proto::ExprNodeType_Name(expr->node_type()).c_str());
                if (FLAGS_open_nonboolean_sql_forbid) {
                    return NOT_BOOL_ERRCODE;
                }
            }
            // TODO 除了not in外，其他计算null的地方在index_selector判断了，应该统一处理
            if (expr->node_type() == proto::NOT_PREDICATE) {
                if (static_cast<NotPredicate *>(expr)->always_null_or_false()) {
                    TLOG_WARN("expr not is always null or false");
            ...
            if (expr->children(0)->is_row_expr()) {
                ...
                for (auto &pair: slots) {
                    ...
                    if (expr->node_type() == proto::IN_PREDICATE) {
                        pred_map[sign].in_preds.emplace_back(expr);
                    } else if (expr->node_type() == proto::FUNCTION_CALL) {
        ...
        for (auto &pair: pred_map) {
            if (pair.second.need_cut()) {
                ...
                if (ret < 0) {
                    TLOG_WARN("expr is always false");
                    ctx->return_empty = true;
                    return 0;
                }
            }
    ...
    int FilterNode::predicate_pushdown(std::vector<ExprNode *> &input_exprs) {
        //having条件暂时不下推
        if (_node_type == proto::HAVING_FILTER_NODE) {
        ...
        if (_children.size() != 1) {
            TLOG_WARN("filter node pushdown fail");
            return -1;
        }
    ...
    int FilterNode::open(RuntimeState *state) {
        ...
        if (ret < 0) {
            TLOG_WARN("{}, ExecNode::open fail, ret:{}", *state, ret);
            return ret;
        }
    ...
    void FilterNode::transfer_pb(int64_t region_id, proto::PlanNode *pb_node) {
        ...
        if (!_is_explain) {
            if (scan_node != nullptr && scan_node->current_use_global_backup()) {
                ...
                if (FLAGS_enable_debug) {
                    ...
                    TLOG_DEBUG("filter_node: {}", filter_node.ShortDebugString().c_str());
                }
#endif
    ...
    int FilterNode::get_next(RuntimeState *state, RowBatch *batch, bool *eos) {
        ...
        while (1) {
            ...
            if (_child_row_batch.is_traverse_over()) {
                ...
                if (_child_eos &&
                    ...
                    *eos = true;
                    ...
                    if (ret < 0) {
                        TLOG_WARN("{}, _children get_next fail", *state);
                        return ret;
                    }
                    //TLOG_WARN("{}, _child_row_batch:{} {}", *state, _child_row_batch.capacity(), _child_row_batch.size());
                    //TLOG_INFO("scan cost:{}", cost.get_time());
                    continue;
                }
            }
            ...
            if (reached_limit()) {
                TLOG_WARN("{}, reach limit size:{}", *state, batch->size());
                *eos = true;
                return 0;
            }
