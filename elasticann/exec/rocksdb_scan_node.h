...
public:
    ...
    RocksdbScanNode(proto::Engine engine): ScanNode(engine) {
    }
...
private:
    ...
    int multi_get_next(proto::StorageType st, SmartRecord record) {
        if (st == proto::ST_PROTOBUF_OR_FORMAT1) {
            return _m_index.get_next(record);
        } else if (st == proto::ST_ARROW) {
            return _m_arrow_index.get_next(record);
        }
        return -1;
    }
    bool multi_valid(proto::StorageType st) {
        if (st == proto::ST_PROTOBUF_OR_FORMAT1) {
...
private:
    ...
    proto::DDLType _ddl_work_type = proto::DDL_NONE;
    ...
    std::vector<proto::MatchMode> _match_modes;
    ...
    proto::StorageType _storage_type = proto::ST_UNKNOWN;
