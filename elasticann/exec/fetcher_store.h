...
#define TLOG_DONE(level, _fmt_, args...) \
    do {\
        TLOG_##level("old_region_id: {}, region_id: {}, retry_times: {}, start_seq_id: {}, current_seq_id: {}, "   \
                   "op_type: {}, log_id: {}, txn_id: {}, sign: {}, addr: {}, backup: {}; " _fmt_,               \
            _old_region_id, _region_id, _retry_times, _start_seq_id, _current_seq_id,                              \
            proto::OpType_Name(_op_type).c_str(), _state->log_id(), _state->txn_id, _state->sign,                     \
            _addr.c_str(), _backup.c_str(), ##args);                                                               \
    } while (0);

    class OnRPCDone : public google::protobuf::Closure {
    public:
        OnRPCDone(FetcherStore *fetcher_store, RuntimeState *state, ExecNode *store_request,
        ...
        std::shared_ptr<proto::TraceNode> get_trace() {
            return _trace_node;
        }
    ...
    private:
        ...
        proto::RegionInfo &_info;
        int64_t _old_region_id = 0;
        int64_t _region_id = 0;
        int _retry_times = 0;
        int _start_seq_id = 0;
        int _current_seq_id = 0;
        const proto::OpType _op_type;
        ...
        proto::StoreReq _request;
        proto::StoreRes _response;
        bool _has_fill_request = false;
        std::shared_ptr<proto::TraceNode> _trace_node = nullptr;
    ...
    private:
        struct TaskGroup {
            ...
            ~TaskGroup() {
                if (doing_cnt != 0) {
                    TLOG_ERROR("memory leak, doing_cnt: {}", doing_cnt);
                }
...
// 全局二级索引降级使用，将主备请求分别发往不同集群
    ...
    public:
        ...
        void send_request(RuntimeState *state,
            ...
            std::set<std::shared_ptr<proto::TraceNode>> traces;
        ...
        static void choose_opt_instance(int64_t region_id, Repeated &&peers, std::string &addr,
                                        proto::Status &addr_status, std::string *backup,
                                        const std::set<std::string> &cannot_access_peers = {}) {
            SchemaFactory *schema_factory = SchemaFactory::get_instance();
            addr_status = proto::NORMAL;
            ...
            if (normal_peers.size() > 0) {
                ...
                addr_status = proto::FAULTY;
                TLOG_DEBUG("all peer faulty, {}", region_id);
            }
        }

        bool need_process_binlog(RuntimeState *state, proto::OpType op_type) {
        ...
        static void choose_other_if_dead(proto::RegionInfo &info, std::string &addr);

        static void other_normal_peer_to_leader(proto::RegionInfo &info, std::string &addr);

        ErrorType process_binlog_start(RuntimeState *state, proto::OpType op_type);

        void process_binlog_done(RuntimeState *state, proto::OpType op_type) {
            binlog_cond.wait();
        }
        ...
        static int64_t get_dynamic_timeout_ms(ExecNode *store_request, proto::OpType op_type, uint64_t sign);

        static int64_t get_sign_latency(proto::OpType op_type, uint64_t sign);
    ...
    void OnRPCDone::select_valid_peers(const std::string &resource_tag,
        ...
        for (auto &peer: peers) {
            ...
            if (!_fetcher_store->peer_status.can_access(_info.region_id(), peer)) {
                // learner/peer异常时报警
                TLOG_DONE(WARN, "has abnormal peer/learner: {}", peer.c_str());
                continue;
            }
