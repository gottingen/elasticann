...
namespace EA {
    ...
    public:
        ...
        void get_node(const proto::PlanNodeType node_type, std::vector<ExecNode *> &exec_nodes);

        ExecNode *get_node(const proto::PlanNodeType node_type);

        ExecNode *get_parent() {
            return _parent;
        }

        ExecNode *get_parent_node(const proto::PlanNodeType node_type) {
        ...
        void join_get_scan_nodes(const proto::PlanNodeType node_type, std::vector<ExecNode *> &exec_nodes);
        ...
        virtual void replace_slot_ref_to_literal(const std::set<int64_t> &sign_set,
            ...
            for (auto child: _children) {
                if (child->node_type() == proto::JOIN_NODE || child->node_type() == proto::APPLY_NODE) {
                    continue;
                }
        ...
        ExecNode *get_specified_node(const proto::PlanNodeType node_type) {
        ...
        void set_trace(proto::TraceNode *trace) {
            _trace = trace;
        }

        proto::TraceNode *get_trace() {
            return _trace;
        }
        ...
        proto::PlanNode *mutable_pb_node() {
            return &_pb_node;
        }

        const proto::PlanNode &pb_node() {
            return _pb_node;
        }

        proto::PlanNodeType node_type() {
            return _node_type;
        }

        bool is_filter_node() {
            return _node_type == proto::TABLE_FILTER_NODE ||
                   _node_type == proto::WHERE_FILTER_NODE ||
                   _node_type == proto::HAVING_FILTER_NODE;
        }
        ...
        void set_region_infos(std::map<int64_t, proto::RegionInfo> region_infos) {
        ...
        static void create_pb_plan(int64_t region_id, proto::Plan *plan, ExecNode *root);

        static int create_tree(const proto::Plan &plan, ExecNode **root);
        ...
        virtual proto::LockCmdType lock_type() { return proto::LOCK_INVALID; }
    ...
    protected:
        ...
        proto::PlanNodeType _node_type;
        std::vector<ExecNode *> _children;
        ExecNode *_parent = nullptr;
        proto::PlanNode _pb_node;
        std::map<int64_t, proto::RegionInfo> _region_infos;
        proto::TraceNode *_trace = nullptr;
    ...
    private:
        static int create_tree(const proto::Plan &plan, int *idx, ExecNode *parent,
                               ExecNode **root);

        static int create_exec_node(const proto::PlanNode &node, ExecNode **exec_node);
    };
