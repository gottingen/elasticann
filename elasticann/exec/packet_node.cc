...
//        TLOG_WARN("col_type: {}, col_flag: {}", expr->col_type(), expr->col_flag());
            if (is_binary(expr->col_flag()) && is_string(expr->col_type())) {
                _fields[i].type = MYSQL_TYPE_BLOB;
//            TLOG_WARN("MYSQL_TYPE_BLOB: {}", _fields[i].name.c_str());
        ...
        if (ret < 0) {
            TLOG_WARN("ExecNode::optimize fail:{}", ret);
            return ret;
        }
        return 0;
    }
    ...
    void
    PacketNode::pack_trace2(std::vector<std::map<std::string, std::string>> &info, const proto::TraceNode &trace_node,
        ...
        if (trace_node.has_instance() && trace_node.has_region_id()) {
            ...
            if (info.size() > 0) {
                ...
                if (trace_node.node_type() == proto::SCAN_NODE && trace_node.get_next_trace().has_time_cost_us()) {
    ...
    int PacketNode::open(RuntimeState *state) {
        ...
        int ret = 0;
        if (!_return_empty || op_type() == proto::OP_SELECT) {
        ...
        if (op_type() != proto::OP_SELECT && op_type() != proto::OP_UNION) {
        ...
        do {
            ...
            for (batch.reset(); !batch.is_traverse_over(); batch.next()) {
                ...
                if (ret < 0) {
                    TLOG_WARN("pack_row fail:{}", ret);
                    return ret;
                }
            }
        } while (!eos);
        //TLOG_WARN("txn_id: {}, pack_time: {}", state->txn_id, pack_time);
        pack_eof();
        return 0;
    }
    ...
    int PacketNode::open_histogram(RuntimeState *state) {
        ...
        if (stat_ptr == nullptr) {
            TLOG_WARN("can`t find statistics");
            return -1;
        }
        std::vector<std::string> names;
        std::vector<std::vector<std::string>> rows;
    ...
    int PacketNode::open_cmsketch(RuntimeState *state) {
        ...
        if (stat_ptr == nullptr) {
            TLOG_WARN("can`t find statistics table_id:{}", table_id);
            return -1;
        }
        std::vector<std::string> names;
        std::vector<std::vector<std::string>> rows;
        ...
        if (cmsketch_ptr == nullptr) {
            TLOG_WARN("can`t find cmsketch, table_id:{}, field_id:{}", table_id, field_id);
            return -1;
        }
    ...
    int PacketNode::open_analyze(RuntimeState *state) {
        ...
        request.set_op_type(proto::OP_UPDATE_STATISTICS);
        proto::Statistics *stat = request.mutable_statistics();
        ...
        if (state->cmsketch != nullptr) {
            proto::CMsketch *cmsketch = stat->mutable_cmsketch();
    ...
    int PacketNode::pack_ok(int num_affected_rows, NetworkSocket *client) {
        ...
        int64_t last_insert_id = (op_type() == proto::OP_INSERT || op_type() == proto::OP_UPDATE)
                                 ? client->last_insert_id : 0;
...
// use for make_stmt_prepare_ok_packet
    ...
    int PacketNode::pack_binary_row(MemRow *row) {
        ...
        // TLOG_WARN("nullptr-Bitmap: {}", str_to_hex(null_map_str).c_str());
