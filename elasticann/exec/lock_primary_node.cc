...
namespace EA {
    ...
    int LockPrimaryNode::init(const proto::PlanNode &node) {
        ...
        const proto::LockPrimaryNode &lock_primary_node = node.derive_node().lock_primary_node();
    ...
    void LockPrimaryNode::transfer_pb(int64_t region_id, proto::PlanNode *pb_node) {
    ...
    int LockPrimaryNode::open(RuntimeState *state) {
        ...
        for (auto &str_record: _pb_node.derive_node().lock_primary_node().put_records()) {
            ...
            if (txn->fits_region_range_for_primary(*_pri_info, record, fit_region) < 0) {
                TLOG_WARN("fits_region_range_for_primary fail, region_id: {}, table_id: {}",
        ...
        switch (_lock_type) {
            ...
            case proto::LOCK_GET: {
                ...
                //只对主表加锁返回
            case proto::LOCK_GET_ONLY_PRIMARY: {
                txn->set_separate(false); // 只加锁不走kv模式
                for (auto &record: put_records) {
                    //TLOG_WARN("{}, record:{}", *state,record->debug_string().c_str());
                    ...
                    if (ret == 0) {
                        ...
                        TLOG_WARN("{}, reverse main table must exist",*state);
                        return -1;
                    }
                    //TLOG_WARN("{}, record:{}",*state, record->debug_string().c_str());
                }
            ...
            case proto::LOCK_GET_DML: {
                for (auto &record: delete_records) {
                    ...
                    if (ret == 1 && _lock_type == proto::LOCK_GET_DML) {
                        _return_records[_pri_info->id].emplace_back(record);
                    }
                    //TLOG_WARN("{}, record:{}", *state,record->debug_string().c_str());
    ...
    int LockPrimaryNode::lock_get_main_table(RuntimeState *state, SmartRecord record) {
        ...
        if (ret == -1 || ret == -5) {
            TLOG_WARN("get lock fail txn_id: {}", txn->txn_id());
            return -1;
        }
        //代表存在
        if (ret == 0) {
            //TLOG_WARN("{}, record:{}", *state,primary_record->debug_string().c_str());
        ...
        for (auto &index_ptr: _affected_indexes) {
            ...
            //TLOG_WARN("{}, record:{}", *state,get_record->debug_string().c_str());
        }
        return 0;
    }
    ...
    int LockPrimaryNode::put_row(RuntimeState *state, SmartRecord record) {
        ...
        for (auto &info_ptr: _affected_indexes) {
            ...
            if (ret < 0) {
                TLOG_WARN("{}, put index:{} fail:{}, table_id:{}", *state,info_ptr->id, ret, _table_id);
                return ret;
            }
        }
