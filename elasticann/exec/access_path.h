...
enum IndexHint {
    ...
    void fetch_field_ids() {
        if (index_type == proto::I_KEY || index_type == proto::I_UNIQ || index_type == proto::I_PRIMARY) {
    ...
    void insert_no_cut_condition(const std::map<ExprNode*, std::unordered_set<int32_t>>& expr_field_map) {
        for (auto& pair : expr_field_map) {
            ...
            if (need_cut_index_range_condition.count(expr) == 0) {
                ...
                if (all_in_index(expr_field_ids, cover_field_ids) && index_type != proto::I_PRIMARY) {
    ...
    void calc_is_covering_index(const proto::TupleDescriptor& desc, std::set<int32_t>* slot_ids = nullptr) {
    ...
    bool is_cover_index() {
        return is_covering_index || index_type == proto::I_PRIMARY;
    }
...
public:
    ...
    proto::IndexType index_type = proto::I_NONE;
