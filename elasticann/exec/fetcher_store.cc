...
OnRPCDone::OnRPCDone(FetcherStore* fetcher_store, RuntimeState* state, ExecNode* store_request, proto::RegionInfo* info_ptr,
...
void OnRPCDone::select_addr() {
    ...
    if (FLAGS_fetcher_resource_tag != "" && _retry_times == 0 && _op_type == proto::OP_SELECT && _state->txn_id == 0) {
...
ErrorType OnRPCDone::handle_response(const std::string& remote_side) {
    ...
    if (_response.errcode() == proto::DISABLE_WRITE_TIMEOUT || _response.errcode() == proto::RETRY_LATER || _response.errcode() == proto::IN_PROCESS) {
        DB_DONE(WARNING, "request failed, errcode: %s", proto::ErrCode_Name(_response.errcode()).c_str());
        bthread_usleep(_retry_times * FLAGS_retry_interval_us);
        return E_RETRY;
    }
    ...
    if (_response.errcode() == proto::REGION_NOT_EXIST || _response.errcode() == proto::INTERNAL_ERROR) {
        DB_DONE(WARNING, "new_leader:%s，errcode: %s", _response.leader().c_str(), proto::ErrCode_Name(_response.errcode()).c_str());
    ...
    if (_response.errcode() != proto::SUCCESS) {
        ...
        DB_DONE(WARNING, "errcode:%s, mysql_errcode:%d, msg:%s, failed",
                proto::ErrCode_Name(_response.errcode()).c_str(), _response.mysql_errcode(), _response.errmsg().c_str());
...
ErrorType FetcherStore::write_binlog(RuntimeState* state,
    ...
    if (query_cost > FLAGS_print_time_us || retry_times > 0) {
        DB_WARNING("write binlog region_id:%ld log_id:%lu txn_id:%ld cost time:%ld op_type:%s ip:%s",
            region_id, log_id, state->txn_id, query_cost, proto::OpType_Name(op_type).c_str(), info.leader().c_str());
    }
...
int FetcherStore::run_not_set_state(RuntimeState* state,
    ...
    if (region_infos.size() == 0) {
        DB_WARNING("region_infos size == 0, op_type:%s", proto::OpType_Name(op_type).c_str());
        return E_OK;
    }
    ...
    if ((op_type == proto::OP_COMMIT || op_type == proto::OP_ROLLBACK)
        ...
        do {
            ...
            if (error != E_OK) {
                DB_FATAL("send optype:%s to region_id:%ld txn_id:%lu failed, log_id:%lu ", proto::OpType_Name(op_type).c_str(),
    ...
    if (ret != E_OK) {
        DB_FATAL("process binlog op_type:%s txn_id:%lu failed, log_id:%lu ", proto::OpType_Name(op_type).c_str(),
             state->txn_id, log_id);
        return -1;
    }
