...
namespace EA {
    int SingleTxnManagerNode::open(RuntimeState *state) {
        ...
        if (ret < 0) {
            TLOG_WARN("exec begin node fail, log_id: {}, txn_id: {}", log_id, state->txn_id);
            return -1;
        }
        ...
        if (ret < 0) {
            TLOG_WARN("TransactionNote: exec dml_node fail: log_id: {}, txn_id: {}, ",*state,
                             log_id, state->txn_id);
            client_conn->seq_id++;
        ...
        if (ret < 0) {
            TLOG_WARN("TransactionNote: exec prepare fail: log_id: {}, txn_id: {}, ",
                       log_id, state->txn_id);
            client_conn->seq_id++;
            ret = exec_rollback_node(state, rollback_node);
            return -1;
        ...
        if (state->optimize_1pc() == false) {
            ...
            if (ret < 0) {
                TLOG_WARN("TransactionNote: commit failed: log_id: {}, txn_id: {}, ",
                           log_id, state->txn_id);
                ret = exec_rollback_node(state, rollback_node);
                return -1;
            }
        } else {
            TLOG_WARN("TransactionNote: optimize_1pc, no commit: log_id: {}, txn_id: {}, ",
                       log_id, state->txn_id);
        }
