...
namespace EA {
    ...
    int FullExportNode::open(RuntimeState *state) {
        ...
        _scan_node = static_cast<RocksdbScanNode *>(get_node(proto::SCAN_NODE));
        ...
        TLOG_WARN("region_count:{}", _send_region_ids.size());
        return 0;
    }
    ...
    int FullExportNode::get_next_region_infos() {
        ...
        if (index_ptr == nullptr) {
            TLOG_WARN("index info not found index_id:{}", scan_index_info.router_index_id);
            return -1;
        }
    ...
    int FullExportNode::get_next(RuntimeState *state, RowBatch *batch, bool *eos) {
        ...
        if (_send_region_ids.empty()) {
            if (_last_router_key.empty()) {
                state->set_eos();
                TLOG_WARN("process full select done log_id:{}", log_id);
                *eos = true;
                return 0;
            }
        ...
        std::map<int64_t, proto::RegionInfo> infos;
        ...
        TLOG_WARN("send region_id:{} log_id:{}", region_id, log_id);
        ++state->region_count;
        if (infos.empty()) {
            return 0;
        }
        ...
        if (ret < 0) {
            TLOG_ERROR("fetcher_store run fail:{}", ret);
            return -1;
        }
