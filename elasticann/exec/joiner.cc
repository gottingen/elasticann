...
namespace EA {
    int Joiner::init(const proto::PlanNode &node) {
        int ret = 0;
    ...
    bool Joiner::is_slot_ref_equal_condition(ExprNode *left, ExprNode *right) {
        if (left->node_type() != proto::SLOT_REF
            || right->node_type() != proto::SLOT_REF) {
            return false;
        }
    ...
    void Joiner::do_plan_router(RuntimeState *state, std::vector<ExecNode *> &scan_nodes, bool &index_has_null) {
        ...
        for (auto &exec_node: scan_nodes) {
            ...
            if (scan_node->engine() == proto::INFORMATION_SCHEMA) {
                continue;
            }
            ExecNode *parent_node_ptr = scan_node->get_parent();
            FilterNode *filter_node = nullptr;
            if (parent_node_ptr->node_type() == proto::WHERE_FILTER_NODE
                || parent_node_ptr->node_type() == proto::TABLE_FILTER_NODE) {
                filter_node = static_cast<FilterNode *>(parent_node_ptr);
            }
            SortNode *sort_node = nullptr;
            while (parent_node_ptr != nullptr
                   && parent_node_ptr->node_type() != proto::SELECT_MANAGER_NODE
                   && parent_node_ptr->node_type() != proto::JOIN_NODE) {
                if (parent_node_ptr->node_type() == proto::SORT_NODE) {
            ...
            auto get_tuple_desc = [state](int32_t tuple_id) ->
                    proto::TupleDescriptor * { return state->get_tuple_desc(tuple_id); };
    ...
    int Joiner::fetcher_inner_table_data(RuntimeState *state,
        ...
        _loops++;
        TLOG_WARN("fetcher_inner_table_data, loops:{}, outer:{}, inner:{}, time_cost:{}",
    ...
    int Joiner::construct_in_condition(std::vector<ExprNode *> &slot_refs,
        ...
        if (slot_refs.size() == 0) {
            ...
            //增加一个in
            proto::ExprNode *in_node = expr.add_nodes();
            ...
            //增加一个slot_ref
            proto::ExprNode *slot_node = expr.add_nodes();
            slot_node->set_node_type(proto::SLOT_REF);
            ...
            if (ret < 0) {
                //如何释放资源
                TLOG_WARN("create in condition fail");
                return ret;
            }
    ...
    void Joiner::encode_hash_key(MemRow *row,
        ...
        for (auto &slot_ref_expr: slot_ref_exprs) {
            ExprValue value = row->get_value(static_cast<SlotRef *>(slot_ref_expr)->tuple_id(),
                                             static_cast<SlotRef *>(slot_ref_expr)->slot_id());
            key.append_value(value.cast_to(proto::STRING));
        }
    }
    ...
    int Joiner::construct_result_batch(RowBatch *batch,
        ...
        if (outer_mem_row != nullptr) {
            ...
            if (ret < 0) {
                TLOG_WARN("copy from left row fail");
                return -1;
            }
        }
        if (inner_mem_row != nullptr) {
            ...
            if (ret < 0) {
                TLOG_WARN("copy from  row fail");
                return -1;
            }
        }
        switch (_join_type) {
            case proto::INNER_JOIN:
            case proto::SEMI_JOIN: {
            ...
            case proto::RIGHT_JOIN: {
                if (is_satisfy_filter(row.get())) {
                    matched = true;
                    if (_compare_type != proto::CMP_ALL) {
                        batch->move_row(std::move(row));
                    }
                } else if (_use_hash_map && _compare_type != proto::CMP_ALL) {
