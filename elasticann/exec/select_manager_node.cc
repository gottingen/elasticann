...
namespace EA {
    ...
    int SelectManagerNode::open(RuntimeState *state) {
        ...
        if (scan_nodes.size() != 1) {
            TLOG_WARN("select manager has more than one scan node, scan_node_size: {}, txn_id: {}, log_id:{}",
    ...
    int SelectManagerNode::subquery_open(RuntimeState *state) {
        ...
        proto::TupleDescriptor *tuple_desc = state->get_tuple_desc(_derived_tuple_id);
    ...
    int SelectManagerNode::single_fetcher_store_open(FetcherInfo *fetcher, RuntimeState *state, ExecNode *exec_node) {
        ...
        if (router_index_id == main_table_id || scan_index_info->covering_index) {
            ret = fetcher->fetcher_store.run_not_set_state(state, fetcher->scan_index->region_infos, _children[0],
                                                           client_conn->seq_id, client_conn->seq_id, proto::OP_SELECT,
        ...
        if (ret < 0) {
            TLOG_WARN("select manager fetcher manager node open fail, txn_id: {}, log_id:{}, "
                       "router_index_id: {}, main_table_id: {}, covering_index: {}",
    ...
    int SelectManagerNode::fetcher_store_run(RuntimeState *state, ExecNode *exec_node) {
        ...
        int64_t dynamic_timeout_ms = FetcherStore::get_dynamic_timeout_ms(exec_node, proto::OP_SELECT, state->sign);
        int ret = 0;
        ...
        if (backup_scan_index != nullptr && dynamic_timeout_ms > 0 && state->txn_id == 0) {
            ...
            if (main_fetcher.status == FetcherInfo::S_SUCC) {
                ...
                TLOG_WARN("both router index fail, txn_id: {}, log_id:{}, main router index_id: {}"
                           "backup router index_id: {}", state->txn_id, state->log_id(),
            ...
            if (ret < 0) {
                ...
                TLOG_WARN("single_fetcher_store_open fail, txn_id: {}, log_id:{}, router index_id: {}",
    ...
    int SelectManagerNode::open_global_index(FetcherInfo *fetcher, RuntimeState *state, ExecNode *exec_node,
        ...
        AggNode *agg_node = static_cast<AggNode *>(get_node(proto::AGG_NODE));
        SortNode *sort_node = static_cast<SortNode *>(get_node(proto::SORT_NODE));
        FilterNode *where_filter_node = static_cast<FilterNode *>(get_node(proto::WHERE_FILTER_NODE));
        FilterNode *table_filter_node = static_cast<FilterNode *>(get_node(proto::TABLE_FILTER_NODE));
        ...
        /*
        if (_children[0]->node_type() != proto::SCAN_NODE
            && _children[0]->node_type() != proto::WHERE_FILTER_NODE) {
        ...
        auto ret = fetcher->fetcher_store.run_not_set_state(state, fetcher->scan_index->region_infos, store_exec,
                                                            ...
                                                            fetcher->global_backup_type);
        if (ret < 0) {
            TLOG_WARN("select manager fetcher mnager node open fail, txn_id: {}, log_id:{}",
        ...
        while (parent != nullptr && parent->node_type() != proto::LIMIT_NODE) {
        ...
        if (ret < 0) {
            TLOG_WARN("construct primary possible index failed");
            return ret;
        }
    ...
    int SelectManagerNode::construct_primary_possible_index(
        ...
        if (pri_info == nullptr) {
            TLOG_WARN("pri index info not found table_id:{}", main_table_id);
            return -1;
        }
        ...
        // proto::ScanNode* pb_scan_node = scan_node->mutable_pb_node()->mutable_derive_node()->mutable_scan_node();
        ...
        while (!eos) {
            ...
            for (batch->reset(); !batch->is_traverse_over(); batch->next()) {
                ...
                for (auto &pri_field: pri_info->fields) {
                    ...
                    if (slot_id == -1) {
                        TLOG_WARN("field_id:{} tuple_id:{}, slot_id:{}", field_id, tuple_id, slot_id);
                        return -1;
                    }
                ...
                if (record->encode_key(*pri_info.get(), key, pri_info->fields.size(), false, false) != 0) {
                    TLOG_ERROR("Fail to encode_key left, table:{}", pri_info->id);
                    return -1;
                }
