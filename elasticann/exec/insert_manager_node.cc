...
namespace EA {
    int InsertManagerNode::init(const proto::PlanNode &node) {
        int ret = 0;
        ...
        _node_type = proto::INSERT_MANAGER_NODE;
        return 0;
    }

    int InsertManagerNode::init_insert_info(UpdateManagerNode *update_manager_node) {
        _op_type = proto::OP_INSERT;
        _table_id = update_manager_node->table_id();
        _table_info = _factory->get_table_info_ptr(_table_id);
        if (_table_info == nullptr) {
            TLOG_WARN("no table found with table_id: {}", _table_id);
            return -1;
        }
        for (const auto index_id: _table_info->indices) {
            auto info_ptr = _factory->get_index_info_ptr(index_id);
            if (info_ptr == nullptr) {
                TLOG_WARN("no index info found with index_id: {}", index_id);
                return -1;
            }
            if (info_ptr->index_hint_status == proto::IHS_DISABLE && info_ptr->state == proto::IS_DELETE_LOCAL) {
                continue;
            }
            if (info_ptr->type == proto::I_PRIMARY) {
                _pri_info = info_ptr;
            }
            if (info_ptr->type == proto::I_PRIMARY || info_ptr->type == proto::I_UNIQ) {
    ...
    int InsertManagerNode::init_insert_info(InsertNode *insert_node, bool is_local) {
        ...
        for (const auto index_id: _table_info->indices) {
            ...
            if (info_ptr->is_global) {
                if (info_ptr->state == proto::IS_NONE) {
                    TLOG_INFO("index info is NONE, skip.");
                    continue;
                }
                _affected_index_num++;
                if (info_ptr->type == proto::I_UNIQ) {
                    _uniq_index_number++;
                }
            }
        }
    ...
    int InsertManagerNode::subquery_open(RuntimeState *state) {
        ...
        for (auto id: _selected_field_ids) {
            if (table_field_map.count(id) == 0) {
                TLOG_WARN("No field for field id: {}", id);
                return -1;
            }
        ...
        for (size_t i = 0; i < _select_projections.size(); i++) {
            proto::PrimitiveType sub_type = _select_projections[i]->col_type();
            bool is_literal = _select_projections[i]->is_literal();
            proto::ExprNodeType node_type = _select_projections[i]->node_type();
            proto::PrimitiveType insert_type = table_field_map[_selected_field_ids[i]]->type;
            if (_table_info->auto_inc_field_id == table_field_map[_selected_field_ids[i]]->id &&
                sub_type == proto::NULL_TYPE) {
                continue;
            }
            if (!is_compatible_type(sub_type, insert_type, is_literal || (node_type == proto::FUNCTION_CALL))) {
                state->error_code = ER_TRUNCATED_WRONG_VALUE_FOR_FIELD;
                state->error_msg << "Incorrect cloumn type expect " << proto::PrimitiveType_Name(insert_type);
                state->error_msg << " but " << proto::PrimitiveType_Name(sub_type);
                return -1;
            }
        }
        ...
        if (record_template == nullptr) {
            TLOG_WARN("table not found table_id: {}", _table_id);
            return -1;
        }
        bool eos = false;
        do {
            ...
            for (batch.reset(); !batch.is_traverse_over(); batch.next()) {
                ...
                for (size_t i = 0; i < _select_projections.size(); i++) {
                    ...
                    proto::PrimitiveType field_type = table_field_map[_selected_field_ids[i]]->type;
                    if (is_datetime_specic(field_type) && result.is_numberic()) {
                        result.cast_to(proto::STRING).cast_to(field_type);
                    } else {
    ...
    int InsertManagerNode::process_binlog(RuntimeState *state, bool is_local) {
        if (state->open_binlog() && _table_info->is_linked) {
            ...
            proto::PrewriteValue *binlog_value = binlog_ctx->mutable_binlog_value();
            ...
            if (is_local) {
                ...
                if (_need_ignore || _is_replace || _on_dup_key_update) {
                    for (auto &str_record: _fetcher_store.return_str_records) {
                        if (need_set_partition_record) {
                            ...
                            if (ret < 0) {
                                TLOG_ERROR("decode to record fail");
                                return -1;
                            }
                ...
                if (has_delete_record) {
                    mutation->add_sequence(proto::MutationType::UPDATE);
                } else {
                    mutation->add_sequence(proto::MutationType::INSERT);
                }
            } else {
    ...
    int InsertManagerNode::open(RuntimeState *state) {
        ...
        if (_children[0]->node_type() == proto::INSERT_NODE) {
        ...
        //TLOG_WARN("insert pre process time_cost: {}, log_id: {}", pre_insert_cost, state->log_id());
        for (auto expr: _update_exprs) {
            ret = expr->open();
            if (ret < 0) {
                TLOG_WARN("expr open fail, log_id:{} ret:{}", state->log_id(), ret);
                return ret;
            }
        }
        if (_need_ignore) {
    ...
    int InsertManagerNode::basic_insert(RuntimeState *state) {
        ...
        if (ret < 0) {
            TLOG_WARN("exec node failed, log_id:{} ret:{} ", state->log_id(), ret);
            return -1;
        }
        ...
        while (iter != _children.end() && cur < _uniq_index_number) {
            ...
            ret = send_request(state, dml_node, _insert_scan_records, _del_scan_records);
            if (ret < 0) {
                TLOG_WARN("exec node failed, log_id:{} _uniq_index_number:{} cur:{} ret:{} ",
        ...
        if (ret < 0) {
            TLOG_WARN("exec concurrency failed, log_id:{} ret:{} ", state->log_id(), ret);
            return ret;
        }
    ...
    int InsertManagerNode::insert_ignore(RuntimeState *state) {
        ...
        for (auto pair: _index_info_map) {
            ...
            for (auto &record: return_records) {
                ...
                if (ret < 0) {
                    TLOG_WARN("encode key failed, index:{} log_id:{} ret:{}", info->id, state->log_id(), ret);
                    return ret;
                }
    ...
    int InsertManagerNode::insert_replace(RuntimeState *state) {
        ...
        if (ret < 0) {
            TLOG_WARN("reverse main_table failed log_id:{} ret:{}", state->log_id(), ret);
            return -1;
        }
        // 判断冲突行
    ...
    int InsertManagerNode::expr_optimize(QueryContext *ctx) {
        ...
        for (auto expr: _insert_values) {
            ...
            if (!expr->is_constant()) {
                TLOG_WARN("insert expr must be constant");
                return -1;
            }
        }
        return 0;
    ...
    int InsertManagerNode::reverse_main_table(RuntimeState *state) {
        ...
        if (!_primary_record_key_record_map_construct) {
            for (auto record: _store_records[_pri_info->id]) {
                ...
                if (ret < 0) {
                    TLOG_WARN("encode key failed, index:{} log_id:{} ret:{}", _pri_info->id, state->log_id(), ret);
                    return ret;
                }
                std::string key = mt_key.data();
        ...
        for (auto &pair: _store_records) {
            ...
            for (auto &record: pair.second) {
                ...
                ret = record->encode_key(*_pri_info, mt_key, -1, false);
                if (ret < 0) {
                    TLOG_WARN("encode key failed, log_id:{} record:{}", state->log_id(),
                ...
                if (pk_record_iter == _primary_record_key_record_map.end()) {
                    ...
                    if (_on_dup_key_update) {
                        ...
                        ret = record->encode_key(*info, mt_key, -1, false);
                        if (ret < 0) {
                            TLOG_WARN("encode key failed, index:{} log_id:{} ret:{}", index_id, state->log_id(),
                                       ret);
                            return ret;
                        }
                        std::string key = mt_key.data();
                        ...
                        //TLOG_WARN("index_id:{} key:{} {} record:{}", index_id, str_to_hex(key).c_str(), key.size(), pk_record->debug_string().c_str());
                    }
                }
            }
        }
                ...
                for (auto record: _store_records[_pri_info->id]) {
                    for (auto pair: reversed_idx_keys_map) {
                        ...
                        if (pair.second.count(key) > 0) {
                            ...
                            //TLOG_WARN("index_id:{} key:{} {} record:{}", index_id, str_to_hex(key).c_str(), key.size(), record->debug_string().c_str());
                        }
                    }
                }
            }
    ...
    int InsertManagerNode::get_record_from_store(RuntimeState *state) {
        ...
        if (_on_dup_key_update) {
            ...
            for (auto record: _store_records[index_id]) {
                ...
                ret = record->encode_key(*_pri_info, mt_key, -1, false);
                if (ret < 0) {
                    TLOG_WARN("encode key failed, index:{} log_id:{} ret:{}", index_id, state->log_id(), ret);
                    return ret;
                }
                std::string key = mt_key.data();
                //TLOG_WARN("index_id:{} key:{} {} record:{}", index_id, str_to_hex(key).c_str(), key.size(), record->debug_string().c_str());
        ...
        while (node_type == proto::LOCK_SECONDARY_NODE) {
    ...
    void InsertManagerNode::set_err_message(IndexInfo &index_info,
        ...
        if (index_info.type == proto::I_PRIMARY) {
            TLOG_WARN("has dup key index:{} log_id:{} ", index_info.id, state->log_id());
    ...
    int InsertManagerNode::process_records_before_send(RuntimeState *state) {
        ...
        for (auto record: _origin_records) {
            for (const auto pair: _index_info_map) {
                ...
                //TLOG_WARN("index_id:{} key:{} id:{}", index_id, key.c_str(), id);
