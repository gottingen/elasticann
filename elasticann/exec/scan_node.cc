...
#include "elasticann/exec/information_schema_scan_node.h"
...
namespace EA {
    int64_t AccessPathMgr::select_index_common() {
        ...
        for (auto &pair: _paths) {
            ...
            if (!path->is_possible && info.type != proto::I_PRIMARY) {
                continue;
            }
            ...
            if (info.type == proto::I_PRIMARY) {
                index_priority = 300;
            } else if (info.type == proto::I_UNIQ) {
                index_priority = 200;
            } else if (info.type == proto::I_KEY) {
    ...
    int ScanNode::init(const proto::PlanNode &node) {
        int ret = 0;
        ...
        if (!has_index() && _scan_indexs.empty()) {
            ...
            if (pos_index.ranges_size() == 1) {
                ...
                if (field_cnt == (int) index_info.fields.size() &&
                    ...
                    if (index_info.type == proto::I_UNIQ || index_info.type == proto::I_PRIMARY) {
            ...
            if (index_info.type == proto::I_KEY || index_info.type == proto::I_UNIQ) {
    ...
    void AccessPathMgr::show_cost(std::vector<std::map<std::string, std::string>> &path_infos) {
        for (auto &pair: _paths) {
            ...
            if (path->is_possible || path->index_type == proto::I_PRIMARY) {
        ...
        if (outer_path->hit_index_field_ids.size() == inner_path->hit_index_field_ids.size()) {
            ...
            if (outer_path->is_cover_index() == inner_path->is_cover_index() &&
                ...
                if (outer_path->index_other_condition_count >
                    ...
                    if (outer_path->index_type == proto::I_PRIMARY) {
                        outer_path->is_possible = false;
                        return -1;
    ...
    void AccessPathMgr::inner_loop_and_compare(std::map<int64_t, SmartPath>::iterator outer_loop_iter) {
        ...
        while ((++inner_loop_iter) != _paths.end()) {
            ...
            if (inner_loop_iter->second->index_type != proto::I_PRIMARY &&
                inner_loop_iter->second->index_type != proto::I_UNIQ &&
                inner_loop_iter->second->index_type != proto::I_KEY) {
                continue;
            }
...
// 两两比较，根据一些简单规则干掉次优索引
    int64_t AccessPathMgr::pre_process_select_index() {
        ...
        for (auto outer_loop_iter = _paths.begin(); outer_loop_iter != _paths.end(); outer_loop_iter++) {
            ...
            if (outer_loop_iter->second->index_type != proto::I_PRIMARY &&
                outer_loop_iter->second->index_type != proto::I_UNIQ &&
                outer_loop_iter->second->index_type != proto::I_KEY) {
                continue;
            }
    ...
    int64_t ScanNode::select_index_in_baikaldb(const std::string &sample_sql) {
        ...
        if (multi_reverse_index.size() > 0) {
            ...
            if (learner_idx != 0 && learner_idx != path->index_id) {
                ...
                if (get_parent()->node_type() == proto::TABLE_FILTER_NODE ||
                    get_parent()->node_type() == proto::WHERE_FILTER_NODE) {
    ...
    ScanNode *ScanNode::create_scan_node(const proto::PlanNode &node) {
        if (node.derive_node().scan_node().has_engine()) {
            proto::Engine engine = node.derive_node().scan_node().engine();
            switch (engine) {
                case proto::ROCKSDB:
                case proto::BINLOG:
                case proto::ROCKSDB_CSTORE:
                    return new RocksdbScanNode;
                case proto::INFORMATION_SCHEMA:
    ...
    int AccessPathMgr::choose_arrow_pb_reverse_index() {
        if (_multi_reverse_index.size() > 1) {
            ...
            for (auto index_id: _multi_reverse_index) {
                ...
                if (type == proto::ST_PROTOBUF_OR_FORMAT1) {
                    pb_indexs.push_back(index_id);
                    ++pb_type_num;
                } else if (type == proto::ST_ARROW) {
            ...
            auto remove_indexs_func = [this](std::vector<int> &to_remove_indexs) {
                _multi_reverse_index.erase(std::remove_if(_multi_reverse_index.begin(), _multi_reverse_index.end(),
                                                          [&to_remove_indexs](const int &index) {
                                                              return std::find(to_remove_indexs.begin(),
                                                                               to_remove_indexs.end(), index)
                                                                     != to_remove_indexs.end() ? true : false;
                                                          }), _multi_reverse_index.end());
            };

            if (filter_type == proto::ST_PROTOBUF_OR_FORMAT1) {
                remove_indexs_func(pb_indexs);
                _fulltext_use_arrow = true;
            } else if (filter_type == proto::ST_ARROW) {
    ...
    int ScanNode::create_fulltext_index_tree(FulltextInfoNode *node, proto::FulltextIndex *root) {
        if (node == nullptr) {
            return 0;
        }
        switch (node->type) {
            case proto::FNT_TERM : {
                auto &inner_node_pair = std::get<FulltextInfoNode::LeafNodeType>(node->info);
                auto &inner_node = inner_node_pair.second;
                root->set_fulltext_node_type(proto::FNT_TERM);
                ...
                if (range_type == range::MATCH_LANGUAGE) {
                    range->set_match_mode(proto::M_NARUTAL_LANGUAGE);
                } else if (range_type == range::MATCH_BOOLEAN) {
                    range->set_match_mode(proto::M_BOOLEAN);
                }
                break;
            }
            case proto::FNT_AND :
            case proto::FNT_OR : {
                auto &inner_node = std::get<FulltextInfoNode::FulltextChildType>(node->info);
    ...
    int ScanNode::create_fulltext_index_tree() {
        ...
        if (create_fulltext_index_tree(_fulltext_index_tree.root.get(), _fulltext_index_pb.get()) != 0) {
            ...
            if (_main_path.fulltext_use_arrow()) {
                proto::ScanNode *pb_scan_node = mutable_pb_node()->mutable_derive_node()->mutable_scan_node();
