...
namespace EA {
    int DeleteManagerNode::open(RuntimeState *state) {
        ...
        //如果没有全局二级索引，直接走逻辑
        if (child_node->node_type() != proto::SELECT_MANAGER_NODE) {
    ...
    int DeleteManagerNode::init_delete_info(const proto::DeleteNode &delete_node) {
        _op_type = proto::OP_DELETE;
    ...
    int DeleteManagerNode::init_delete_info(const proto::UpdateNode &update_node) {
        _op_type = proto::OP_DELETE;
    ...
    int DeleteManagerNode::open_global_delete(RuntimeState *state) {
        ...
        if (ret < 0) {
            TLOG_WARN("select manager node fail");
            return ret;
        }
    ...
    int DeleteManagerNode::process_binlog(RuntimeState *state, bool is_local) {
        if (state->open_binlog() && _table_info->is_linked) {
            ...
            mutation->add_sequence(proto::MutationType::DELETE);
