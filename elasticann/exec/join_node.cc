...
namespace EA {
    int JoinNode::init(const proto::PlanNode &node) {
        int ret = 0;
        ...
        const proto::JoinNode &join_node = node.derive_node().join_node();
    ...
    int JoinNode::predicate_pushdown(std::vector<ExprNode *> &input_exprs) {
        ...
        if (_join_type == proto::INNER_JOIN) {
        ...
        if (_join_type == proto::LEFT_JOIN || _join_type == proto::RIGHT_JOIN) {
            auto iter = input_exprs.begin();
            while (iter != input_exprs.end()) {
                if (outer_contains_expr(*iter)) {
                    outer_push_exprs.push_back(*iter);
    ...
    void JoinNode::convert_to_inner_join(std::vector<ExprNode *> &input_exprs) {
        ...
        if (_join_type == proto::RIGHT_JOIN) {
        ...
        if (_inner_node->node_type() == proto::JOIN_NODE) {
            ((JoinNode *) _inner_node)->convert_to_inner_join(full_exprs);
        }
        if (_outer_node->node_type() == proto::JOIN_NODE) {
        ...
        for (auto &expr: input_exprs) {
            ...
            if (inner_contains_expr(expr)
                ...
                set_join_type(proto::INNER_JOIN);
                return;
            }
            if (contains_expr(expr) && !expr->contains_null_function()
                && !expr->contains_special_operator(proto::OR_PREDICATE)) {
                set_join_type(proto::INNER_JOIN);
                return;
            }
        }
    ...
    void JoinNode::transfer_pb(int64_t region_id, proto::PlanNode *pb_node) {
    ...
    int JoinNode::hash_join(RuntimeState *state) {
        SortNode *sort_node = static_cast<SortNode *>(_outer_node->get_node(proto::SORT_NODE));
        if (sort_node != nullptr) {
            JoinNode *join_node = static_cast<JoinNode *>(sort_node->get_node(proto::JOIN_NODE));
            if (join_node == nullptr) {
                std::vector<ExecNode *> scan_nodes;
                _outer_node->get_node(proto::SCAN_NODE, scan_nodes);
        ...
        // watt基准循环过滤
        if (_outer_node->get_node(proto::FULL_EXPORT_NODE) != nullptr) {
        ...
        if (ret < 0) {
            TLOG_WARN("ExecNode::inner table open fial");
            return -1;
        }
        if (_join_type == proto::LEFT_JOIN
            || _join_type == proto::RIGHT_JOIN) {
    ...
    int JoinNode::get_next_for_nested_loop_join(RuntimeState *state, RowBatch *batch, bool *eos) {
        ...
        while (1) {
            ...
            while (_inner_iter != _inner_tuple_data.end()) {
                ...
                ++_num_rows_returned;
                if (matched && _join_type == proto::INNER_JOIN) {
                    break;
                } else {
                    ++_inner_iter;
                }
    ...
    int JoinNode::get_next_for_hash_inner_join(RuntimeState *state, RowBatch *batch, bool *eos) {
        ...
        while (1) {
            if (_inner_row_batch.is_traverse_over()) {
                if (_child_eos) {
                    *eos = true;
                    TLOG_WARN("when join, get next complete, child eos, time_cost:{}",
    ...
    int JoinNode::get_next_for_loop_hash_inner_join(RuntimeState *state, RowBatch *batch, bool *eos) {
        ...
        if (_outer_iter == _outer_tuple_data.end()) {
            ...
            FullExportNode *full_export = static_cast<FullExportNode *>(_outer_node->get_node(proto::FULL_EXPORT_NODE));
            if (full_export == nullptr) {
                TLOG_WARN("full_export is null");
                return -1;
            }
            ...
            if (ret < 0) {
                TLOG_WARN("fetcher outer node fail");
                return ret;
            }
            if (_outer_tuple_data.size() == 0) {
                _outer_table_is_null = true;
