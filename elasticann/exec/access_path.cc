...
#include "elasticann/exec/access_path.h"
...
namespace EA {
    ...
    bool AccessPath::need_add_to_learner_paths() {
        ...
        if (table_info_ptr->learner_resource_tags.size() > 0) {
            if (index_info_ptr->index_hint_status == proto::IHS_NORMAL) {
                if (butil::gettimeofday_us() - index_info_ptr->disable_time < _2h &&
                    butil::gettimeofday_us() - index_info_ptr->restore_time < _1h) {
                    // 关闭以后马上打开，可以使用
                    return true;
                ...
                return false;
            } else if (index_info_ptr->index_hint_status == proto::IHS_DISABLE) {
                if (butil::gettimeofday_us() - index_info_ptr->restore_time < _2h &&
                    butil::gettimeofday_us() - index_info_ptr->disable_time < _1h) {
                    // 打开以后马上关闭，不可以使用
                    return false;
    ...
    bool AccessPath::need_select_learner_index() {
        if (butil::gettimeofday_us() - index_info_ptr->restore_time <
            FLAGS_index_use_for_learner_delay_s * 1000 * 1000LL
...
// 对于row_expr，满足全命中索引range后裁剪
    void AccessPath::calc_normal(Property &sort_property) {
        ...
        for (auto &field: index_info_ptr->fields) {
            ...
            switch (range.type) {
                case RANGE: {
                    ...
                    auto range_func = [&range, this, field_cnt, field]() {
                        ...
                        if (range.left.size() == 1) {
                            ...
                            size_t row_idx = 0;
                            while (row_idx < range.left_row_field_ids.size() &&
                                   field_idx < index_info_ptr->fields.size()) {
                        ...
                        if (range.right.size() == 1) {
                            ...
                            size_t row_idx = 0;
                            while (row_idx < range.right_row_field_ids.size() &&
                                   field_idx < index_info_ptr->fields.size()) {
                ...
                case LIKE_PREFIX:
                    // 数字 like不能命中索引
                    if (range.type == LIKE_PREFIX && field.type != proto::STRING) {
                        field_break = true;
                        break;
                    }
                    ++field_cnt;
        ...
        TLOG_DEBUG("is_sort_index:{}, eq_count:{}, sort_property:{}",
    ...
    void AccessPath::calc_index_range() {
        if (index_type == proto::I_FULLTEXT) {
            return;
        }
                ...
                case IN:
                    if (range.is_row_expr && in_row_expr_map.count(*range.conditions.begin()) == 1) {
                        ...
                        if (range.eq_in_values.size() > 0 && in_records.size() % range.eq_in_values.size() == 0) {
                            ...
                            TLOG_ERROR("inx:{} in_records.size() {} != values.size()'s multiples {} ",
        ...
        if (_left_field_cnt == 0 && _right_field_cnt == 0) {
            ...
            for (auto &rg: in_records) {
                ...
                if (_left_field_cnt == index_info_ptr->fields.size()
                    && (index_type == proto::I_PRIMARY || index_type == proto::I_UNIQ)
                    && !_like_prefix) {
    ...
    void AccessPath::calc_fulltext() {
        ...
        switch (range.type) {
            ...
            case EQ:
            case IN:
                if (index_type == proto::I_FULLTEXT && index_info_ptr->segment_type == proto::S_NO_SEGMENT) {
                    hit_index = true;
                    need_cut_index_range_condition.insert(range.conditions.begin(), range.conditions.end());
    ...
    double AccessPath::calc_field_selectivity(int32_t field_id, FieldRange &range) {
        switch (range.type) {
            case RANGE: {
                ...
                //TLOG_WARN("left:{} {} right:{} {}", left.type, left.get_string().c_str(), right.type, right.get_string().c_str());
    ...
    double AccessPath::fields_to_selectivity(const std::unordered_set<int32_t> &field_ids,
        ...
        for (auto &field_id: field_ids) {
            ...
            TLOG_DEBUG("field_id:{} selectivity:{}", field_id, field_sel);
    ...
    void
    AccessPath::calc_cost(std::map<std::string, std::string> *cost_info, std::map<int32_t, double> &filed_selectivity) {
        ...
        if (!is_covering_index && index_type != proto::I_PRIMARY) {
        ...
        TLOG_DEBUG(
                ...
                table_rows, index_read_rows, selectivity, index_other_condition_selectivity,
                other_condition_selectivity, cost);
        if (cost_info != nullptr) {
    ...
    void
    AccessPath::show_cost(std::map<std::string, std::string> *cost_info, std::map<int32_t, double> &filed_selectivity) {
