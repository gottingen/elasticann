...
namespace EA {
    ...
    int AggNode::init(const proto::PlanNode &node) {
        int ret = 0;
        ret = ExecNode::init(node);
        if (ret < 0) {
            TLOG_WARN("ExecNode::init fail, ret:%d", ret);
            return ret;
        }
        _is_merger = node.node_type() == proto::MERGE_AGG_NODE;
        ...
        //TLOG_INFO("_group_exprs: {} {}", node.derive_node().agg_node().group_exprs_size(), node.DebugString().c_str());
        for (auto &expr: node.derive_node().agg_node().agg_funcs()) {
            if (expr.nodes_size() < 1 || expr.nodes(0).node_type() != proto::AGG_EXPR) {
                TLOG_WARN("AggNode::init fail, expr.nodes_size:{}", expr.nodes_size());
                return -1;
            }
    ...
    int AggNode::expr_optimize(QueryContext *ctx) {
        ...
        proto::TupleDescriptor *agg_tuple_desc = ctx->get_tuple_desc(_agg_tuple_id);
    ...
    int AggNode::open(RuntimeState *state) {
        ...
        for (auto expr: _group_exprs) {
            ret = expr->open();
            if (ret < 0) {
                TLOG_WARN("{}, expr open fail, ret:%d", *state, ret);
                return ret;
            }
        }
        for (auto agg: _agg_fn_calls) {
            ret = agg->open();
            if (ret < 0) {
                TLOG_WARN("{}, agg open fail, ret:{}", *state, ret);
                return ret;
            }
        }
        ...
        for (auto child: _children) {
            ...
            do {
                ...
                if (ret < 0) {
                    _iter = _hash_map.begin();
                    TLOG_WARN("{}, child->get_next fail, ret:{}", *state, ret);
                    return ret;
                }
                ...
                if (state->memory_limit_exceeded(_row_cnt, used_size) != 0) {
                    _iter = _hash_map.begin();
                    TLOG_WARN("{}, memory limit exceeded", *state);
                    return -1;
                }
                // 对于用order by分组的特殊优化
        ...
        if (_hash_map.size() == 0 && _group_exprs.size() == 0) {
            ExecNode *packet = get_parent_node(proto::PACKET_NODE);
    ...
    void AggNode::transfer_pb(int64_t region_id, proto::PlanNode *pb_node) {
