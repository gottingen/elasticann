...
namespace EA {
    int SortNode::init(const proto::PlanNode &node) {
        int ret = 0;
        ...
        for (auto &expr: node.derive_node().sort_node().order_exprs()) {
            ExprNode *order_expr = nullptr;
            if (expr.nodes(0).node_type() != proto::SLOT_REF && expr.nodes(0).node_type() != proto::AGG_EXPR) {
                //create slot ref
    ...
    int SortNode::expr_optimize(QueryContext *ctx) {
        ...
        proto::TupleDescriptor *tuple_desc = nullptr;
        if (_tuple_id >= 0) {
        ...
        for (auto expr: _order_exprs) {
            ...
            if (expr->node_type() != proto::SLOT_REF && expr->node_type() != proto::AGG_EXPR) {
                if (tuple_desc == nullptr) {
                    TLOG_WARN("tuple_desc is null, but have !SLOT_REF !AGG_EXPR");
                    return -1;
                }
                if (slot_idx < tuple_desc->slots_size()) {
                    ...
                    TLOG_WARN("slot_idx:{} < slot_size:{}", slot_idx, tuple_desc->slots_size());
                }
            }
            // union中数据都放到tuple:0里了，函数也额外放到tuple:0占了新slot
            // 目前union中的order by不支持函数
            if (_tuple_id == 0 && tuple_desc != nullptr && expr->node_type() == proto::SLOT_REF) {
        ...
        for (auto expr: _slot_order_exprs) {
            if (expr->node_type() == proto::AGG_EXPR) {
                ret = expr->expr_optimize();
                if (ret < 0) {
                    return ret;
                }
    ...
    int SortNode::open(RuntimeState *state) {
        ...
        if (state->sort_use_index()) {
            //TLOG_WARN("{}, sort use index, limit:{}", _limit);
            return 0;
        }
        ...
        //TLOG_WARN("{}, sort_size:{}", *state, count);
        TimeCost sort_time;
        _sorter->sort();
    ...
    int SortNode::get_next(RuntimeState *state, RowBatch *batch, bool *eos) {
        ...
        if (ret < 0) {
            TLOG_WARN("{}, _sorter->get_next fail, ret:{}", *state, ret);
            return ret;
        }
