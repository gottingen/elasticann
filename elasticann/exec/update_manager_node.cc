...
namespace EA {
    int UpdateManagerNode::init(const proto::PlanNode &node) {
        int ret = 0;
    ...
    int UpdateManagerNode::init_update_info(UpdateNode *update_node) {
        ...
        for (auto index_id: _table_info->indices) {
            ...
            if (info.state == proto::IS_NONE) {
                TLOG_INFO("index info is NONE, skip.");
                continue;
            }
            if (info.index_hint_status == proto::IHS_VIRTUAL) {
                TLOG_INFO("index info is virtual, skip.");
                continue;
            }
            if (info.is_global) {
                if (info.type == proto::I_UNIQ) {
        ...
        for (auto index_id: _global_affected_index_ids) {
            auto info_ptr = SchemaFactory::get_instance()->get_index_info_ptr(index_id);
            if (info_ptr == nullptr) {
                TLOG_WARN("index info not found index_id:{}", index_id);
                return -1;
            }
            ...
            //TLOG_WARN("index:{} {}", index_id, _affect_global_index);
        }
    ...
    int UpdateManagerNode::open(RuntimeState *state) {
        int ret = 0;
        if (_children[0]->node_type() == proto::UPDATE_NODE) {
        ...
        if (client_conn == nullptr) {
            TLOG_WARN("connection is nullptr sxn_id:{}", state->txn_id);
            return -1;
        }
        if (state->tuple_descs().size() > 0) {
            _tuple_desc = const_cast<proto::TupleDescriptor *> (&(state->tuple_descs()[0]));
        }
