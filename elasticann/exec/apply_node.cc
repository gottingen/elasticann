...
namespace EA {
    int ApplyNode::init(const proto::PlanNode &node) {
        int ret = 0;
        ...
        const proto::ApplyNode &apply_node = node.derive_node().apply_node();
    ...
    void ApplyNode::extract_eq_inner_slots(ExprNode *expr_node) {
        if (expr_node->node_type() != proto::SLOT_REF) {
            return;
        }
    ...
    void ApplyNode::decorrelate() {
        ...
        ExecNode *agg_node = inner_node->get_specified_node(proto::AGG_NODE);
        ExecNode *merge_agg_node = inner_node->get_specified_node(proto::MERGE_AGG_NODE);
        ...
        while (1) {
            //TLOG_WARN("NODE TYPE:{}", proto::PlanNodeType_Name(inner_node->node_type()).c_str());
            if (inner_node->node_type() == proto::WHERE_FILTER_NODE ||
                inner_node->node_type() == proto::HAVING_FILTER_NODE) {
                        ...
                        if (is_equal) {
                            ...
                            if (merge_agg_node != nullptr && agg_node != nullptr) {
                                for (auto &ref: _inner_eq_slot_refs) {
                                    proto::Expr slot_expr;
                                    proto::ExprNode *node = slot_expr.add_nodes();
            ...
            if (inner_node->node_type() == proto::SORT_NODE
                || inner_node->node_type() == proto::LIMIT_NODE) {
                ExecNode *new_child = inner_node->children(0);
            ...
            if (inner_node->node_type() == proto::MERGE_AGG_NODE) {
                if (_is_select_field) {
            ...
            if (inner_node->node_type() == proto::AGG_NODE
                || inner_node->node_type() == proto::SELECT_MANAGER_NODE
                || inner_node->node_type() == proto::MERGE_AGG_NODE) {
                parent_node = inner_node;
    ...
    void ApplyNode::transfer_pb(int64_t region_id, proto::PlanNode *pb_node) {
    ...
    int ApplyNode::check_unique_key(RuntimeState *state, const std::map<int32_t, std::set<int32_t>> &tuple_field_ids) {
        ...
        for (auto &iter: tuple_field_ids) {
            proto::TupleDescriptor *tuple_desc = state->get_tuple_desc(iter.first);
            int ret = SchemaFactory::get_instance()->is_unique_field_ids(tuple_desc->table_id(), iter.second);
            if (ret < 0) {
                TLOG_WARN("process table info failed desc:{}", tuple_desc->ShortDebugString().c_str());
                return -1;
            } else if (ret > 0) {
                _max_one_row = false;
                _children[1]->reset_limit(-1);
    ...
    int ApplyNode::open(RuntimeState *state) {
        ...
        if (ret < 0) {
            TLOG_WARN("fill equal slot fail");
            return -1;
        }
        ...
        TLOG_WARN("_use_hash_map:{} _max_one_row:{} _conditions_has_agg:{}", _use_hash_map, _max_one_row,
                  _conditions_has_agg);
        if (_use_hash_map && !_max_one_row && !_conditions_has_agg) {
            // inner_node会被多次调用
            if (_parent->get_limit() > 0 && _join_type != proto::LEFT_JOIN && _join_type != proto::ANTI_SEMI_JOIN) {
    ...
    int ApplyNode::nested_loop_apply(RuntimeState *state) {
        ...
        if (ret < 0) {
            TLOG_WARN("ExecNode:: left table open fail");
            return ret;
        }
        ret = fetcher_full_table_data(state, _outer_node, _outer_tuple_data);
        if (ret < 0) {
            TLOG_WARN("ExecNode::join open fail when fetch left table");
            return ret;
        }
        if (_is_explain) {
            ...
            if (ret < 0) {
                TLOG_WARN("ExecNode::inner table open fail");
                return -1;
            }
            return 0;
        }
        ...
        TLOG_WARN("data size:{}", _outer_tuple_data.size());
        ...
        _inner_node->get_node(proto::SCAN_NODE, _scan_nodes);
    ...
    int ApplyNode::hash_apply(RuntimeState *state) {
        ...
        if (ret < 0) {
            TLOG_WARN("ExecNode::create in condition for right table fail");
            return ret;
        }
        //表达式下推，下推的那个节点重新做索引选择，路由选择
        _inner_node->predicate_pushdown(in_exprs);
        if (in_exprs.size() > 0) {
            TLOG_WARN("inner node add filter node");
            _inner_node->add_filter_node(in_exprs);
        }
        std::vector<ExecNode *> scan_nodes;
        _inner_node->get_node(proto::SCAN_NODE, scan_nodes);
        ...
        if (_join_type == proto::LEFT_JOIN || _join_type == proto::ANTI_SEMI_JOIN) {
            ret = fetcher_full_table_data(state, _inner_node, _inner_tuple_data);
            if (ret < 0) {
                TLOG_WARN("fetcher inner node fail");
                return ret;
            }
    ...
    int ApplyNode::fetcher_inner_table_data(RuntimeState *state,
        ...
        //TLOG_WARN("inter row: {} ", outer_tuple_data->debug_string(0).c_str());
    ...
    int ApplyNode::get_next_for_nested_loop_join(RuntimeState *state, RowBatch *batch, bool *eos) {
        ...
        while (1) {
            if (_outer_iter == _outer_tuple_data.end()) {
                TLOG_WARN("when join, outer iter is end, time_cost: {}", get_next_time.get_time());
                *eos = true;
                return 0;
            }
            ...
            while (_inner_iter != _inner_tuple_data.end()) {
                if (reached_limit()) {
                    TLOG_WARN("when join, reach limit size:{}, time_cost:{}",
                ...
                if (ret < 0) {
                    TLOG_WARN("construct result batch fail");
                    return ret;
                }
                ++_num_rows_returned;
                if (matched && _join_type == proto::SEMI_JOIN) {
            ...
            if (!matched) {
                switch (_join_type) {
                    case proto::LEFT_JOIN:
                    case proto::ANTI_SEMI_JOIN: {
        ...
        while (1) {
            ...
            if (inner_mem_rows != nullptr) {
                ...
                for (; _result_row_index < inner_mem_rows->size(); ++_result_row_index) {
                    ...
                    if (batch->is_full()) {
                        //TLOG_WARN("when join, batch is full, time_cost:{}", get_next_time.get_time());
                        return 0;
                    }
                    bool matched = false;
                ...
                if (_has_matched && _join_type == proto::LEFT_JOIN) {
                    if (_compare_type == proto::CMP_ALL && _all_matched) {
                        int ret = construct_null_result_batch(batch, *_outer_iter);
                        if (ret < 0) {
                            TLOG_WARN("construct result batch fail");
                            return ret;
    ...
    int ApplyNode::get_next_via_outer_hash_map(RuntimeState *state, RowBatch *batch, bool *eos) {
        while (1) {
            ...
            if (outer_mem_rows != nullptr) {
                ...
                if (_has_matched && _join_type == proto::SEMI_JOIN) {
                    if (_compare_type == proto::CMP_ALL && _all_matched) {
    ...
    int ApplyNode::get_next_via_loop_outer_hash_map(RuntimeState *state, RowBatch *batch, bool *eos) {
        if (_inner_iter == _inner_tuple_data.end()) {
            ...
            // construct outer
            if (_outer_iter == _outer_tuple_data.end()) {
                TLOG_WARN("when join, outer iter is end, loops:{}", _loops);
                *eos = true;
                return 0;
            }
            std::vector<MemRow *> outer_tuple_data;
        ...
        while (_inner_iter != _inner_tuple_data.end()) {
            ...
            if (outer_mem_rows != nullptr) {
                for (; _result_row_index < outer_mem_rows->size(); ++_result_row_index) {
                    ...
                    bool matched = false;
                    int ret = construct_result_batch(batch, (*outer_mem_rows)[_result_row_index], inner_mem_row,
                                                     matched);
                    if (ret < 0) {
                        TLOG_WARN("construct result batch fail");
                        return ret;
