...
namespace EA {
    ...
    struct FulltextNode {
        ...
        std::variant<FulltextChildType, LeafType> info;
        NodeType type;
    };
    ...
    using FulltextInfoTree = FulltextTree<proto::FulltextNodeType, IndexFieldRange>;
    using FulltextInfoNode = FulltextNode<proto::FulltextNodeType, IndexFieldRange>;
    ...
    public:
        ...
        void add_access_path(const SmartPath &access_path) {
            ...
            if ((access_path->index_type == proto::I_FULLTEXT)
                && access_path->is_possible) {
                _use_fulltext = true;
            }

            if (access_path->index_info_ptr->index_hint_status == proto::IHS_DISABLE
                && access_path->is_possible) {
        ...
        void reset() {
            ...
            for (auto &pair: _paths) {
                ...
                if ((path->index_type == proto::I_FULLTEXT)
                    && path->is_possible) {
                    _use_fulltext = true;
                }

                if (path->index_info_ptr->index_hint_status == proto::IHS_DISABLE
                    && path->is_possible) {
                    _has_disable_index = true;
                }
    ...
    struct ScanIndexInfo {
        ...
        int64_t index_id = 0;
        proto::PossibleIndex *router_index = nullptr;
    ...
    public:
        ...
        ScanNode(proto::Engine engine) : _engine(engine) {
        }

        virtual ~ScanNode() {
        }

        static ScanNode *create_scan_node(const proto::PlanNode &node);
        ...
        const google::protobuf::RepeatedPtrField<proto::RegionInfo> &old_region_infos() {
            return _old_region_infos;
        }

        void set_old_region_infos(google::protobuf::RepeatedPtrField<proto::RegionInfo> &&region_infos) {
        ...
        void
        serialize_index_and_set_router_index(const proto::PossibleIndex &pos_index, proto::PossibleIndex *router_index,
                                             bool covering_index,
        ...
        int create_fulltext_index_tree(FulltextInfoNode *node, proto::FulltextIndex *root);
    ...
    protected:
        proto::Engine _engine = proto::ROCKSDB;
        int32_t _tuple_id = 0;
        int64_t _table_id = -1;
        ...
        proto::LockCmdType _lock = proto::LOCK_NO;
        RouterPolicy _router_policy = RouterPolicy::RP_RANGE;
        google::protobuf::RepeatedPtrField<proto::RegionInfo> _old_region_infos;
        FulltextInfoTree _fulltext_index_tree; //目前只有两层，第一层为and，第二层为or。
        std::unique_ptr<proto::FulltextIndex> _fulltext_index_pb;
