...
namespace EA {
    ...
    void DMLNode::add_delete_conditon_fields() {
        if (_node_type == proto::UPDATE_NODE || _node_type == proto::DELETE_NODE ||
            _node_type == proto::LOCK_PRIMARY_NODE) {
    ...
    int DMLNode::init_schema_info(RuntimeState *state) {
        ...
        if (_all_indexes.empty()) {
            ...
            for (auto index_id: _table_info->indices) {
                ...
                if (!index_info->is_global && index_info->index_hint_status != proto::IHS_VIRTUAL) {
        ...
        if (_node_type == proto::UPDATE_NODE || _on_dup_key_update
            || (_local_index_binlog && (_is_replace || _node_type == proto::DELETE_NODE))) {
        ...
        if (!_update_slots.empty()) {
            ...
            if (!_update_affect_primary && !ttl) {
                // cstore下只更新涉及列
                if (_table_info->engine == proto::ROCKSDB_CSTORE) {
    ...
    int DMLNode::insert_row(RuntimeState *state, SmartRecord record, bool is_update) {
        ...
        if (!is_update && _node_type != proto::LOCK_PRIMARY_NODE) {
        ...
        bool cstore_update_fields_partly = !_update_affect_primary &&
                                           (_ttl_timestamp_us == 0) &&
                                           (is_update || _node_type == proto::LOCK_PRIMARY_NODE);
        ...
        if ((!is_update && _node_type != proto::LOCK_PRIMARY_NODE) || _update_affect_primary) {
        ...
        if (_local_index_binlog &&
            (_is_replace || _need_ignore || _on_dup_key_update || _node_type == proto::UPDATE_NODE)) {
    ...
    int DMLNode::get_lock_row(RuntimeState *state, SmartRecord record, std::string *pk_str, MemRow *row) {
        ...
        *pk_str = pk_key.data();
        ...
        if (row != nullptr && _tuple_desc != nullptr
            && (_node_type == proto::DELETE_NODE || _node_type == proto::UPDATE_NODE)) {
            for (auto slot: _tuple_desc->slots()) {
                auto field = record->get_field_by_tag(slot.field_id());
    ...
    int DMLNode::remove_row(RuntimeState *state, SmartRecord record,
        ...
        for (auto &info_ptr: *_indexes_ptr) {
            ...
            if (info.index_hint_status == proto::IHS_DISABLE
                && info.state == proto::IS_DELETE_LOCAL) {
                continue;
            }
...
// todo : 全局索引update/delete流程不同，重新判断条件待完善
    bool DMLNode::satisfy_condition_again(RuntimeState *state, MemRow *row) {
        ...
        if (_node_type != proto::DELETE_NODE && _node_type != proto::UPDATE_NODE &&
            _node_type != proto::LOCK_PRIMARY_NODE) {
            return true;
        }
