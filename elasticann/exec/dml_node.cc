...
int DMLNode::insert_row(RuntimeState* state, SmartRecord record, bool is_update) {
    ...
    if (_local_index_binlog && (_is_replace || _need_ignore || _on_dup_key_update || _node_type == proto::UPDATE_NODE)) {
...
bool DMLNode::satisfy_condition_again(RuntimeState* state, MemRow* row) {
    ...
    if (_node_type != proto::DELETE_NODE &&  _node_type != proto::UPDATE_NODE && _node_type != proto::LOCK_PRIMARY_NODE) {
        return true;
    }
