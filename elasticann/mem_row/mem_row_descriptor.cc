...
namespace EA {
    ...
    int32_t MemRowDescriptor::init(std::vector<proto::TupleDescriptor> &tuple_desc) {
        ...
        if (nullptr == (_proto = new(std::nothrow)google::protobuf::FileDescriptorProto)) {
            TLOG_WARN("create FileDescriptorProto failed");
            return -1;
        }
        ...
        for (auto &tuple: tuple_desc) {
            ...
            //TLOG_WARN("desc:{}", tuple.ShortDebugString().c_str());
            int slot_cnt = tuple.slots_size();
            for (int idx = 0; idx < slot_cnt; ++idx) {
                const proto::SlotDescriptor &slot = tuple.slots(idx);
                ...
                if (pb_type == -1) {
                    TLOG_WARN("un-supported mysql type: {}", slot.slot_type());
                    return -1;
                }
        ...
        if (!memrow_desc) {
            TLOG_WARN("build memrow_desc failed.");
            return -1;
        }
        for (auto tuple: tuples) {
            ...
            if (!descriptor) {
                TLOG_WARN("FindMessageTypeByName [{}] failed.", tuple);
                return -1;
            }
            const google::protobuf::Message *message = _factory->GetPrototype(descriptor);
            if (!message) {
                TLOG_WARN("create dynamic message failed.");
                return -1;
            }
    ...
    google::protobuf::Message *MemRowDescriptor::new_tuple_message(int32_t tuple_id) {
        ...
        if (iter == _id_tuple_mapping.end()) {
            TLOG_WARN("no tuple found: {}", tuple_id);
            return nullptr;
        }
        if (iter->second == nullptr) {
            TLOG_WARN("message is nullptr: {}", tuple_id);
            return nullptr;
        }
