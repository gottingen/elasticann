...
namespace EA {
    ...
    void BaikalHeartBeat::construct_heart_beat_request(proto::BaikalHeartBeatRequest &request, bool is_backup) {
        ...
        auto schema_read_recallback = [&request, factory](const SchemaMapping &schema) {
            for (auto &info_pair: schema.table_info_mapping) {
                if (info_pair.second->engine != proto::ROCKSDB &&
                    ...
                    info_pair.second->engine != proto::BINLOG) {
                    continue;
                }
                ...
                for (auto &index_id: info_pair.second->indices) {
                    ...
                    //主键索引
                    if (index.type != proto::I_PRIMARY && !index.is_global) {
                        continue;
                    }
        ...
        TaskFactory<proto::RegionDdlWork>::get_instance()->construct_heartbeat(request,
                                                                               &proto::BaikalHeartBeatRequest::add_region_ddl_works);
        TaskFactory<proto::DdlWorkInfo>::get_instance()->construct_heartbeat(request,
                                                                             &proto::BaikalHeartBeatRequest::add_ddl_works);
        ...
        for (auto &it1: index_id_name_map) {
            for (auto &it2: index_id_sample_sqls_map[it1.first]) {
                proto::VirtualIndexInfluence virtual_index_influence;
                ...
                proto::VirtualIndexInfluence *info_affect = request.add_info_affect();
                *info_affect = virtual_index_influence;
            }
        }
    }

    void BaikalHeartBeat::process_heart_beat_response(const proto::BaikalHeartBeatResponse &response, bool is_backup) {
        SchemaFactory *factory = nullptr;
        if (is_backup) {
            factory = SchemaFactory::get_backup_instance();
        } else {
        ...
        TaskFactory<proto::RegionDdlWork>::get_instance()->process_heartbeat(response,
                                                                             static_cast<
                                                                                     const google::protobuf::RepeatedPtrField<proto::RegionDdlWork> &(proto::BaikalHeartBeatResponse::*)() const
                                                                                     >(&proto::BaikalHeartBeatResponse::region_ddl_works));
        TaskFactory<proto::DdlWorkInfo>::get_instance()->process_heartbeat(response,
                                                                           static_cast<
                                                                                   const google::protobuf::RepeatedPtrField<proto::DdlWorkInfo> &(proto::BaikalHeartBeatResponse::*)() const
                                                                                   >(&proto::BaikalHeartBeatResponse::ddl_works));
    }

    void BaikalHeartBeat::process_heart_beat_response_sync(const proto::BaikalHeartBeatResponse &response) {
        TimeCost cost;
