...
namespace EA {
    ...
    void SampleSorter::packet_column(proto::ColumnInfo *column_info) {
        ...
        for (auto &bucket_info: _bucket_infos) {
            proto::BucketInfo *pb_bucket_info = column_info->add_bucket_infos();
            pb_bucket_info->set_distinct_cnt(bucket_info.distinct_cnt);
            pb_bucket_info->set_bucket_size(bucket_info.bucket_size);
            proto::ExprValue *start_pb = pb_bucket_info->mutable_start();
            bucket_info.start.to_proto(start_pb);
            proto::ExprValue *end_pb = pb_bucket_info->mutable_end();
