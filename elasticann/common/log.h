...
#define DB_NOTICE_LONG(_fmt_, args...) \
    do {\
        ::EA::glog_info_writelog_long("[%s:%d][%s][%llu]" _fmt_, \
...
#define DB_DEBUG(_fmt_, args...) \
    do {\
        ...
        ::EA::glog_info_writelog("[%s:%d][%s][%lu]" _fmt_, \
                strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__, bthread_self(), ##args);\
    } while (0);
#else
#define DB_DEBUG(_fmt_, args...) 
...
#define DB_WARNING(_fmt_, args...) \
    do {\
        ::EA::glog_warning_writelog("[%s:%d][%s][%lu]" _fmt_, \
                strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__, bthread_self(), ##args);\
    } while (0);

#define DB_FATAL(_fmt_, args...) \
    do {\
        ::EA::glog_error_writelog("[%s:%d][%s][%lu]" _fmt_, \
                strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__, bthread_self(), ##args);\
    } while (0);
...
#define SQL_TRACE(_fmt_, args...) \
    do {\
        if (!FLAGS_enable_self_trace) break; \
        ::EA::glog_info_writelog_long("[%s:%d][%s][%lu]" _fmt_, \
                strrchr(__FILE__, '/') + 1, __LINE__, __FUNCTION__, bthread_self(), ##args);\
    } while (0);
