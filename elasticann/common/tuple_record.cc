...
namespace EA {
    inline const google::protobuf::FieldDescriptor *get_field(
        ...
        if (mem_row == nullptr) {
            ...
            if (slot == 0) {
                TLOG_ERROR("field:{}, slot is 0, can't be here", field_info->id);
                return nullptr;
            }
...
// 验证pb与fields是否匹配，for online TTL
    int TupleRecord::verification_fields(int32_t max_field_id) {
        ...
        while (_offset < _size) {
            ...
            if (_offset >= _size) {
                TLOG_DEBUG("error: {}, {}", _offset, _size);
                return -1;
            }

            TLOG_DEBUG("field_num: {}, wired_type: {}, max_field_id: {} _offset:{}",
                     field_num, wired_type, max_field_id, _offset);

            if (field_num > max_field_id || field_num == 0) {
                TLOG_DEBUG("error: field_num: {}, wired_type: {}, max_field_id: {}",
            ...
            switch (wired_type) {
                ...
                case 5:
                    ...
                    break;
                default:
                    TLOG_DEBUG("invalid wired_type: {}, offset: {},{}", wired_type, _offset, _size);
                    return -1;
            }
        }
            ...
            if (field_num < static_cast<uint64_t>(iter->first)) {
                ...
                switch (wired_type) {
                    ...
                    case 5:
                        ...
                        break;
                    default:
                        TLOG_ERROR("invalid wired_type: {}, offset: {},{}", wired_type, _offset, _size);
                        return -1;
                }
                ...
                switch (field->type()) {
                    ...
                    default: {
                        TLOG_ERROR("invalid TYPE: {}, field_id:{}, offset: {},{}",
                                 static_cast<int>(field->type()), iter->first, _offset, _size);
                        return -1;
                    }
                }
                if (record == nullptr) {
