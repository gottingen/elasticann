...
// 只在几个自己控制gc和写很少，需要很高性能时候使用这个
    ...
    extern void update_schema_conf_common(const std::string &table_name, const proto::SchemaConf &schema_conf,
                                          proto::SchemaConf *p_conf);

    extern void update_op_version(proto::SchemaConf *p_conf, const std::string &desc);

    extern int primitive_to_proto_type(proto::PrimitiveType type);
    ...
    extern const std::string &rand_peer(proto::RegionInfo &info);

    extern void other_peer_to_leader(proto::RegionInfo &info);

    extern int brpc_with_http(const std::string &host, const std::string &url, std::string &response);

    extern void
    parse_sample_sql(const std::string &sample_sql, std::string &database, std::string &table, std::string &sql);
