...
struct ExprValue {
        ...
        switch (type) {
            ...
            case proto::BITMAP: {
                ...
                if (value.string_val().size() > 0) {
                    try {
                        *_u.bitmap = bluebird::Bitmap::readSafe(value.string_val().data(), value.string_val().size());
                    } catch (...) {
   ...
   uint64_t hash(uint32_t seed = 0x110) const {
    ...
    bool is_string() const {
        return type == proto::STRING || type == proto::HEX || type == proto::BITMAP || type == proto::HLL || type == proto::TDIGEST;
    }
