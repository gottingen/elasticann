...
namespace EA {
    ...
    typedef std::map<ExprValue, std::shared_ptr<BucketInfo>, std::function<bool(const ExprValue &,
                                                                                const ExprValue &)>> HistogramMap;

    // 列直方图，处理单列中直方图桶操作
    class Histogram {
    public:
        Histogram(proto::PrimitiveType type, int32_t field_id, int32_t distinct_cnt, int32_t null_value_cnt) :
        ...
        void add_proto(const proto::ColumnInfo &column_info) {
        ...
        int32_t calc_scalar(const ExprValue &lower, const ExprValue &upper,
            ...
            if (lower.compare(bucket_ptr->start) <= 0 && upper.compare(bucket_ptr->end) >= 0) {
                ...
                ratio = calc_fraction(bucket_ptr->start, bucket_ptr->end, upper) -
                        calc_fraction(bucket_ptr->start, bucket_ptr->end, lower);
                ratio = ratio > 1.0 ? 1.0 : ratio;
                ratio = ratio < distinct_1 ? distinct_1 : ratio;
                count = ceil(bucket_ptr->bucket_size * ratio);
            }
...
// 对从store获取的抽样行，按每列进行排序，生成列直方图
    ...
    public:
        ...
        void packet_column(proto::ColumnInfo *column_info);
...
// 处理从store获取的抽样行
    ...
    public:
        PacketSample(std::vector<std::shared_ptr<RowBatch> > &batch_vector,
                     std::vector<ExprNode *> &slot_order_exprs, proto::TupleDescriptor *tuple_desc) :
        ...
        int packet_sample(proto::Histogram *histogram) {
            int i = 0;
            for (auto expr: _order_exprs) {
                SampleSorter sample_sorter(_batch_vector, expr);
                sample_sorter.sort();
                _batch_vector.clear();
                proto::ColumnInfo *column_info = histogram->add_column_infos();
