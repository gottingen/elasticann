...
namespace EA {
    class Statistics {
    public:
        Statistics(const proto::Statistics &statistics) {
        ...
        void histogram_to_string(std::vector<std::vector<std::string>> &rows, std::vector<ResultField> &fields) {
            if (_field_histogram.size() != fields.size()) {
                TLOG_ERROR("use select * from table_name");
                return;
            }
            int i = 0;
    ...
    private:
        void init_histogram(const proto::Histogram &histogram) {
        ...
        void init_cmsketch(const proto::CMsketch &cmsketch) {
