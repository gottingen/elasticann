...
namespace EA {
    ...
    public:
        ...
        void to_proto(proto::CMsketchColumn *cmsketch_column) {
            cmsketch_column->set_field_id(_field_id);
            for (int i = 0; i < _depth; i++) {
                for (int j = 0; j < _width; j++) {
                    if (_array[i][j] > 0) {
                        proto::CMsketchItem *item = cmsketch_column->add_cmitems();
                        item->set_depth(i);
                        item->set_width(j);
        ...
        void add_proto(const proto::CMsketchColumn &cmsketch_column) {
