...
#include "elasticann/raft/rocksdb_file_system_adaptor.h"
...
namespace EA {
    ...
    ssize_t RocksdbReaderAdaptor::read(butil::IOPortal *portal, off_t offset, size_t size) {
        if (_closed) {
            TLOG_ERROR("rocksdb reader has been closed, region_id: {}, offset: {}",
                     _region_id, offset);
            return -1;
        }
        if (offset < 0) {
            TLOG_ERROR("region_id: {} read error. offset: {}", _region_id, offset);
            return -1;
        }

        if (region_shutdown()) {
            TLOG_ERROR("region_id: {} shutdown, "
                     "last_off:{}, off:{}, ctx->off:{}, size:{}",
        ...
        if (!_is_meta_reader && !_context->need_copy_data) {
            _context->done = true;
            TLOG_WARN("region_id: {} need not copy data, time_cost: {}",
                       _region_id, time_cost.get_time());
            return 0;
        }
        if (offset > _context->offset) {
            TLOG_ERROR("region_id: {}, retry last_offset, offset biger fail "
                     "time_cost: {}, last_off:{}, off:{}, ctx->off:{}, size:{}",
        ...
        if (offset < _context->offset) {
            ...
            if (_last_offset == offset) {
                *portal = _last_package;
                TLOG_ERROR("region_id: {}, retry last_offset time_cost: {}, "
                         "off:{}, ctx->off:{}, size:{}, ret_size:{}",
            ...
            // 重置 _context
            if (offset == 0 &&
                _context->offset_update_time.get_time() > FLAGS_snapshot_timeout_min * 60 * 1000 * 1000ULL) {
                _last_offset = 0;
                _num_lines = 0;
                context_reset();
                TLOG_ERROR("region_id: {}, context_reset", _region_id);
            } else {
                TLOG_ERROR("region_id: {}, retry last_offset fail time_cost: {}, "
                         "last_off:{}, off:{}, ctx->off:{}, size:{}",
                         _region_id, time_cost.get_time(), _last_offset, offset, _context->offset, size);
                return -1;
            }
        ...
        while (count < size) {
            if (!_context->iter->Valid()
                ...
                TLOG_WARN("region_id: {} snapshot read over, total size: {}", _region_id, _context->offset);
                auto region = Store::get_instance()->get_region(_region_id);
                if (region == nullptr) {
                    TLOG_ERROR("region_id: {} is null region", _region_id);
                    return -1;
                }
            ...
            if (_context->is_meta_sst && _context->iter->key().compare(region_info_key) == 0) {
                ...
                TLOG_WARN("region_id: {} meta_sst region_info:{}", _region_id,
            ...
            if (_context->is_meta_sst && _context->iter->key().starts_with(pre_commit_prefix)) {
                TLOG_ERROR("region_id: {} should not have this message, txn_id: {}", _region_id,
            ...
            if (_context->is_meta_sst && _context->iter->key().starts_with(log_index_prefix)) {
                ...
                auto ret = LogEntryReader::get_instance()->read_log_entry(_region_id, log_index,
                                                                          _context->applied_index, txn_id, txn_infos);
                if (ret < 0) {
                    _context->done = true;
                    TLOG_ERROR("read txn info fail, may be has removed, region_id: {}", _region_id);
                    _context->iter->Next();
                    continue;
                }
                for (auto &txn_info: txn_infos) {
                    ++key_num;
                    read_size += serialize_to_iobuf(portal,
                                                    MetaWriter::get_instance()->transcation_pb_key(_region_id, txn_id,
                                                                                                   txn_info.first));
        ...
        TLOG_WARN("region_id: {} read done. count: {}, key_num: {}, time_cost: {}, "
                   "off:{}, size:{}, last_off:{}, last_count:{}",
    ...
    bool RocksdbReaderAdaptor::close() {
        if (_closed) {
            TLOG_WARN("file has been closed, region_id: {}, num_lines: {}, path: {}",
    ...
    ssize_t RocksdbReaderAdaptor::write(const butil::IOBuf &data, off_t offset) {
        (void) data;
        (void) offset;
        TLOG_ERROR("RocksdbReaderAdaptor::write not implemented");
        return -1;
    }
    ...
    int SstWriterAdaptor::open() {
        ...
        if (_region_ptr == nullptr) {
            TLOG_ERROR("open sst file path: {} failed, region_id: {} not exist",
        ...
        if (!s.ok()) {
            TLOG_ERROR("open sst file path: {} failed, err: {}, region_id: {}",
                     path.c_str(), s.ToString().c_str(), _region_id);
            return -1;
        }
        _closed = false;
        TLOG_WARN("rocksdb sst writer open, path: {}, region_id: {}", path.c_str(), _region_id);
        return 0;
    }
    ...
    ssize_t SstWriterAdaptor::write(const butil::IOBuf &data, off_t offset) {
        ...
        if (region_shutdown()) {
            TLOG_ERROR("write sst file path: {} failed, region shutdown, data len: {}, region_id: {}",
                     path.c_str(), data.size(), _region_id);
            return -1;
        }
        if (_closed) {
            TLOG_ERROR("write sst file path: {} failed, file closed: {} data len: {}, region_id: {}",
                     path.c_str(), _closed, data.size(), _region_id);
            return -1;
        }
        if (data.size() == 0) {
            TLOG_WARN("write sst file path: {} data len = 0, region_id: {}", path.c_str(), _region_id);
        }
        auto ret = iobuf_to_sst(data);
        if (ret < 0) {
            TLOG_ERROR("write sst file path: {} failed, received invalid data, data len: {}, region_id: {}",
                     path.c_str(), data.size(), _region_id);
            return -1;
        }
        // 大region addpeer中重置time_cost，防止version=0超时删除
        ...
        if (!_is_meta && _writer->file_size() >= SST_FILE_LENGTH) {
            ...
            TLOG_WARN("rocksdb sst write, region_id: {}, path: {}, offset: {}, "
                       "file_size:{}, total_count: {}, all_size:{}",
                       _region_id, path.c_str(), offset, _writer->file_size(), _count, _data_size);
        }
        //if (_is_meta) {
        TLOG_WARN("rocksdb sst write, region_id: {}, path: {}, offset: {}, data.size: {},"
                   " file_size: {}, total_count: {}", _region_id, path.c_str(), offset, data.size(),
    ...
    bool SstWriterAdaptor::close() {
        if (_closed) {
            TLOG_WARN("file has been closed, path: {}", _path.c_str());
            return true;
        }
        _closed = true;
        if (_is_meta) {
    ...
    bool SstWriterAdaptor::finish_sst() {
        ...
        if (_count > 0) {
            TLOG_WARN("_writer finished, path: {}, region_id: {}, file_size: {}, total_count: {}, all_size:{}",
                       path.c_str(), _region_id, _writer->file_size(), _count, _data_size);
            auto s = _writer->finish();
            if (!s.ok()) {
                TLOG_ERROR("finish sst file path: {} failed, err: {}, region_id: {}",
                         path.c_str(), s.ToString().c_str(), _region_id);
                return false;
            }
        } else {
            bool ret = butil::DeleteFile(butil::FilePath(path), false);
            TLOG_WARN("count is 0, delete path: {}, region_id: {}", path.c_str(), _region_id);
            if (!ret) {
                TLOG_ERROR("delete sst file path: {} failed, region_id: {}",
...
*/
    ...
    int SstWriterAdaptor::iobuf_to_sst(butil::IOBuf data) {
        ...
        while (!data.empty()) {
            ...
            if (nbytes < sizeof(size_t)) {
                TLOG_ERROR("read key size from iobuf fail, region_id: {}", _region_id);
                return -1;
            }
            rocksdb::Slice key;
            ...
            if (key_size <= sizeof(key_buf)) {
                ...
                //TLOG_WARN("region_id: {}, key_size: {} too big", _region_id, key_size);
                key.data_ = static_cast<const char *>(data.fetch(big_key_buf.get(), key_size));
            }
            key.size_ = key_size;
            if (key.data_ == nullptr) {
                TLOG_ERROR("read key from iobuf fail, region_id: {}, key_size: {}",
                         _region_id, key_size);
                return -1;
            }
            ...
            if (nbytes < sizeof(size_t)) {
                TLOG_ERROR("read value size from iobuf fail, region_id: {}", _region_id);
                return -1;
            }
            rocksdb::Slice value;
            ...
            if (value_size <= sizeof(value_buf)) {
                ...
                //TLOG_WARN("region_id: {}, value_size: {} too big", _region_id, value_size);
                value.data_ = static_cast<const char *>(data.fetch(big_value_buf.get(), value_size));
            }
            value.size_ = value_size;
            if (value.data_ == nullptr) {
                TLOG_ERROR("read value from iobuf fail, region_id: {}, value_size: {}",
            ...
            if (_is_meta) {
                if (key == region_info_key) {
                    ...
                    TLOG_WARN("region_id: {} meta_sst recv region_info:{}",
                               _region_id, region_info.ShortDebugString().c_str());
                }
                if (key == applied_index_key) {
                    TLOG_WARN("region_id: {} meta_sst recv applied_index:{}", _region_id,
            ...
            if (!s.ok()) {
                TLOG_ERROR("write sst file failed, err: {}, region_id: {}",
    ...
    ssize_t SstWriterAdaptor::size() {
        TLOG_ERROR("SstWriterAdaptor::size not implemented, region_id: {}", _region_id);
        return -1;
    }
    ...
    ssize_t SstWriterAdaptor::read(butil::IOPortal *portal, off_t offset, size_t size) {
        TLOG_ERROR("SstWriterAdaptor::read not implemented, region_id: {}", _region_id);
        return -1;
    }
    ...
    RocksdbFileSystemAdaptor::~RocksdbFileSystemAdaptor() {
        ...
        TLOG_WARN("region_id: {} RocksdbFileSystemAdaptor released", _region_id);
    }
    ...
    braft::FileAdaptor *RocksdbFileSystemAdaptor::open(const std::string &path, int oflag,
        ...
        if (!is_snapshot_data_file(path) && !is_snapshot_meta_file(path)) {
            ...
            TLOG_WARN("open file: {}, region_id: {}, {}", path.c_str(), _region_id, oflag & O_WRONLY);
            return adaptor;
        }
    ...
    braft::FileAdaptor *RocksdbFileSystemAdaptor::open_writer_adaptor(const std::string &path, int oflag,
        ...
        TLOG_WARN("open for write file, path: {}, region_id: {}", path.c_str(), _region_id);
        return writer;
    }
    ...
    braft::FileAdaptor *RocksdbFileSystemAdaptor::open_reader_adaptor(const std::string &path, int oflag,
        ...
        if (sc == nullptr) {
            TLOG_ERROR("snapshot no found, path: {}, region_id: {}", snapshot_path.c_str(), _region_id);
            if (e != nullptr) {
        ...
        if (is_snapshot_data_file(path)) {
            ...
            if (iter_context == nullptr) {
                ...
                for (auto iter: status.stable_followers) {
                    ...
                    TLOG_WARN("region_id: {} {} {} {}", _region_id, iter.first.to_string().c_str(),
                ...
                TLOG_WARN("region_id: {} open reader, data_index:{},peer_next_index:{}, path: {}, time_cost: {}",
        ...
        if (iter_context->reading) {
            TLOG_WARN("snapshot reader is busy, path: {}, region_id: {}", path.c_str(), _region_id);
            if (e != nullptr) {
        ...
        reader->open();
        TLOG_WARN(
                "region_id: {} open reader: path: {} snapshot_index: {} applied_index: {} data_index: {} , time_cost: {}",
    ...
    bool RocksdbFileSystemAdaptor::open_snapshot(const std::string &path) {
        TLOG_WARN("region_id: {} lock _snapshot_mutex before open snapshot", _region_id);
        ...
        if (iter != _snapshots.end()) {
            ...
            if (iter->second.cost.get_time() > 3600 * 1000 * 1000LL
                ...
                TLOG_WARN("region_id: {} snapshot path: {} is hang over 1 hour, erase", _region_id, path.c_str());
            } else {
                // leaner拉取快照会保留raft_snapshot_reader_expire_time_s
                TLOG_WARN("region_id: {} snapshot path: {} is busy", _region_id, path.c_str());
        ...
        TLOG_WARN("region_id: {} lock_commit_meta_mutex before open snapshot", _region_id);
        ...
        TLOG_WARN("region_id: {}, data_index:{}, binlog_check_point:{}, open snapshot path: {}",
    ...
    void RocksdbFileSystemAdaptor::close_snapshot(const std::string &path) {
        TLOG_WARN("region_id: {} close snapshot path: {}", _region_id, path.c_str());
        BAIDU_SCOPED_LOCK(_snapshot_mutex);
        auto iter = _snapshots.find(path);
        if (iter != _snapshots.end()) {
            _snapshots[path].count--;
            if (_snapshots[path].count == 0) {
                ...
                TLOG_WARN("region_id: {} close snapshot path: {} relase", _region_id, path.c_str());
            }
        }
    }
    ...
    void RocksdbFileSystemAdaptor::close(const std::string &path) {
        ...
        if (iter == _snapshots.end()) {
            TLOG_ERROR("no snapshot found when close reader, path:{}, region_id: {}",
                     path.c_str(), _region_id);
            return;
        }
        auto &snapshot_ctx = iter->second;
        if (is_snapshot_data_file(path) && snapshot_ctx.ptr->data_context != nullptr) {
            TLOG_WARN("read snapshot data file close, path: {}", path.c_str());
            snapshot_ctx.ptr->data_context.reset();
        } else if (snapshot_ctx.ptr->meta_context != nullptr) {
            TLOG_WARN("read snapshot meta data file close, path: {}", path.c_str());
