...
struct MemRegionDdlWork {
    proto::RegionDdlWork region_info;
...
public:
    ...
    bool should_change(int64_t table_id, proto::IndexState status);
...
public:
    ...
    void process_common_task_hearbeat(const std::string& address, const proto::BaikalHeartBeatRequest* request,
        proto::BaikalHeartBeatResponse* response);
    void process_broadcast_task_hearbeat(const std::string& address, const proto::BaikalHeartBeatRequest* request,
        proto::BaikalHeartBeatResponse* response);

    int execute_task(MemRegionDdlWork& work);
    int restore_task(const proto::RegionDdlWork& work);

    int execute_broadcast_task(proto::DdlWorkInfo& work) {
...
public:
    ...
    int init_index_ddlwork(int64_t table_id, const proto::IndexInfo& index_info,
        std::unordered_map<int64_t, std::set<int64_t>>& partition_regions);
    
    int init_column_ddlwork(int64_t table_id, const proto::DdlWorkInfo& work_info,
        std::unordered_map<int64_t, std::set<int64_t>>& partition_regions);

    int init_del_index_ddlwork(int64_t table_id, const proto::IndexInfo& index_info);
    // 定时线程处理所有ddl work。
    int work();
    ...
    //处理单个ddl work
    int add_index_ddlwork(proto::DdlWorkInfo& val);
    int add_column_ddlwork(proto::DdlWorkInfo& ddl_work);
    ...
    int drop_index_ddlwork(proto::DdlWorkInfo& val);
    int load_table_ddl_snapshot(const proto::DdlWorkInfo& val);
    ...
    int delete_index_ddlwork_info(int64_t table_id, const proto::DdlWorkInfo& work_info);
    int update_index_ddlwork_region_info(const proto::RegionDdlWork& work);

    int raft_update_info(const proto::MetaManagerRequest& request,
                            const int64_t apply_index,
                            braft::Closure* done);

    int apply_raft(const proto::MetaManagerRequest& request);

    void delete_ddlwork(const proto::MetaManagerRequest& request, braft::Closure* done);
    void get_index_ddlwork_info(const proto::QueryRequest* request, proto::QueryResponse* response);
    ...
    void get_ddlwork_info(int64_t table_id, proto::QueryResponse* query_response);
...
private:
    ...
    void update_table_ddl_mem(const EA::proto::DdlWorkInfo& info) {
        BAIDU_SCOPED_LOCK(_table_mutex);
    ...
    bool get_ddl_mem(int64_t table_id, EA::proto::DdlWorkInfo& work_info) {
        BAIDU_SCOPED_LOCK(_table_mutex);
        auto it = _table_ddl_mem.find(table_id);
        if (it != _table_ddl_mem.end()) {
    ...
    void set_wait_txn_info(int64_t table_id, proto::DdlWorkInfo& work_info) {
        BAIDU_SCOPED_LOCK(_txn_mutex);
