...
void DBManager::process_common_task_hearbeat(const std::string& address, const proto::BaikalHeartBeatRequest* request,
...
void DBManager::process_broadcast_task_hearbeat(const std::string& address, const proto::BaikalHeartBeatRequest* request,
    proto::BaikalHeartBeatResponse* response) {
    {
    ...
    while (!_shutdown) {
        ...
        std::unordered_map<int64_t, MemDdlInfo> temp_ddl_mem;
        {
            ...
            for (auto iter = _table_ddl_mem.begin(); iter != _table_ddl_mem.end(); iter++) {
                if (iter->second.work_info.errcode() == proto::SUCCESS || iter->second.work_info.errcode() == proto::EXEC_FAIL) {
                    ...
                    if (iter->second.work_info.errcode() == proto::EXEC_FAIL && iter->second.work_info.op_type() == proto::OP_ADD_INDEX) {
