...
namespace EA {
    ...
    struct Instance {
        ...
        Instance(const proto::InstanceInfo &instance_info) :
            ...
            if (instance_info.has_status() && instance_info.status() == proto::FAULTY) {
                instance_status.state = proto::FAULTY;
            } else {
                instance_status.state = proto::NORMAL;
            }
            instance_status.timestamp = butil::gettimeofday_us();
    ...
    public:
        ...
        void process_instance_heartbeat_for_store(const proto::InstanceInfo &request);

        void process_instance_param_heartbeat_for_store(const proto::StoreHeartBeatRequest *request,
                                                        proto::StoreHeartBeatResponse *response);

        void process_instance_param_heartbeat_for_baikal(const proto::BaikalOtherHeartBeatRequest *request,
    ...
    private:
        ...
        // 集群或实例的配置
        std::unordered_map<std::string, proto::InstanceParam> _instance_param_map;
