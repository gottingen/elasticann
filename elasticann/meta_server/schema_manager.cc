...
namespace EA {
    ...
    void SchemaManager::process_schema_info(google::protobuf::RpcController *controller,
        ...
        switch (request->op_type()) {
            ...
            case proto::OP_DROP_NAMESPACE: {
                ...
                // if (request->op_type() == proto::OP_MODIFY_NAMESPACE
            ...
            case proto::OP_UPDATE_CHARSET: {
                ...
                if (request->op_type() == proto::OP_UPDATE_BYTE_SIZE
                ...
                if (request->op_type() == proto::OP_UPDATE_TABLE_COMMENT
                    && !request->table_info().has_comment()) {
                    ERROR_SET_RESPONSE(response, proto::INPUT_PARAM_ERROR,
                                       "no table comment", request->op_type(), log_id);
                    return;
                }
                if ((request->op_type() == proto::OP_RENAME_TABLE || request->op_type() == proto::OP_SWAP_TABLE)
                ...
                if (request->op_type() == proto::OP_UPDATE_DISTS) {
                    auto mutable_request = const_cast<proto::MetaManagerRequest *>(request);
                ...
                if (request->op_type() == proto::OP_UPDATE_MAIN_LOGICAL_ROOM) {
                    auto mutable_request = const_cast<proto::MetaManagerRequest *>(request);
                ...
                if (request->op_type() == proto::OP_ADD_INDEX || request->op_type() == proto::OP_DROP_INDEX
                ...
                if (request->op_type() == proto::OP_UPDATE_TTL_DURATION
    ...
    void SchemaManager::process_peer_heartbeat_for_store(const proto::StoreHeartBeatRequest *request,
                                                         proto::StoreHeartBeatResponse *response,
                                                         uint64_t log_id) {
    ...
    void SchemaManager::process_leader_heartbeat_for_store(const proto::StoreHeartBeatRequest *request,
                                                           proto::StoreHeartBeatResponse *response,
                                                           uint64_t log_id) {
        if (!_meta_state_machine->is_leader()) {
            response->set_errcode(proto::NOT_LEADER);
    ...
    void SchemaManager::process_baikal_heartbeat(const proto::BaikalHeartBeatRequest *request,
            ...
            for (const auto &table_info: request->heartbeat_tables()) {
                if (need_binlog_heartbeat && table_info.table_name() == "*") {
                    ...
                    const std::string &full_table_name =
                            table_info.namespace_name() + "\001" + table_info.database() + "\001" +
    ...
    int SchemaManager::check_and_get_for_privilege(proto::UserPrivilege &user_privilege) {
    ...
    int SchemaManager::pre_process_for_create_table(const proto::MetaManagerRequest *request,
                                                    proto::MetaManagerResponse *response,
                                                    uint64_t log_id) {
        ...
        proto::DataBaseInfo database_info;
...
#undef SET_REQUEST_TABLE_INFO
    ...
    int SchemaManager::pre_process_for_merge_region(const proto::MetaManagerRequest *request,
                                                    proto::MetaManagerResponse *response,
                                                    uint64_t log_id) {
        TimeCost time_cost;
        ...
        proto::RegionMergeResponse *region_merge = response->mutable_merge_response();
    ...
    int SchemaManager::pre_process_for_split_region(const proto::MetaManagerRequest *request,
                                                    proto::MetaManagerResponse *response,
                                                    uint64_t log_id) {
    ...
    int SchemaManager::whether_dists_legal(proto::MetaManagerRequest *request,
                                           proto::MetaManagerResponse *response,
        ...
        for (auto &dist: request->table_info().dists()) {
            ...
            if (ClusterManager::get_instance()->logical_and_physical_room_valid(dist.logical_room(),
                                                                                dist.physical_room())) {
                total_count += dist.count();
                continue;
            }
    ...
    int SchemaManager::whether_main_logical_room_legal(proto::MetaManagerRequest *request,
                                                       proto::MetaManagerResponse *response,
                                                       uint64_t log_id) {
        if (!request->table_info().has_main_logical_room()) {
            return 0;
