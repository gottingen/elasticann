...
void QueryTableManager::get_flatten_schema(const proto::QueryRequest* request,
            proto::QueryResponse* response) {
    TableManager* manager = TableManager::get_instance();
    ...
    *(response->add_schema_infos()) = schema_pb;
    for (auto& field_info : schema_pb.fields()) {
        proto::QuerySchema field_schema;
        ...
        field_schema.set_column_type(proto::PrimitiveType_Name(field_info.mysql_type()));
    ...
    for (auto& index_info : schema_pb.indexs()) {
        ...
        for (auto& field_name : index_info.field_names()) {
            ...
            extra += field_name + " " + proto::PrimitiveType_Name(type) + ", ";
            ++i;
        }
        extra[extra.size() - 2] = ']';
...
void QueryTableManager::construct_query_table(const TableMem& table_info, 
             proto::QueryTable* flatten_table_info) {
    TableManager* manager = TableManager::get_instance();
...
void QueryTableManager::get_flatten_table(const proto::QueryRequest* request,
        proto::QueryResponse* response) {
    std::string input_table_name = request->table_name();
    boost::trim(input_table_name);
    std::string input_database = request->database();
    ...
    std::map<std::string, proto::QueryTable> table_infos;
...
void QueryTableManager::process_console_heartbeat(const proto::ConsoleHeartBeatRequest* request,
            proto::ConsoleHeartBeatResponse* response, uint64_t log_id) {
    TimeCost step_time_cost; 
    std::unordered_map<int64_t, int64_t> report_table_map;
    std::unordered_map<int64_t, proto::RegionHeartBeat> region_version_map;
    ...
    TableManager* manager = TableManager::get_instance();
    std::unordered_map<int64_t, std::tuple<proto::SchemaInfo, int64_t, int64_t>> table_schema_map;
    {
        BAIDU_SCOPED_LOCK(manager->_table_mutex);
...
proto::PrimitiveType QueryTableManager::get_field_type(
                    int64_t table_id, 
                    int32_t field_id, 
...
void QueryTableManager::get_primary_key_string(int64_t table_id, std::string& primary_key_string) {
    ...
    for (auto& index_info : table_info.indexs()) {
        ...
        for (int idx = 0; idx < index_info.field_names_size(); ++idx) {
            ...
            proto::PrimitiveType field_type = get_field_type(table_id, index_info.field_ids(idx), table_info);
            primary_key_string += " " + proto::PrimitiveType_Name(field_type) + ",";
        }
    }
...
void QueryTableManager::get_virtual_index_influence_info(const proto::QueryRequest* request, proto::QueryResponse* response){
    ...
    for (auto& it1 : virtual_index_sql_map) {
        for (auto& it2 : it1.second) {
            proto::VirtualInfoAndSqls virtual_index_info_sqls;
            ...
            proto::VirtualInfoAndSqls* virtual_index_influence_info = response->add_virtual_index_influence_info();
            *virtual_index_influence_info = virtual_index_info_sqls;
        }
    }
}

void QueryTableManager::get_table_in_fast_importer(const proto::QueryRequest* request, proto::QueryResponse* response){
    TableManager* manager = TableManager::get_instance();
    ...
    for (auto& tb : tables_ts) {
        ...
        proto::QueryTable* table_info = response->add_flatten_tables();
