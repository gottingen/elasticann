...
void ClusterManager::process_cluster_info(google::protobuf::RpcController* controller, 
    ...
    case proto::OP_DROP_PHYSICAL:{
        if (!request->has_physical_rooms()) {
            ERROR_SET_RESPONSE(response, proto::INPUT_PARAM_ERROR, "no physical room", request->op_type(), log_id);
            return;
        }
        _meta_state_machine->process(controller, request, response, done_guard.release());
        return;
    ...
    case proto::OP_UPDATE_INSTANCE: {
        if (!request->has_instance()) {
            ERROR_SET_RESPONSE(response, proto::INPUT_PARAM_ERROR, "no instance info", request->op_type(), log_id);
            return;
        }
        _meta_state_machine->process(controller, request, response, done_guard.release());
        return;
    ...
    case proto::OP_UPDATE_INSTANCE_PARAM: {
        if (request->instance_params().size() < 1) {
            ERROR_SET_RESPONSE(response, proto::INPUT_PARAM_ERROR, "no instance params", request->op_type(), log_id);
            return;
        }
        _meta_state_machine->process(controller, request, response, done_guard.release());
        return;
    ...
    case proto::OP_MOVE_PHYSICAL: {
        if (!request->has_move_physical_request()) {
            ERROR_SET_RESPONSE(response, proto::INPUT_PARAM_ERROR, "no move physical request", request->op_type(), log_id);
            return;
        }
        _meta_state_machine->process(controller, request, response, done_guard.release());
        return;
...
// 如果已存在则修改，不存在则新添加
inline void agg_instance_param(const proto::InstanceParam& old_param, const proto::InstanceParam& new_param, proto::InstanceParam* out_param) {
