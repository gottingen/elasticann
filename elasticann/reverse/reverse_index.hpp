...
namespace EA {
    ...
    int ReverseIndex<Schema>::reverse_merge_func(proto::RegionInfo info, bool need_remove_third) {
    ...
    int ReverseIndex<Schema>::insert_reverse(
            ...
            const std::string &pk,
            SmartRecord record) {
        return handle_reverse(txn, proto::REVERSE_NODE_NORMAL, word, pk, record);
    }
    ...
    int ReverseIndex<Schema>::delete_reverse(
            ...
            const std::string &pk,
            SmartRecord record) {
        return handle_reverse(txn, proto::REVERSE_NODE_DELETE, word, pk, record);
    }
    ...
    int ReverseIndex<Schema>::_reverse_merge_to_second_level(
        ...
        FirstLevelMSIterator<ReverseNode, ReverseList> first_iter(iterator, prefix,
                                                                  _key_range, merge_term, true, _rocksdb,
                                                                  txn->get_txn());
    ...
    int MutilReverseIndex<Schema>::search(
            ...
            const std::vector<proto::MatchMode> &modes,
    ...
    int MutilReverseIndex<Schema>::search(
            ...
            bool is_fast, const proto::FulltextIndex &fulltext_index_info) {
    ...
    int MutilReverseIndex<Schema>::init_operator_executor(
            const proto::FulltextIndex &fulltext_index_info, OperatorBooleanExecutor<Schema> *&exe) {

        if (fulltext_index_info.fulltext_node_type() == proto::FNT_AND) {
            ...
            for (const auto &child: fulltext_index_info.nested_fulltext_indexes()) {
                if (child.fulltext_node_type() == proto::FNT_AND || child.fulltext_node_type() == proto::FNT_OR) {
        ...
        } else if (fulltext_index_info.fulltext_node_type() == proto::FNT_OR) {
    ...
    int MutilReverseIndex<Schema>::init_term_executor(
            const proto::FulltextIndex &fulltext_index_info, BooleanExecutor<Schema> *&exe) {
