...
//--common interface
    ...
    int NewSchema<Node, List>::create_executor(const std::string &search_data,
        ...
        //TLOG_WARN("zero create_exe search_data[{}]", search_data.c_str());
        // 先用规则支持 or 操作
        // TODO 用bison来支持mysql bool查询

        if (mode == proto::M_NARUTAL_LANGUAGE) {
            or_search.push_back(search_data);
        } else if (mode == proto::M_BOOLEAN) {
        ...
        for (auto &or_item: or_search) {
            ...
            if (ret < 0) {
                TLOG_WARN("[word:{}]segment error {}", or_item.c_str(), ret);
                return -1;
            }
            if (term_map.size() == 0) {
                continue;
        ...
        TLOG_DEBUG("query_words : {}", _query_words.c_str());
