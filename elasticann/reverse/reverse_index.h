...
public:
    ...
    virtual int reverse_merge_func(proto::RegionInfo info, bool need_remove_third) = 0;
    //新加正排 创建倒排索引
...
public:
    ...
    virtual bool valid() {
        if (_exe != nullptr) {
            while (true) {
                ...
                if (_cur_node) {
                    if (_cur_node->flag() == proto::REVERSE_NODE_NORMAL) {
                        return true;
                    } else {
                        continue;
                    }
    ...
    virtual int create_executor(const std::string& search_data, 
            proto::MatchMode mode, proto::SegmentType segment_type, const proto::Charset& charset) = 0;
...
public:
    ...
    ReverseIndex(
            ...
            proto::SegmentType segment_type = proto::S_DEFAULT,
    ...
    virtual int reverse_merge_func(proto::RegionInfo info, bool need_remove_third);
    //0:success    -1:fail
    virtual int insert_reverse(
                        SmartTransaction& txn,
                        const std::string& word, 
...
private:
    ...
    proto::SegmentType _segment_type;
    bool _is_over_cache;
    bool _is_seg_cache;
...
public:
    ...
    int init_operator_executor(const proto::FulltextIndex& fulltext_index_info, OperatorBooleanExecutor<Schema>*& exe);
