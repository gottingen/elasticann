            ...
            if (transaction->txn_id() != 0) {
                if (status().error_code() != EPERM) {
                    ...
                    if (op_type != proto::OP_COMMIT && op_type != proto::OP_ROLLBACK) {
                            ...
                            region_id, log_id, transaction->txn_id(), seq_id, proto::OpType_Name(op_type).c_str());
                    }
                } else {
                    DB_WARNING("leader changed region_id: %ld log_id:%lu txn_id: %lu:%d, op_type: %s",
                            region_id, log_id, transaction->txn_id(), transaction->seq_id(), proto::OpType_Name(op_type).c_str());
                }
            } else {
                // 1pc状态机外执行失败rollback
