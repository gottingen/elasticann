...
void Region::binlog_query_primary_region(const int64_t& start_ts, const int64_t& txn_id, proto::RegionInfo& region_info, int64_t rollback_ts) {
...
void Region::binlog_fill_exprvalue(const proto::BinlogDesc& binlog_desc, proto::OpType op_type, std::map<std::string, ExprValue>& field_value_map) {
...
int BinlogReadMgr::binlog_add_to_response(int64_t commit_ts, const std::string& binlog_value, proto::StoreRes* response) {
...
int BinlogReadMgr::get_binlog_value(int64_t commit_ts, int64_t start_ts, proto::StoreRes* response, int64_t binlog_row_cnt) {
    int ret = 0;
    if (commit_ts == start_ts) {
        _fake_binlog_cnt++;
    }
...
bool Region::flash_back_need_read(const proto::StoreReq* request, const std::map<std::string, ExprValue>& field_value_map) {
