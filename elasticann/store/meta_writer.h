...
namespace EA {
    ...
    public:
        ...
        int init_meta_info(const proto::RegionInfo &region_info);

        int update_region_info(const proto::RegionInfo &region_info);
        ...
        int parse_region_infos(std::vector<proto::RegionInfo> &region_infos);
        ...
        int read_region_info(int64_t region_id, proto::RegionInfo &region_info);
        ...
        [[nodiscard]] std::string binlog_oldest_ts_key(int64_t region_id) const;
    ...
    public:
        [[nodiscard]] std::string region_info_key(int64_t region_id) const;

        //[[nodiscard]] std::string region_for_store_key(int64_t region_id) const;

        [[nodiscard]] std::string applied_index_key(int64_t region_id) const;

        [[nodiscard]] std::string num_table_lines_key(int64_t region_id) const;

        [[nodiscard]] std::string transcation_log_index_key(int64_t region_id, uint64_t txn_id) const;

        [[nodiscard]] std::string log_index_key_prefix(int64_t region_id) const;

        [[nodiscard]] std::string transcation_pb_key(int64_t region_id, uint64_t txn_id, int64_t log_index) const;

        [[nodiscard]] std::string transcation_pb_key_prefix(int64_t region_id) const;

        [[nodiscard]] std::string pre_commit_key_prefix(int64_t region_id) const;

        [[nodiscard]] std::string pre_commit_key(int64_t region_id, uint64_t txn_id) const;

        [[nodiscard]] std::string doing_snapshot_key(int64_t region_id) const;

        [[nodiscard]] std::string learner_key(int64_t region_id) const;

        [[nodiscard]] std::string encode_applied_index(int64_t applied_index, int64_t data_index) const;

        [[nodiscard]] std::string encode_num_table_lines(int64_t line) const;

        [[nodiscard]] std::string encode_region_info(const proto::RegionInfo &region_info) const;

        [[nodiscard]] std::string encode_learner_flag(int64_t ts) const;

        [[nodiscard]] std::string encode_transcation_pb_value(const proto::StoreReq &txn) const;

        [[nodiscard]] std::string encode_transcation_log_index_value(int64_t log_index) const;

        [[nodiscard]] std::string encode_removed_ddl_key(int64_t region_id, int64_t index_id) const;

        [[nodiscard]] std::string rollbacked_transcation_key(int64_t region_id, uint64_t txn_id) const;
        ...
        [[nodiscard]] std::string region_ddl_info_key(int64_t region_id) const;

        int update_region_ddl_info(const proto::StoreRegionDdlInfo &region_ddl_info);

        int read_region_ddl_info(int64_t region_id, proto::StoreRegionDdlInfo &region_ddl_info);

        std::string meta_info_prefix(int64_t region_id);

        [[nodiscard]] std::string encode_binlog_ts(int64_t ts) const;

        [[nodiscard]] std::string binlog_check_point_key(int64_t region_id) const;
