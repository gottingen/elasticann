...
namespace EA {
    ...
    int RegionControl::remove_data(int64_t drop_region_id) {
        ...
        if (data_cf == nullptr) {
            TLOG_WARN("get rocksdb data column family failed, region_id: {}", drop_region_id);
            return -1;
        }
        TimeCost cost;
        ...
        TLOG_WARN("region clear data, remove_range cost:{}, region_id: {}", cost.get_time(), drop_region_id);
        return 0;
    }
    ...
    void RegionControl::compact_data(int64_t region_id) {
        ...
        auto data_cf = rocksdb->get_data_handle();
        if (data_cf == nullptr) {
            TLOG_WARN("get rocksdb data column family failed, region_id: {}", region_id);
            return;
        }
        TimeCost cost;
        ...
        if (!res.ok()) {
            TLOG_WARN("region_id:{}, compact_range error: code={}, msg={}",
                       region_id, res.code(), res.ToString().c_str());
        }
        TLOG_WARN("region_id: {}, compact_range cost:{}", region_id, cost.get_time());
    }
    ...
    void RegionControl::compact_data_in_queue(int64_t region_id) {
        ...
        if (in_compact_regions.count(region_id) == 1) {
            TLOG_WARN("region_id: {} have be put in queue before", region_id);
            return;
        }
    ...
    int RegionControl::remove_log_entry(int64_t drop_region_id) {
        ...
        TLOG_WARN("remove raft log entry, region_id: {}, cost: {}", drop_region_id, cost.get_time());
        ...
        if (iter->Valid()) {
            ...
            TLOG_WARN("remove raft log entry, region_id: {}, cost:{}, log_index:{}, type:{}, {}",
...
//todo 测一下如果目录为空，删除返回值时啥
    int RegionControl::remove_snapshot_path(int64_t drop_region_id) {
        ...
        try {
            ...
            turbo::filesystem::path stable_path(stable_path_str);
            turbo::filesystem::remove_all(snapshot_path);
            turbo::filesystem::remove_all(stable_path);
        } catch (turbo::filesystem::filesystem_error &e) {
            TLOG_ERROR("remove error:{}, region_id: {}", e.what(), drop_region_id);
            return -1;
        }
        TLOG_WARN("drop snapshot directory, region_id: {}", drop_region_id);
        return 0;
    }

    int RegionControl::clear_all_infos_for_region(int64_t drop_region_id) {
        TLOG_WARN("region_id: {}, clear_all_infos_for_region do compact in queue", drop_region_id);
    ...
    int RegionControl::ingest_data_sst(const std::string &data_sst_file, int64_t region_id, bool move_files) {
        ...
        if (!res.ok()) {
            ...
            if (!FLAGS_allow_blocking_flush) {
                ...
                if (!res.ok()) {
                    TLOG_WARN("flush data to rocksdb fail, err_msg:{}", res.ToString().c_str());
                    return -1;
                }
    ...
    int RegionControl::sync_do_snapshot() {
        TLOG_WARN("region_id: {} sync_do_snapshot start", _region_id);
        ...
        if (ret < 0) {
            TLOG_WARN("send no op fail, region_id:{}", _region_id);
            return -1;
        }
        ...
        if (!_region->is_learner()) {
            ...
            if (_region->_learner != nullptr) {
                ...
                TLOG_ERROR("region_id {} learner is nullptr.", _region_id);
            }
        }
        sync_sign.wait();
        TLOG_WARN("region_id: {} sync_do_snapshot success", _region_id);
        return 0;
    }
    ...
    void RegionControl::raft_control(google::protobuf::RpcController *controller,
        ...
        if (!_region->is_leader() && !request->force()) {
            TLOG_WARN("node is not leader when add_peer, region_id: {}", _region_id);
            if (response != nullptr) {
        ...
        if (make_region_status_doing() == 0) {
            TLOG_WARN("region status to doing becase of raft control, region_id: {}, request:{}",
                       _region_id, pb2json(*request).c_str());
            common_raft_control(controller, request, response, done_guard.release(), &_region->_node);
        } else {
            TLOG_ERROR("region status is not idle, region_id: {}, lod_id:{}", _region_id, log_id);
            response->set_errcode(proto::REGION_ERROR_STATUS);
            response->set_errmsg("status not idle");
        }
    }

    void RegionControl::add_peer(const proto::AddPeer &add_peer, SmartRegion region, ExecutionQueue &queue) {
        ...
        if (make_region_status_doing() != 0) {
            TLOG_WARN("make region status doing fail, region_id: {}", _region_id);
            return;
        }
        TimeCost cost;
        TLOG_WARN("region status to doing becase of add peer of heartbeat response, "
                   "region_id: {} new_instance: {}", _region_id, new_instance.c_str());
        if (legal_for_add_peer(add_peer, nullptr) != 0) {
            reset_region_status();
            return;
        }
        auto init_and_add_peer = [add_peer, new_instance, region, cost]() {
            ...
            if (region->_shutdown) {
                TLOG_WARN("region_id:{} has REMOVE", region->get_region_id());
                control.reset_region_status();
                return;
            }
            TLOG_WARN("start init_region, region_id: {}, wait_time:{}",
                       region->get_region_id(), cost.get_time());
            if (cost.get_time() > FLAGS_store_heart_beat_interval_us * 5) {
                TLOG_WARN("region_id: {} add peer timeout", region->get_region_id());
                control.reset_region_status();
                return;
            }
    ...
    void RegionControl::add_peer(const proto::AddPeer *request,
        ...
        TLOG_WARN("region status to doing because of add peer of request, region_id: {}",
                   _region_id);
        if (legal_for_add_peer(*request, response) != 0) {
            reset_region_status();
            return;
    ...
    int RegionControl::transfer_leader(const proto::TransLeaderRequest &trans_leader_request,
        ...
        auto transfer_call = [this, region, new_leader]() {
            ...
            if (ret != 0) {
                TLOG_WARN("get_peer_applied_index fail,region_id: {}", region->get_region_id());
                return;
            }
            if ((region->_applied_index - peer_applied_index) * peer_dml_latency
                > (FLAGS_transfer_leader_catchup_time_threshold)) {
                TLOG_WARN("peer applied index: {} is less than applied index: {}, peer_dml_latency: {}",
            ...
            if (make_region_status_doing() != 0) {
                TLOG_ERROR("region status is not idle when transfer leader, region_id: {}", _region_id);
                return;
            } else {
                TLOG_WARN("region status to doning becase of transfer leader of heartbeat response,"
                           " region_id: {}", _region_id);
            }
            ret = region->transfer_leader_to(new_leader);
            reset_region_status();
            if (ret != 0) {
                TLOG_WARN("node:{} {} transfer leader fail",
    ...
    int RegionControl::init_region_to_store(const std::string instance_address,
        ...
        if (response.errcode() == proto::CONNECT_FAIL) {
            TLOG_ERROR("add peer fail when connect new instance, region_id: {}, instance:{}",
        ...
        if (response.errcode() == proto::EXEC_FAIL) {
            TLOG_WARN("add peer fail when init region,"
                       " region_id: {}, instance:{}, cntl.Failed",
        ...
        if (response.errcode() != proto::SUCCESS) {
            TLOG_WARN("add peer fail when init region,"
                       " region_id: {}, instance:{}, errcode:{}",
                       region_id, instance_address.c_str(),
                       proto::ErrCode_Name(response.errcode()).c_str());
            return -1;
        }
        TLOG_WARN("send init region to store:{} success, region_id: {}, cost: {}",
                   instance_address.c_str(), _region_id, cost.get_time());
        return 0;
    }


    void RegionControl::construct_init_region_request(proto::InitRegion &init_request) {
        proto::RegionInfo *region_info = init_request.mutable_region_info();
    ...
    int RegionControl::legal_for_add_peer(const proto::AddPeer &add_peer, proto::StoreRes *response) {
        TLOG_WARN("start legal_for_add_peer, region_id: {}", _region_id);
        ...
        if (_region->_shutdown) {
            TLOG_WARN("node is shutdown when add_peer, region_id: {}", _region_id);
            if (response != nullptr) {
                response->set_errcode(proto::NOT_LEADER);
                response->set_errmsg("not leader");
            }
        ...
        if (!_region->_region_info.can_add_peer() || (!add_peer.is_split() && _region->_region_info.version() < 1)) {
            TLOG_WARN("region_id: {} can not add peer, can_add_peer:{}, region_version:{}",
        ...
        if (!_region->_node.list_peers(&peers).ok()) {
            TLOG_WARN("node list peer fail when add_peer, region_id: {}", _region_id);
            if (response != nullptr) {
        ...
        if ((int) peers.size() != add_peer.old_peers_size()) {
            TLOG_WARN("peer size is not equal when add peer, region_id: {}", _region_id);
            if (response != nullptr) {
                response->set_errcode(proto::PEER_NOT_EQUAL);
        ...
        for (auto &old_peer: add_peer.old_peers()) {
            ...
            if (iter == str_peers.end()) {
                TLOG_ERROR("old_peer not equal to list peers, region_id: {}, old_peer:{}",
    ...
    void RegionControl::node_add_peer(const proto::AddPeer &add_peer,
                                      const std::string &new_instance,
                                      proto::StoreRes *response,
                                      google::protobuf::Closure *done) {
        TLOG_WARN("_leader_add_peer, region_id: {} ,new_instance: {}",
                   _region_id, new_instance.c_str());
        //send add_peer to self
        Concurrency::get_instance()->add_peer_concurrency.increase_wait();
        TLOG_WARN("_leader_add_peer, region_id: {} ,new_instance: {} wait sucess",
                   _region_id, new_instance.c_str());
        ...
        TLOG_WARN("_leader_add_peer, region_id: {} ,new_instance: {} send sucess",
                   _region_id, new_instance.c_str());
    }
