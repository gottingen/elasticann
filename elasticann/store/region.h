...
namespace EA {
    ...
    public:
        ...
        int select_sample(RuntimeState &state, ExecNode *root, const proto::AnalyzeInfo &analyze_info,
                          proto::StoreRes &response);
        ...
        void validate_and_add_version(const proto::StoreReq &request, braft::Closure *done, int64_t applied_index,
                                      int64_t term);

        void add_version_for_split_region(const proto::StoreReq &request, braft::Closure *done, int64_t applied_index,
                                          int64_t term);
    ...
    private:
        ...
        void read_binlog(const proto::StoreReq *request, proto::StoreRes *response, const std::string &remote_side,
                         uint64_t log_id);
        ...
        bool
        flash_back_need_read(const proto::StoreReq *request, const std::map<std::string, ExprValue> &field_value_map);

        std::string
        binlog_get_str_val(const std::string &name, const std::map<std::string, ExprValue> &field_value_map);
        ...
        void binlog_query_primary_region(const int64_t &start_ts, const int64_t &txn_id, proto::RegionInfo &region_info,
                                         int64_t rollback_ts);

        void binlog_fill_exprvalue(const proto::BinlogDesc &binlog_desc, proto::OpType op_type,
