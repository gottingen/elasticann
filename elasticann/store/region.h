...
#include "elasticann/runtime/runtime_state_pool.h"
#include "rapidjson/document.h"
#include "elasticann/raft/rocksdb_file_system_adaptor.h"
#include "elasticann/store/region_control.h"
...
struct MultiSplitRegion {
    ...
    std::string end_key;
    MultiSplitRegion(const proto::MultiSplitRegion& region_info) {
...
struct FollowerReadCond {
    BthreadCond cond;
    proto::ErrCode errcode;
    bool is_learner = false;
    FollowerReadCond(bool is_learner) : errcode(proto::SUCCESS), is_learner(is_learner) {};
    void set_failed() {
        if (is_learner) {
            errcode = proto::LEARNER_NOT_READY;
        } else {
            errcode = proto::NOT_LEADER;
        }
...
enum GetMode {
    ...
    ~BinlogReadMgr() { }
    int get_binlog_value(int64_t commit_ts, int64_t start_ts, proto::StoreRes* response, int64_t binlog_row_cnt);
    int fill_fake_binlog(int64_t fake_ts, std::string& binlog);
    int get_binlog_finish(proto::StoreRes* response);
    int binlog_add_to_response(int64_t commit_ts, const std::string& binlog_value, proto::StoreRes* response);
...
public:
    ...
    Region(RocksWrapper* rocksdb, 
            ...
            const proto::RegionInfo& region_info,
            int64_t region_id,
            bool is_learner = false) :
                _rocksdb(rocksdb),
                _factory(factory),
    ...
    void dml(const proto::StoreReq& request,
            proto::StoreRes& response,
            int64_t applied_index, 
            int64_t term, bool need_txn_limit);

    void dml_2pc(const proto::StoreReq& request,
    ...
    void dml_1pc(const proto::StoreReq& request,
            proto::OpType op_type,
            const proto::Plan& plan,
            const RepeatedPtrField<proto::TupleDescriptor>& tuples,
            proto::StoreRes& response,
    ...
    int select(const proto::StoreReq& request, proto::StoreRes& response);
    int select(const proto::StoreReq& request,
            const proto::Plan& plan,
            const RepeatedPtrField<proto::TupleDescriptor>& tuples,
            proto::StoreRes& response);
    int select_normal(RuntimeState& state, ExecNode* root, proto::StoreRes& response);
    int select_sample(RuntimeState& state, ExecNode* root, const proto::AnalyzeInfo& analyze_info, proto::StoreRes& response);
    void do_apply(int64_t term, int64_t index, const proto::StoreReq& request, braft::Closure* done);
    ...
    void construct_heart_beat_request(proto::StoreHeartBeatRequest& request, bool need_peer_balance);

    void construct_peers_status(proto::LeaderHeartBeat* leader_heart);
    ...
    void validate_and_add_version(const proto::StoreReq& request, braft::Closure* done, int64_t applied_index, int64_t term);
    void add_version_for_split_region(const proto::StoreReq& request, braft::Closure* done, int64_t applied_index, int64_t term);
    void apply_txn_request(const proto::StoreReq& request, braft::Closure* done, int64_t index, int64_t term);
    void adjustkey_and_add_version(const proto::StoreReq& request,
                                           braft::Closure* done, 
                                           int64_t applied_index, 
                                           int64_t term);
    void exec_update_primary_timestamp(const proto::StoreReq& request,
    ...
    void adjustkey_and_add_version_query(google::protobuf::RpcController* controller,
            ...
            google::protobuf::Closure* done);
    //开始做merge操作
    void start_process_merge(const proto::RegionMergeResponse& merge_response);
    //开始做split操作
    //第一步通过raft状态机,创建迭代器，取出当前的index,自此之后的log不能再删除
    void start_process_split(const proto::RegionSplitResponse& split_response,
            bool tail_split,
    ...
    void exec_kv_out_txn(const proto::StoreReq* request,
            proto::StoreRes* response,
            const char* remote_side,
            google::protobuf::Closure* done);
    
    int execute_cached_cmd(const proto::StoreReq& request, proto::StoreRes& response,
            uint64_t txn_id, 
    ...
    //}
    void copy_region(proto::RegionInfo* region_info) {
        std::lock_guard<std::mutex> lock(_region_lock);
    ...
    proto::PeerStatus region_status() const {
    ...
    bool is_dml_op_type(const proto::OpType& op_type) {
    ...
    bool is_2pc_op_type(const proto::OpType& op_type) {
    ...
    bool is_async_apply_op_type(const proto::OpType& op_type) {
        if (is_dml_op_type(op_type)
            || is_2pc_op_type(op_type)
            || op_type == proto::OP_NONE
            || op_type == proto::OP_UPDATE_PRIMARY_TIMESTAMP
            || op_type == proto::OP_KILL) {
                return true;
            }
        return false;
    }
    void check_peer_latency();
    void get_read_index(proto::StoreRes* response);
...
private:
    struct SplitParam {
        ...
        std::unordered_map<uint64_t, proto::TransactionInfo> applied_txn;

        // 尾分裂多region
        std::vector<MultiSplitRegion> multi_new_regions;

        int64_t sub_num_table_lines = 0;
        std::vector<proto::TransactionInfo> adjust_txns;
    ...
    struct BinlogParam {
        ...
        //binlog function
    void recover_binlog();
    void read_binlog(const proto::StoreReq* request, proto::StoreRes* response, const std::string& remote_side, uint64_t log_id);
    void query_binlog_ts(const proto::StoreReq* request, proto::StoreRes* response);
    void apply_binlog(const proto::StoreReq& request, braft::Closure* done);
    ...
    bool flash_back_need_read(const proto::StoreReq* request, const std::map<std::string, ExprValue>& field_value_map);
    ...
    int get_primary_region_info(int64_t primary_region_id, proto::RegionInfo& region_info);
    
    void binlog_query_primary_region(const int64_t& start_ts, const int64_t& txn_id, proto::RegionInfo& region_info, int64_t rollback_ts);
    void binlog_fill_exprvalue(const proto::BinlogDesc& binlog_desc, proto::OpType op_type, std::map<std::string, ExprValue>& field_value_map);
    //binlog end
    void apply_kv_in_txn(const proto::StoreReq& request, braft::Closure* done,
                         int64_t index, int64_t term);

    void apply_kv_out_txn(const proto::StoreReq& request, braft::Closure* done,
                                  int64_t index, int64_t term);
    bool validate_version(const proto::StoreReq* request, proto::StoreRes* response);
    void print_log_entry(const int64_t start_index, const int64_t end_index);
    void set_region(const proto::RegionInfo& region_info) {
        std::lock_guard<std::mutex> lock(_region_lock);
        _region_info.CopyFrom(region_info);
        _version = _region_info.version();
    }
    void set_region_with_update_range(const proto::RegionInfo& region_info) {
        std::lock_guard<std::mutex> lock(_region_lock);
        _region_info.CopyFrom(region_info);
        _version = _region_info.version();
    ...
    bool check_key_exist(SmartTransaction txn, const proto::KvOp& kv_op) {
...
private:
    ...
    proto::RegionInfo      _merge_region_info;
    // 倒排索引需要
    ...
    proto::PeerStatus  _region_status = proto::STATUS_NORMAL;
    ...
    struct StreamingResult {
        bthread::Mutex  mutex;
        std::unordered_map<brpc::StreamId, proto::StreamState> state;
