...
namespace EA {
    ...
    int Store::init_before_listen(std::vector<std::int64_t> &init_region_ids) {
        ...
        for (auto txn: recovered_txns) {
            ...
            std::vector<std::string> split_vec = turbo::StrSplit(txn_name, '_', turbo::SkipEmpty());
        ...
        proto::StoreHeartBeatRequest request;
    ...
    void Store::init_region(google::protobuf::RpcController *controller,
        ...
        const proto::RegionInfo &region_info = request->region_info();
    ...
    void Store::region_raft_control(google::protobuf::RpcController *controller,
        ...
        if (request->op_type() == proto::SetPeer && request->force() && region->removed()) {
        ...
        region->raft_control(controller,
                             ...
                             response,
                             done_guard.release());
        if (request->op_type() == proto::TransLeader && response->errcode() == proto::SUCCESS) {
    ...
    void Store::health_check(google::protobuf::RpcController *controller,
        ...
        int64_t qps = RocksdbVars::get_instance()->rocksdb_get_time_cost_qps.get_value(bvar::FLAGS_bvar_dump_interval) +
                      RocksdbVars::get_instance()->rocksdb_seek_time_cost_qps.get_value(
                              bvar::FLAGS_bvar_dump_interval) +
                      RocksdbVars::get_instance()->rocksdb_scan_time_cost_qps.get_value(
                              bvar::FLAGS_bvar_dump_interval) / FLAGS_get_token_weight;
        int64_t fetch_token_qps = RocksdbVars::get_instance()->qos_fetch_tokens_qps.get_value(
                bvar::FLAGS_bvar_dump_interval);
        ...
        if (match_reject_condition ||
            ...
            response->set_errcode(proto::STORE_BUSY);
        }
        ...
        if (FLAGS_store_rocks_hang_check) {
            if (last_rocks_hang_check_ok.get_time() > 30 * 1000 * 1000LL
                ...
                response->set_errcode(proto::STORE_ROCKS_HANG);
            }
        }
    }
    ...
    void Store::restore_region(google::protobuf::RpcController *controller,
                               const EA::proto::RegionIds *request,
                               proto::StoreRes *response,
                               google::protobuf::Closure *done) {
        brpc::ClosureGuard done_guard(done);
        response->set_errcode(proto::SUCCESS);
    ...
    void Store::get_applied_index(google::protobuf::RpcController *controller,
                                  const EA::proto::GetAppliedIndex *request,
                                  proto::StoreRes *response,
                                  google::protobuf::Closure *done) {
        brpc::ClosureGuard done_guard(done);
        response->set_errcode(proto::SUCCESS);
...
//store上报心跳到meta_server
    ...
    void Store::flush_memtable_thread() {
        ...
        while (!_shutdown) {
            ...
            _rocksdb->get_db()->GetIntProperty(_rocksdb->get_data_handle(), "rocksdb.oldest-snapshot-time",
                                               &snapshot_time);
            if (snapshot_time > 0) {
                rocksdb_snapshot_difftime.set_value((int64_t) time(nullptr) - (int64_t) snapshot_time);
            }
    ...
    void Store::process_merge_request(int64_t table_id, int64_t region_id) {
        ...
        request.set_op_type(proto::OP_MERGE_REGION);
        proto::RegionMergeRequest *region_merge = request.mutable_region_merge();
    ...
    void
    Store::process_split_request(int64_t table_id, int64_t region_id, bool tail_split, const std::string &split_key,
                                 int64_t key_term) {
        ++_split_num;
        //构造请求
        proto::MetaManagerRequest request;
        proto::MetaManagerResponse response;
        request.set_op_type(proto::OP_SPLIT_REGION);
        proto::RegionSplitRequest *region_split = request.mutable_region_split();
    ...
    void Store::start_db_statistics() {
        ...
        while (!_shutdown) {
            ...
            std::string str = (db_options.statistics.get())->ToString();
    ...
    void
    Store::update_schema_info(const proto::SchemaInfo &table, std::map<int64_t, std::set<int64_t>> *reverse_index_map) {
        ...
        for (size_t idx = 0; idx < table.indexs_size(); ++idx) {
            const proto::IndexInfo &index_info = table.indexs(idx);
            if (index_info.index_type() == proto::I_FULLTEXT && index_info.state() != proto::IS_PUBLIC) {
    ...
    void Store::construct_heart_beat_request(proto::StoreHeartBeatRequest &request) {
        static int64_t count = 0;
        ...
        //构造instance信息
        proto::InstanceInfo *instance_info = request.mutable_instance_info();
        ...
        for (auto table_info: table_id_version_map) {
            proto::SchemaHeartBeat *schema = request.add_schema_infos();
    ...
    void Store::process_heart_beat_response(const proto::StoreHeartBeatResponse &response) {
        {
