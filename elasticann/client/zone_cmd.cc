...
namespace EA::client {
    ...
    turbo::Table show_meta_query_zone_response(const EA::proto::QueryResponse &res) {
        turbo::Table result;
        auto &dbs = res.zone_infos();
        result.add_row(turbo::Table::Row_t{"zone size", turbo::Format(dbs.size())});
        auto last = result.size() - 1;
        result[last].format().font_color(turbo::Color::green);
        turbo::Table sumary;
        sumary.add_row({"namespace", "zone", "id", "version", "quota", "replica number", "resource tag",
                                "region split lines"});
        for (auto &ns: dbs) {
            sumary.add_row(
                    turbo::Table::Row_t{ns.namespace_name(), ns.zone(), turbo::Format(ns.zone_id()),
