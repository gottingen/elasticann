...
namespace EA {
    ...
    int LoadPlanner::plan() {
        ...
        proto::LoadNode *load_node = derive->mutable_load_node();
    ...
    int LoadPlanner::parse_load_info(proto::LoadNode *node, proto::InsertNode *insert_node) {
        std::string database;
        std::string table;
        std::string alias;
        ...
        if (ret < 0) {
            TLOG_ERROR("path:{} not exists", tmp_path.c_str());
            _ctx->stat_info.error_code = ER_NO_DB_ERROR;
            _ctx->stat_info.error_msg << "file " << tmp_path << " not exists";
            return -1;
        }
        if (!S_ISREG(stat_buf.st_mode)) {
            TLOG_ERROR("path:{} not a file", tmp_path.c_str());
            _ctx->stat_info.error_code = ER_NO_DB_ERROR;
    ...
    int LoadPlanner::parse_field_list(proto::LoadNode *node, proto::InsertNode *insert_node) {
        ...
        for (int i = 0; i < _load_stmt->columns.size(); ++i) {
            ...
            if (alias_name.empty()) {
                TLOG_WARN("get_field_alias_name failed: {}", _load_stmt->columns[i]->to_string().c_str());
                return -1;
            }
    ...
    int LoadPlanner::parse_set_list(proto::LoadNode *node) {
        for (int i = 0; i < _load_stmt->set_list.size(); ++i) {
            if (_load_stmt->set_list[i]->name == nullptr) {
                TLOG_WARN("set_list name[{}] is enmty", i);
                return -1;
            }
            ...
            if (0 != create_expr_tree(_load_stmt->set_list[i]->expr, set_expr, CreateExprOptions())) {
                TLOG_WARN("create set value expr failed");
                return -1;
            }
