...
namespace EA {
    ...
    struct ScanTupleInfo {
        ...
        int64_t table_id = -1;
        int32_t slot_cnt = 1;
        std::unordered_map<int32_t, proto::SlotDescriptor> field_slot_mapping;
    ...
    struct CreateExprOptions {
        ...
        int row_expr_size = 1;
        proto::CompareType compare_type = proto::CMP_NULL;
    };
    ...
    public:
        ...
        LogicalPlanner(QueryContext *ctx, const SmartPlanTableCtx &plan_state) : _ctx(ctx),
        ...
        static std::map<parser::JoinType, proto::JoinType> join_type_mapping;

        std::vector<std::string> &select_names() {
            return _select_names;
        }

        std::vector<proto::Expr> &select_exprs() {
    ...
    protected:
        ...
        int flatten_filter(const parser::ExprNode *item, std::vector<proto::Expr> &filters,
                           const CreateExprOptions &options);

        void create_order_func_slot();

        // @agg format: agg_func(col_name) / count_star()
        std::vector<proto::SlotDescriptor> &get_agg_func_slot(
                const std::string &agg, const std::string &fn_name, bool &new_slot);

        int create_agg_expr(const parser::FuncExpr *expr_item, proto::Expr &expr, const CreateExprOptions &options);

        // (col between A and B) ==> (col >= A) and (col <= B)
        int create_between_expr(const parser::FuncExpr *item, proto::Expr &expr, const CreateExprOptions &options);

        int create_values_expr(const parser::FuncExpr *item, proto::Expr &expr);

        // TODO in next stage: fill full func name
        // fill arg_types, return_type(col_type) and has_var_args
        int create_scala_func_expr(const parser::FuncExpr *item, proto::Expr &expr, parser::FuncType op,
                                   const CreateExprOptions &options);

        int create_expr_tree(const parser::Node *item, proto::Expr &expr, const CreateExprOptions &options);
        ...
        //TODO: ColumnType len
        int
        create_term_slot_ref_node(const parser::ColumnName *term, proto::Expr &expr, const CreateExprOptions &options);
        ...
        proto::SlotDescriptor &get_scan_ref_slot(const std::string &alias_name,
                                                 int64_t table, int32_t field, proto::PrimitiveType type);

        proto::SlotDescriptor &get_values_ref_slot(int64_t table, int32_t field, proto::PrimitiveType type);

        // create common plan nodes
        int create_packet_node(proto::OpType op_type);

        int create_filter_node(std::vector<proto::Expr> &filters, proto::PlanNodeType type);
    ...
    private:
        ...
        int create_common_subquery_expr(const parser::SubqueryExpr *item, proto::Expr &expr,
        ...
        int handle_compare_subquery(const parser::ExprNode *item, proto::Expr &expr,
                                    const CreateExprOptions &options);

        int handle_exists_subquery(const parser::ExprNode *item, proto::Expr &expr,
                                   const CreateExprOptions &options);

        void construct_literal_expr(const ExprValue &value, proto::ExprNode *node);

        int construct_in_predicate_node(const parser::FuncExpr *func_item, proto::Expr &expr, proto::ExprNode **node);
    ...
    protected:
        ...
        std::vector<proto::TupleDescriptor> _scan_tuples;
        proto::TupleDescriptor _values_tuple;  // INSERT ... ON DUPLICATE KEY UPDATE values() tuple
        ScanTupleInfo _values_tuple_info;

        int32_t _agg_tuple_id = -1;
        int32_t _agg_slot_cnt = 1;
        std::vector<proto::Expr> _agg_funcs;
        std::vector<proto::Expr> _distinct_agg_funcs;
        std::vector<proto::Expr> _orderby_agg_exprs;
        std::unordered_map<std::string, std::vector<proto::SlotDescriptor>> _agg_slot_mapping;
        ...
        std::vector<proto::Expr> _select_exprs;
        std::vector<std::string> _select_names;

        int32_t _order_tuple_id = -1;
        int32_t _order_slot_cnt = 1;
        std::vector<proto::SlotDescriptor> _order_slots;
        std::vector<proto::Expr> _order_exprs;
