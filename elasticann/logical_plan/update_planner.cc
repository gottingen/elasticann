...
namespace EA {
    ...
    int UpdatePlanner::plan() {
        ...
        create_packet_node(proto::OP_UPDATE);
        proto::PlanNode *update_node = _ctx->add_plan_node();
    ...
    int UpdatePlanner::create_update_node(proto::PlanNode *update_node) {
        if (_plan_table_ctx->table_tuple_mapping.size() != 1) {
            TLOG_WARN("no database name, specify database by USE cmd");
        }
        ...
        update_node->set_node_type(proto::UPDATE_NODE);
        update_node->set_limit(_limit_count);
        update_node->set_is_explain(_ctx->is_explain);
        update_node->set_num_children(1); //TODO 
        proto::DerivePlanNode *derive = update_node->mutable_derive_node();
        proto::UpdateNode *update = derive->mutable_update_node();
    ...
    int UpdatePlanner::parse_kv_list() {
        ...
        if (table_info_ptr == nullptr) {
            TLOG_WARN("table:{} is nullptr", table_id);
            return -1;
        }
        ...
        for (int idx = 0; idx < set_list.size(); ++idx) {
            ...
            if (table_info.partition_ptr != nullptr &&
                table_info.partition_ptr->partition_field_id() == field_info->id) {
    ...
    int UpdatePlanner::parse_where() {
        if (_update->where == nullptr) {
            TLOG_WARN("update sql [{}] does not contain where conjunct", _ctx->sql.c_str());
    ...
    int UpdatePlanner::parse_orderby() {
        if (_update->order != nullptr) {
            TLOG_WARN("update doesnot support orderby");
            return -1;
        }
        return 0;
    }
