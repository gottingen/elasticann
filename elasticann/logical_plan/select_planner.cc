...
namespace EA {
    ...
    int SelectPlanner::plan() {
        ...
        if (_select->table_refs == nullptr) {
            ...
            if (_agg_funcs.empty() && _distinct_agg_funcs.empty() && _group_exprs.empty()) {
                create_packet_node(proto::OP_SELECT);
        ...
        if (0 != create_filter_node(_having_filters, proto::HAVING_FILTER_NODE)) {
            return -1;
        }
        // create_agg_node
        if (0 != create_agg_node()) {
        ...
        //     TLOG_WARN("plan_node: {}", _ctx->plan.nodes(idx).DebugString().c_str());
        // }
        return 0;
    }
            ...
            for (auto &iter_out: outer_ref_map) {
                ...
                for (auto &field_slot: iter_out.second) {
                    ...
                    if (iter == inter_column_map.end()) {
                        TLOG_WARN("field not found:{}", field_slot.first.c_str());
                        continue;
                    }
                    int32_t inter_column_id = iter->second;
                    _ctx->slot_column_mapping[iter_out.first][outer_slot_id] = inter_column_id;
                    //TLOG_WARN("tuple_id:{} outer_slot_id:{} inter_column_id:{}", iter_out.first, outer_slot_id, inter_column_id);
                }
            }
        }
    }
    ...
    int SelectPlanner::subquery_rewrite() {
        ...
        proto::Expr expr = _select_exprs[0];
        proto::Expr agg_expr;
        ...
        std::vector<proto::SlotDescriptor> slots;
        // > >=
    ...
    int SelectPlanner::create_agg_node() {
        if (_select->select_opt != nullptr && _select->select_opt->distinct == true) {
            ...
            if (!_agg_funcs.empty() || !_distinct_agg_funcs.empty() || !_group_exprs.empty()) {
                TLOG_WARN("distinct query doesnot support group by");
                return -1;
            }
...
//            expr->add_nodes()->CopyFrom(_select_exprs[idx].nodes(0));
        ...
        if (!_distinct_agg_funcs.empty() || !_orderby_agg_exprs.empty()) {
            agg_node->set_node_type(proto::MERGE_AGG_NODE);
        }
        agg_node->set_limit(-1);
        agg_node->set_is_explain(_ctx->is_explain);
        agg_node->set_num_children(1); //TODO 
        ...
        for (uint32_t idx = 0; idx < _agg_funcs.size(); ++idx) {
            proto::Expr *expr = agg->add_agg_funcs();
        ...
        if (!_distinct_agg_funcs.empty() || !_orderby_agg_exprs.empty()) {
            proto::PlanNode *agg_node2 = _ctx->add_plan_node();
            agg_node2->set_node_type(proto::AGG_NODE);
            agg_node2->set_limit(-1);
            agg_node2->set_is_explain(_ctx->is_explain);
            agg_node2->set_num_children(1); //TODO 
            proto::DerivePlanNode *derive = agg_node2->mutable_derive_node();
            proto::AggNode *agg2 = derive->mutable_agg_node();

            for (uint32_t idx = 0; idx < _group_exprs.size(); ++idx) {
                proto::Expr *expr = agg2->add_group_exprs();
                expr->CopyFrom(_group_exprs[idx]);
            }
            ...
            for (uint32_t idx = 0; idx < _agg_funcs.size(); ++idx) {
                if (_agg_funcs[idx].nodes(0).fn().name() == "group_concat" &&
                    ...
                    proto::Expr *expr = agg2->add_agg_funcs();
    ...
    void SelectPlanner::add_single_table_columns(const std::string &table_name, TableInfo *table_info) {
        for (auto &field: table_info->fields) {
            ...
            proto::SlotDescriptor slot = get_scan_ref_slot(table_name, table_info->id, field.id, field.type);
...
// TODO: select * from multiple tables or join clause
        ...
        if (wild_card->db_name.empty() && wild_card->table_name.empty()) {
            ...
            for (auto &table_name: _table_names) {
                ...
                if (table_info == nullptr) {
                    ...
                    TLOG_WARN("no table found for select field: {}", field->to_string().c_str());
                    return -1;
                }
            ...
            if (!wild_card->db_name.empty()) {
                ...
                if (dbs.size() == 0) {
                    ...
                    TLOG_WARN("no database found for field: {}", table_name.c_str());
                    return -1;
                    ...
                    TLOG_WARN("ambiguous table_name: {}", table_name.c_str());
                    return -1;
                }
    ...
    int SelectPlanner::parse_select_field(parser::SelectField *field) {
        ...
        if (field->expr == nullptr) {
            TLOG_WARN("field expr is nullptr");
            return -1;
        }
        ...
        if (0 != create_expr_tree(field->expr, select_expr, options)) {
            TLOG_WARN("create select expr failed");
            return -1;
        }
...
// ref: https://dev.mysql.com/doc/refman/5.7/en/select.html
    int SelectPlanner::parse_select_fields() {
        ...
        for (int idx = 0; idx < fields.size(); ++idx) {
            if (fields[idx] == nullptr) {
                TLOG_WARN("cur_item->data is nullptr");
                return -1;
            }
    ...
    int SelectPlanner::parse_groupby() {
        ...
        for (int idx = 0; idx < by_items.size(); ++idx) {
            if (by_items[idx]->node_type != parser::NT_BY_ITEM) {
                TLOG_WARN("un-supported group-by item type: {}", by_items[idx]->node_type);
                return -1;
            }
            // create group by expr node
            proto::Expr group_expr;
    ...
    void SelectPlanner::create_agg_tuple_desc() {
        ...
        proto::TupleDescriptor agg_tuple;
        ...
        for (auto &id_slot: id_slot_mapping) {
            const proto::SlotDescriptor &desc = id_slot.second;
            proto::SlotDescriptor *slot = agg_tuple.add_slots();
            slot->CopyFrom(desc);
        }
    ...
    int SelectPlanner::get_base_subscribe_scan_ref_slot() {
        ...
        if (pk_field_info_ptr == nullptr) {
            TLOG_WARN("Fail to get_index_info_ptr, index_id: {}", _ctx->base_subscribe_table_id);
            return -1;
        }
        ...
        if (table_info_ptr == nullptr) {
            TLOG_WARN("Fail to get_table_info_ptr, table_id: {}", _ctx->base_subscribe_table_id);
            return -1;
        }
...
// proto::SlotDescriptor& SelectPlanner::_get_group_expr_slot() {
//     if (_group_tuple_id == -1) {
//         _group_tuple_id = _plan_table_ctx->tuple_cnt++;
//     }
//     _group_slots.push_back(proto::SlotDescriptor());
//     proto::SlotDescriptor& slot = _group_slots.back();
...
//         proto::SlotDescriptor* slot = group_tuple.add_slots();
//         slot->CopyFrom(iter);
//     }
