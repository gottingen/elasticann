...
int DDLWorkPlanner::create_txn_dml_node(std::unique_ptr<SingleTxnManagerNode>& txn_node, std::unique_ptr<ScanNode> scan_node) {
    ...
    if (_is_global_index) {
        ...
        plan_node.mutable_derive_node()->mutable_lock_secondary_node()->set_table_id(
                _table_id);
        plan_node.mutable_derive_node()->mutable_lock_secondary_node()->set_lock_secondary_type(proto::LST_GLOBAL_DDL);
        ...
        do {
            ...
            if (!_is_column_ddl) {
                ...
                if (index_info_ptr == nullptr ||
                    (index_info_ptr->state != proto::IS_WRITE_LOCAL && index_info_ptr->state != proto::IS_WRITE_ONLY)) {
                    //说明任务已经完成，或者任务失败，该索引正在被删除。
                    DB_FATAL("index info ptr is nullptr or index state is not proto::IS_WRITE_LOCAL/proto::IS_WRITE_ONLY");
