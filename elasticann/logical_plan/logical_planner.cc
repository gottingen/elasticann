...
#include "elasticann/logical_plan/load_planner.h"
...
namespace EA {
    ...
    std::map<parser::JoinType, proto::JoinType> LogicalPlanner::join_type_mapping{
            ...
            {parser::JT_INNER_JOIN, proto::INNER_JOIN},
            {parser::JT_LEFT_JOIN,  proto::LEFT_JOIN},
            {parser::JT_RIGHT_JOIN, proto::RIGHT_JOIN}
    };
    ...
    int LogicalPlanner::create_n_ary_predicate(const parser::FuncExpr *func_item,
        ...
        node->set_col_type(proto::BOOL);
        //split NOT_NULL/NOT_IN/NOT_LIKE into two predicates
        if (func_item->is_not) {
            node->set_node_type(proto::NOT_PREDICATE);
            node->set_num_children(1);
            proto::Function *item = node->mutable_fn();
        ...
        if (type == proto::LIKE_PREDICATE) {
            node->set_charset(_ctx->charset == "gbk" ? proto::GBK : proto::UTF8);
        }
        ...
        if (type == proto::OR_PREDICATE || type == proto::AND_PREDICATE) {
            ...
            while (!node_stack.empty()) {
                ...
                if (child_expr_item->expr_type == parser::ET_FUNC) {
                    ...
                    if ((type == proto::OR_PREDICATE && child_func_item->func_type == parser::FT_LOGIC_OR)
                        || (type == proto::AND_PREDICATE && child_func_item->func_type == parser::FT_LOGIC_AND)) {
    ...
    int LogicalPlanner::construct_in_predicate_node(const parser::FuncExpr *func_item, proto::Expr &expr,
                                                    proto::ExprNode **node) {
        proto::ExprNode *node_val = *node;
        node_val->set_col_type(proto::BOOL);
        //split NOT_NULL/NOT_IN/NOT_LIKE into two predicates
        if (func_item->is_not) {
            node_val->set_node_type(proto::NOT_PREDICATE);
            node_val->set_num_children(1);
            proto::Function *item = node_val->mutable_fn();
            item->set_name("logic_not");
            item->set_fn_op(parser::FT_LOGIC_NOT);
    ...
    int LogicalPlanner::handle_in_subquery(const parser::FuncExpr *func_item,
        ...
        if (!_is_correlate_subquery_expr) {
            ...
            for (int i = 0; i < tmp_expr.nodes_size(); i++) {
                proto::ExprNode *child_node = expr.add_nodes();
            ...
            proto::ExprNode *node = expr.add_nodes();
            node->set_node_type(proto::FUNCTION_CALL);
            node->set_col_type(proto::INVALID_TYPE);
            node->set_num_children(2);
            proto::JoinType join_type = proto::SEMI_JOIN;
            proto::Function *func = node->mutable_fn();
            if (!func_item->is_not) {
                func->set_name("eq");
...
// static, planner entrance
    ...
    int LogicalPlanner::generate_sql_sign(QueryContext *ctx, parser::StmtNode *stmt) {
        ...
        if (ctx->plan.nodes_size() > 0 && ctx->plan.nodes(0).node_type() == proto::PACKET_NODE) {
        ...
        if (!stat_info->family.empty() && !stat_info->table.empty()) {
            ...
            uint64_t out[2];
            butil::MurmurHash3_x64_128(stat_info->sample_sql.str().c_str(), stat_info->sample_sql.str().size(), 0x1234,
                                       out);
            ...
            if (!ctx->sign_forceindex.empty()) {
                if (ctx->sign_forceindex.count(stat_info->sign) > 0) {
                    ...
                    for (int i = 0; i < ctx->plan.nodes_size(); i++) {
                        ...
                        proto::DerivePlanNode *derive = node->mutable_derive_node();
                        proto::ScanNode *scan = derive->mutable_scan_node();
    ...
    int LogicalPlanner::add_derived_table(const std::string &database, const std::string &table,
        ...
        if (ok) {
            ...
            for (auto &field_name: _select_names) {
                ...
                field_info.type = proto::INVALID_TYPE;
    ...
    int LogicalPlanner::add_table(const std::string &database, const std::string &table,
        ...
        if (_plan_table_ctx->table_info.count(try_to_lower(alias_full_name)) == 0) {
            ...
            for (auto &sign_index: tbl_ptr->sign_forceindex) {
                std::vector<std::string> vec = turbo::StrSplit(sign_index, ':');
                if (vec.size() != 2) {
                    continue;
                }
            ...
            proto::OpType op_type = proto::OP_INSERT;
    ...
    */
        ...
        if (condition != nullptr) {
            ...
            for (uint32_t idx = 0; idx < join_filters.size(); ++idx) {
                proto::Expr *expr = join_node_mem->join_node.add_conditions();
    ...
    int
    LogicalPlanner::parse_using_cols(const parser::Vector<parser::ColumnName *> &using_col, JoinMemTmp *join_node_mem) {
        //解析using cols
...
//join节点的叶子节点
    int
    LogicalPlanner::create_join_node_from_table_name(const parser::TableName *table_name, JoinMemTmp **join_root_ptr) {
        std::string db;
        std::string table;
    ...
    int LogicalPlanner::create_join_node_from_terminator(const std::string db,
        ...
        std::unique_ptr<JoinMemTmp> join_node_mem(new JoinMemTmp);
        //叶子节点
        join_node_mem->join_node.set_join_type(proto::NULL_JOIN);
    ...
    int LogicalPlanner::flatten_filter(const parser::ExprNode *item, std::vector<proto::Expr> &filters,
                                       const CreateExprOptions &options) {
        ...
        if (!_is_correlate_subquery_expr) {
            ...
            proto::Expr *expr = _apply_root->apply_node.add_conditions();
            expr->CopyFrom(root);
    ...
    void LogicalPlanner::create_order_func_slot() {
        ...
        slot.set_slot_type(proto::INVALID_TYPE);
        _order_slots.push_back(slot);
    }

    std::vector<proto::SlotDescriptor> &LogicalPlanner::get_agg_func_slot(
        ...
        std::vector<proto::SlotDescriptor> *slots = nullptr;
        ...
        if (iter != _agg_slot_mapping.end()) {
            ...
            (*slots)[0].set_slot_type(proto::STRING);
    ...
    int LogicalPlanner::create_agg_expr(const parser::FuncExpr *expr_item, proto::Expr &expr,
                                        const CreateExprOptions &options) {
        ...
        proto::Expr agg_expr;
        proto::ExprNode *node = agg_expr.add_nodes();
        node->set_node_type(proto::AGG_EXPR);
        node->set_col_type(proto::INVALID_TYPE);
        proto::Function *func = node->mutable_fn();
        ...
        proto::DeriveExprNode *derive_node = node->mutable_derive_node();
    ...
    int LogicalPlanner::create_between_expr(const parser::FuncExpr *item, proto::Expr &expr,
                                            const CreateExprOptions &options) {
        proto::ExprNode *and_node = expr.add_nodes();
        and_node->set_col_type(proto::BOOL);
        and_node->set_node_type(proto::AND_PREDICATE);
        and_node->set_num_children(2);
        proto::Function *func = and_node->mutable_fn();
    ...
    int LogicalPlanner::create_values_expr(const parser::FuncExpr *func_item, proto::Expr &expr) {
        if (_ctx->stmt_type != parser::NT_INSERT) {
            proto::ExprNode *node = expr.add_nodes();
            node->set_node_type(proto::NULL_LITERAL);
            node->set_col_type(proto::NULL_TYPE);
            return 0;
        }
    ...
    int LogicalPlanner::construct_apply_node(QueryContext *sub_ctx,
                                             proto::Expr &expr,
                                             const proto::JoinType join_type,
                                             const CreateExprOptions &options) {
        ...
        const proto::PacketNode &packet_node = sub_ctx->plan.nodes(0).derive_node().packet_node();
        ...
        for (int i = 0; i < packet_node.projections_size(); i++) {
            const proto::Expr &child_expr = packet_node.projections(i);
        ...
        proto::Plan inner_plan;
        // 消除 packet node
        for (int i = 1; i < sub_ctx->plan.nodes_size(); i++) {
            proto::PlanNode *node = inner_plan.add_nodes();
...
// fill arg_types, return_type(col_type) and has_var_args
    int LogicalPlanner::create_scala_func_expr(const parser::FuncExpr *item,
                                               proto::Expr &expr, parser::FuncType op,
                                               const CreateExprOptions &options) {
    ...
    void LogicalPlanner::construct_literal_expr(const ExprValue &value, proto::ExprNode *node) {
...
// in (SubSelect) or select (SubSelect)
    int LogicalPlanner::create_common_subquery_expr(const parser::SubqueryExpr *item, proto::Expr &expr,
    ...
    int LogicalPlanner::handle_compare_subquery(const parser::ExprNode *expr_item, proto::Expr &expr,
                                                const CreateExprOptions &options) {
        parser::CompareSubqueryExpr *sub_query_expr = (parser::CompareSubqueryExpr *) expr_item;
        proto::Expr compare_expr;
        proto::ExprNode *node = compare_expr.add_nodes();
        node->set_node_type(proto::FUNCTION_CALL);
        node->set_col_type(proto::INVALID_TYPE);
        proto::Function *func = node->mutable_fn();
        ...
        if (!_cur_sub_ctx->expr_params.is_correlated_subquery) {
            ...
            if (_cur_sub_ctx->expr_params.func_type == parser::FT_EQ) {
                proto::ExprNode *node = compare_expr.mutable_nodes(0);
                node->set_col_type(proto::BOOL);
                node->set_node_type(proto::IN_PREDICATE);
                proto::Function *func = node->mutable_fn();
                func->set_name("in");
                ...
                if (_cur_sub_ctx->expr_params.cmp_type == parser::CMP_ALL
                    ...
                    // > >= all (null) 恒为true
                    proto::ExprNode *node = expr.add_nodes();
                    node->set_node_type(proto::BOOL_LITERAL);
                    node->set_col_type(proto::BOOL);
            ...
            switch (sub_query_expr->cmp_type) {
                case parser::CMP_ANY:
                    tmp_options.compare_type = proto::CMP_ANY;
                    break;
                case parser::CMP_ALL:
                    tmp_options.compare_type = proto::CMP_ALL;
                    break;
                case parser::CMP_SOME:
                    tmp_options.compare_type = proto::CMP_SOME;
                    break;
                default:
                    break;
            }
    ...
    int LogicalPlanner::handle_exists_subquery(const parser::ExprNode *expr_item, proto::Expr &expr,
                                               const CreateExprOptions &options) {
        ...
        if (expr_item->expr_type == parser::ET_LITETAL) {
            ...
            switch (func->func_type) {
                case parser::FT_LOGIC_NOT:
                    return create_n_ary_predicate(func, expr, proto::NOT_PREDICATE, options);
                case parser::FT_LOGIC_AND:
                    return create_n_ary_predicate(func, expr, proto::AND_PREDICATE, options);
                case parser::FT_LOGIC_OR:
                    return create_n_ary_predicate(func, expr, proto::OR_PREDICATE, options);
                case parser::FT_LOGIC_XOR:
                    return create_n_ary_predicate(func, expr, proto::XOR_PREDICATE, options);
                case parser::FT_IS_NULL:
                case parser::FT_IS_UNKNOWN:
                    return create_n_ary_predicate(func, expr, proto::IS_NULL_PREDICATE, options);
                case parser::FT_IS_TRUE:
                    return create_n_ary_predicate(func, expr, proto::IS_TRUE_PREDICATE, options);
                case parser::FT_IN:
                    return create_in_predicate(func, expr, options);
                case parser::FT_LIKE:
                case parser::FT_EXACT_LIKE:
                    return create_n_ary_predicate(func, expr, proto::LIKE_PREDICATE, options);
                case parser::FT_REGEXP:
                    return create_n_ary_predicate(func, expr, proto::REGEXP_PREDICATE, options);
    ...
    proto::SlotDescriptor &LogicalPlanner::get_scan_ref_slot(
            const std::string &alias_name, int64_t table,
            int32_t field, proto::PrimitiveType type) {
        ScanTupleInfo *tuple_info = get_scan_tuple(alias_name, table);
        _ctx->current_tuple_ids.emplace(tuple_info->tuple_id);
        proto::SlotDescriptor *slot_desc = nullptr;
    ...
    proto::SlotDescriptor &LogicalPlanner::get_values_ref_slot(int64_t table,
                                                               int32_t field, proto::PrimitiveType type) {
...
//TODO: error_code
    int LogicalPlanner::create_term_slot_ref_node(
        ...
        std::unordered_map<std::string, proto::ExprNode> *vars = nullptr;
        std::string var_name;
        auto client = _ctx->client_conn;
        if (turbo::StartsWithIgnoreCase(origin_name, "@@global.")) {
//        // TODO handle set global variable
//        return -1;
            var_name = origin_name.substr(strlen("@@global."));
            vars = &client->session_vars;
        } else if (turbo::StartsWithIgnoreCase(origin_name, "@@session.")) {
            var_name = origin_name.substr(strlen("@@session."));
            vars = &client->session_vars;
        } else if (turbo::StartsWithIgnoreCase(origin_name, "@@local.")) {
            var_name = origin_name.substr(strlen("@@local."));
            vars = &client->session_vars;
        } else if (turbo::StartsWithIgnoreCase(origin_name, "@@")) {
...
//TODO: primitive len for STRING, BOOL and nullptr
    int LogicalPlanner::create_term_literal_node(const parser::LiteralExpr *literal, proto::Expr &expr) {
        proto::ExprNode *node = expr.add_nodes();
        node->set_num_children(0);
        ...
        switch (literal->literal_type) {
            ...
            case parser::LT_DOUBLE:
                node->set_node_type(proto::DOUBLE_LITERAL);
                node->set_col_type(proto::DOUBLE);
            ...
            case parser::LT_STRING:
                ...
                break;
            case parser::LT_HEX:
                node->set_node_type(proto::HEX_LITERAL);
                node->set_col_type(proto::HEX);
                node->mutable_derive_node()->set_string_val(literal->_u.str_val.to_string());
                break;
            case parser::LT_BOOL:
                node->set_node_type(proto::BOOL_LITERAL);
            ...
            case parser::LT_PLACE_HOLDER:
                node->set_node_type(proto::PLACE_HOLDER_LITERAL);
                node->set_col_type(proto::PLACE_HOLDER);
                node->mutable_derive_node()->set_int_val(literal->_u.int64_val);
                break;
            default:
    ...
    int LogicalPlanner::create_row_expr_node(const parser::RowExpr *item, proto::Expr &expr,
                                             const CreateExprOptions &options) {
        proto::ExprNode *node = expr.add_nodes();
        node->set_col_type(proto::INVALID_TYPE);
    ...
    void LogicalPlanner::create_scan_tuple_descs() {
        for (auto &pair: _plan_table_ctx->table_tuple_mapping) {
            ...
            std::map<int32_t, proto::SlotDescriptor> id_slot_mapping;
            //reorder the slot descriptors by slot id
            for (auto &kv: slot_map) {
                const proto::SlotDescriptor &desc = kv.second;
    ...
    void LogicalPlanner::create_values_tuple_desc() {
        ...
        for (auto &kv: id_slot_mapping) {
            const proto::SlotDescriptor &desc = kv.second;
            proto::SlotDescriptor *slot = _values_tuple.add_slots();
            slot->CopyFrom(desc);
        }
    ...
    void LogicalPlanner::create_order_by_tuple_desc() {
        ...
        proto::TupleDescriptor order_tuple;
    ...
    int LogicalPlanner::create_packet_node(proto::OpType op_type) {
    ...
    int LogicalPlanner::create_filter_node(std::vector<proto::Expr> &filters,
                                           proto::PlanNodeType type) {
        if (filters.size() == 0) {
            return 0;
        }
        proto::PlanNode *where_node = _ctx->add_plan_node();
        ...
        proto::DerivePlanNode *derive = where_node->mutable_derive_node();
        proto::FilterNode *filter = derive->mutable_raw_filter_node();
    ...
    int LogicalPlanner::create_join_and_scan_nodes(JoinMemTmp *join_root, ApplyMemTmp *apply_root) {
        ...
        if (apply_root != nullptr) {
            proto::PlanNode *apply_node = _ctx->add_plan_node();
            apply_node->set_node_type(proto::APPLY_NODE);
            apply_node->set_limit(-1);
            apply_node->set_is_explain(_ctx->is_explain);
            apply_node->set_num_children(2);
            proto::DerivePlanNode *derive = apply_node->mutable_derive_node();
            ...
            *join = apply_root->apply_node;
            ...
            for (int i = 0; i < apply_root->inner_plan.nodes_size(); i++) {
                proto::PlanNode *node = _ctx->add_plan_node();
        ...
        if (join_root->is_derived_table) {
            proto::PlanNode *scan_node = _ctx->add_plan_node();
            scan_node->set_node_type(proto::DUAL_SCAN_NODE);
            scan_node->set_limit(-1);
            scan_node->set_is_explain(_ctx->is_explain);
            scan_node->set_num_children(0);
            proto::DerivePlanNode *derive = scan_node->mutable_derive_node();
            proto::ScanNode *scan = derive->mutable_scan_node();
        ...
        //叶子节点
        if (join_root->join_node.join_type() == proto::NULL_JOIN) {
            proto::PlanNode *scan_node = _ctx->add_plan_node();
            scan_node->set_node_type(proto::SCAN_NODE);
            scan_node->set_limit(-1);
            scan_node->set_is_explain(_ctx->is_explain);
            scan_node->set_num_children(0);
            proto::DerivePlanNode *derive = scan_node->mutable_derive_node();
        ...
        join_node->set_node_type(proto::JOIN_NODE);
        join_node->set_limit(-1);
        join_node->set_is_explain(_ctx->is_explain);
        join_node->set_num_children(2);
        proto::DerivePlanNode *derive = join_node->mutable_derive_node();
    ...
    void LogicalPlanner::plan_begin_txn() {
        create_packet_node(proto::OP_BEGIN);
        proto::PlanNode *plan_node = _ctx->add_plan_node();
        plan_node->set_node_type(proto::BEGIN_MANAGER_NODE);
        ...
        txn_node->set_txn_cmd(proto::TXN_BEGIN);
    ...
    void LogicalPlanner::plan_commit_txn() {
        create_packet_node(proto::OP_COMMIT);
        proto::PlanNode *plan_node = _ctx->add_plan_node();
        plan_node->set_limit(-1);
        plan_node->set_num_children(0);
        plan_node->set_node_type(proto::COMMIT_MANAGER_NODE);
        proto::DerivePlanNode *derived_node = plan_node->mutable_derive_node();
        proto::TransactionNode *txn_node = derived_node->mutable_transaction_node();

        //txn_node->set_txn_id(_ctx->txn_id);
        txn_node->set_txn_cmd(proto::TXN_COMMIT);
    }
    ...
    void LogicalPlanner::plan_commit_and_begin_txn() {
        ...
        txn_node->set_txn_cmd(proto::TXN_COMMIT_BEGIN);
        return;
    }

    void LogicalPlanner::plan_rollback_txn() {
        create_packet_node(proto::OP_ROLLBACK);
        proto::PlanNode *plan_node = _ctx->add_plan_node();
        plan_node->set_node_type(proto::ROLLBACK_MANAGER_NODE);
        plan_node->set_limit(-1);
        plan_node->set_num_children(0);
        proto::DerivePlanNode *derived_node = plan_node->mutable_derive_node();
        proto::TransactionNode *txn_node = derived_node->mutable_transaction_node();
    ...
    void LogicalPlanner::plan_rollback_and_begin_txn() {
        ...
        txn_node->set_txn_cmd(proto::TXN_ROLLBACK_BEGIN);
    }
} //namespace
