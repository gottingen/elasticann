...
namespace EA {
    ...
    typedef std::map<int, proto::CachePlan> CachePlanMap;
    ...
    public:
        ...
        void set_seq_id(int seq_id) {
            if (seq_id < _seq_id) {
                TLOG_WARN("txn:{} seq_id fallback seq_id:{} _seq_id:{}", _txn_id, seq_id, _seq_id);
                return;
            }
            _seq_id = seq_id;
        }
        ...
        void push_cmd_to_cache(int seq_id, proto::CachePlan plan_item) {
        ...
        int get_cache_plan_infos(proto::TransactionInfo &txn_info, bool for_num_rows) {
            BAIDU_SCOPED_LOCK(_cache_map_mutex);
        ...
        bool is_cstore() {
            if (_table_info.get() == nullptr) {
                ...
                TLOG_ERROR("error: no table_info");
                return false;
            }
            return _table_info->engine == proto::ROCKSDB_CSTORE;
        }
        ...
        proto::StoreReq *get_raftreq() {
            return &_store_req;
        }

        void save_last_response(proto::StoreRes &response) {
            _store_res.Clear();
            _store_res.CopyFrom(response);
        }

        void load_last_response(proto::StoreRes &response) {
        ...
        bool need_write_rollback(proto::OpType op_type) {
            if (op_type == proto::OP_ROLLBACK && (_primary_region_id != -1)) {
                return true;
            }
            return false;
        }
    ...
    public:
        ...
        proto::ErrCode err_code = proto::SUCCESS;
    ...
    private:
        ...
        void add_kvop_put(std::string &key, std::string &value, int64_t ttl_timestamp_us, bool is_primary_key) {
            //TLOG_WARN("txn:{}, add kvop put key:{}, value:{}", this,
            //           str_to_hex(key), str_to_hex(value));
            proto::KvOp *kv_op = _store_req.add_kv_ops();
            kv_op->set_op_type(proto::OP_PUT_KV);
            kv_op->set_key(key);
        ...
        void add_kvop_delete(std::string &key, bool is_primary_key) {
            //TLOG_WARN("txn:{}, add kvop delete key:{}", this, str_to_hex(key));
            proto::KvOp *kv_op = _store_req.add_kv_ops();
            kv_op->set_op_type(proto::OP_DELETE_KV);
            kv_op->set_key(key);
            kv_op->set_is_primary_key(is_primary_key);
        }
        ...
        proto::StoreReq _store_req;
        proto::StoreRes _store_res;
        ...
        proto::RegionInfo *_region_info = nullptr;
