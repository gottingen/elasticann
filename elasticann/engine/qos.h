...
// 令牌桶
    ...
    public:
        ...
        void reset_rate(const int64_t rate) {
            ...
            if (_is_extended) {
                ...
                TLOG_WARN("statistics_threshold: {}", _statistics_threshold);
            }
        }
    ...
    public:
        ...
        void inc_index_count(const int64_t index_id) {
            ...
            // 不在锁中打日志
            if (print_log) {
                TLOG_WARN("sign: {}, index_id: {}", _sign, index_id);
            }
        ...
        void dec_index_count(const int64_t index_id) {
            ...
            if (count < 0) {
                TLOG_ERROR("inc dec mismatch. sign: {}", _sign);
            }
        }
        ...
        void analyze_qps(int64_t &get_qps, int64_t &scan_qps) {
            ...
            if (get_qps != 0 || scan_qps != 0) {
                TLOG_WARN("sign: {}, get_qps: {}, scan_qps: {}, sql_qps: {},"
                           " get_real: {}, scan_real: {}, token_fetch_qps: {},"
                           " concurrency: {}",
    ...
    public:
        ...
        void adder(const int count, const int64_t expire_time) {
            ...
            if (_adder_time.compare_exchange_strong(time, expire_time)) {
                // TLOG_WARN("time: {}, expire_time: {}", time, expire_time);
                _adder << count;
            }
        }
    ...
    public:
        ...
        void analyze_qps(int64_t &total_get_qps, int64_t &total_scan_qps) {
            ...
            TLOG_WARN("total_get_qps: {}, total_scan_qps: {}, global select concurrency: {}",
        ...
        bool is_new_sign(std::shared_ptr<SqlQos> sqlqos_ptr) {
            ...
            if (_start_time.get_time() <= FLAGS_qps_statistics_minutes_ago * 2 * 60LL * 1000000LL) {
                // TLOG_INFO("StoreQos start time(s): {}", _start_time.get_time() / 1000000LL);
                return false;
            }
