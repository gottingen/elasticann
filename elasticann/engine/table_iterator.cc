...
namespace EA {
    ...
    TableIterator *Iterator::scan_primary(
        ...
        if (0 != iter->open(range, fields, field_slot, txn)) {
            TLOG_WARN("open table iterator failed");
            delete iter;
            return nullptr;
        }
        return iter;
    ...
    IndexIterator *Iterator::scan_secondary(
        ...
        if (0 != iter->open(range, dummy, field_slot, txn)) {
            TLOG_WARN("open index iterator failed");
            delete iter;
            return nullptr;
        }
        return iter;
    ...
    int Iterator::open(const IndexRange &range, std::map<int32_t, FieldInfo *> &fields,
        ...
        if (proto::I_NONE == (_idx_type = _index_info->type)) {
            TLOG_WARN("get index_type failed: {}", index_id);
            return -1;
        }
        ...
        if (nullptr == (_schema = SchemaFactory::get_instance())) {
            TLOG_WARN("get schema factory failed");
            return -1;
        }
        _is_cstore = _schema->get_table_engine(_pri_info->id) == proto::ROCKSDB_CSTORE;
        ...
        if (range.left) {
            ...
            if (-2 == ret) {
                TLOG_WARN("left key has null fields: {}", index_id);
                _valid = false;
                return 0;
            } else if (0 != ret) {
                TLOG_ERROR("Fail to encode_key, table: {}", index_id);
                return -1;
            }
            if (_idx_type == proto::I_KEY && left_primary_field_cnt > 0) {
                if (0 != range.left->encode_primary_key(*_index_info, _start, left_primary_field_cnt)) {
                    TLOG_ERROR("Fail to append_index, reg: {}, tab: {}", _region, _index_info->pk);
                    return -1;
                }
            }
        ...
        if (range.right) {
            ...
            if (-2 == ret) {
                TLOG_WARN("right key has null fields: {}", index_id);
                _valid = false;
                return 0;
            } else if (0 != ret) {
                TLOG_ERROR("Fail to encode_key, table: {}", index_id);
            ...
            if (_idx_type == proto::I_KEY && right_primary_field_cnt > 0) {
        ...
        if (_need_check_region && (_idx_type == proto::I_PRIMARY || _index_info->is_global)) {
            ...
            // TLOG_WARN("region: {}, right_key: {}, end_key:{}", _region,
            //     rocksdb::Slice(right_key).ToString(true),
            //     rocksdb::Slice(range.region_info->end_key()).ToString(true));
        ...
        // TLOG_WARN("region: {}, forward:{}, lower:{}, upper:{}, l_open:{}, r_open:{}, lower_is_start:{}, upper_is_end:{}",
        //      _region, _forward,
        //     rocksdb::Slice(_lower_bound.data()).ToString(true),
        //     rocksdb::Slice(_upper_bound.data()).ToString(true),
        ...
        if (!_iter) {
            TLOG_ERROR("create iterator failed: {}", index_id);
            return -1;
        }
        ...
        if (_forward) {
            ...
            //TLOG_DEBUG("region:{}, SeekForPrev cost:{}", _region, cost.get_time());
        ...
        if (_is_cstore && _idx_type == proto::I_PRIMARY && _valid) {
            if (0 != open_columns(fields, txn)) {
                TLOG_ERROR("create column iterators failed: {}", index_id);
                return -1;
            }
        }
        return 0;
    ...
    int Iterator::open_columns(std::map<int32_t, FieldInfo *> &fields, SmartTransaction txn) {
        ...
        for (auto &pair: fields) {
            ...
            if (!iter) {
                TLOG_ERROR("create iterator failed: {}", field_id);
                return -1;
            }
            if (_forward) {
                TimeCost cost;
                iter->Seek(key.data());
                TLOG_DEBUG("region:{}, field:{}, Seek cost:{}, valid={}",
                           _region, field_id, cost.get_time(), iter->Valid());
            } else {
                TimeCost cost;
                iter->SeekForPrev(key.data());
                TLOG_DEBUG("region:{}, field:{}, SeekForPrev cost:{}, valid={}",
                           _region, field_id, cost.get_time(), iter->Valid());
            }
    ...
    bool Iterator::_fits_left_bound(const rocksdb::Slice &key) {
        rocksdb::Slice lower(_lower_bound.data().data(), _lower_bound.size() - _lower_suffix);
    ...
    bool Iterator::_fits_right_bound(const rocksdb::Slice &key) {
        //check range end_key
        rocksdb::Slice upper(_upper_bound.data().data(), _upper_bound.size() - _upper_sufix);
        ...
        //TLOG_WARN("_fits_right_bound: {}", fits);
        return fits;
    }
    ...
    int TableIterator::get_next_internal(SmartRecord *record, int32_t tuple_id, std::unique_ptr<MemRow> *mem_row) {
        ...
        if (VAL_ONLY == _mode || KEY_VAL == _mode) {
            if (!_is_cstore) {
                ...
                if (0 != tuple_record.decode_fields(_fields, &_field_slot, record, tuple_id, mem_row)) {
                    TLOG_WARN("decode value failed: {}, _use_ttl: {}", _index_info->id, _use_ttl);
                    _valid = false;
                    return -1;
                }
            } else {
        ...
        if (KEY_ONLY == _mode || KEY_VAL == _mode) {
            ...
            if (record != nullptr) {
                if (0 != (*record)->decode_key(*_index_info, key, pos)) {
                    TLOG_WARN("decode key failed: {}", _index_info->id);
                    _valid = false;
                    return -1;
                }
            } else {
        ...
        //TLOG_WARN("parse: {} add_batch: {} nexttime: {}", parse, add_batch,next_time);
        return 0;
    }
        ...
        for (size_t i = 0; i < batch->size(); ++i) {
            ...
            while (true) {
                ...
                if (!column_key.starts_with(prefix_key.data())) {
                    mem_row->set_value(tuple_id, slot_id, field.default_expr_value);
                    TLOG_DEBUG("not match prefix, field_id={}, key={}, default_value={}",
                               field_id, iter->key().ToString(true), field.default_value);
                    break;
                }
    ...
    int IndexIterator::get_next_internal(SmartRecord *record, int32_t tuple_id, std::unique_ptr<MemRow> *mem_row) {
        while (_valid) {
            ...
            if ((_forward && !_fits_right_bound(iter_key)) || (!_forward && !_fits_left_bound(iter_key))) {
                _valid = false;
                //TLOG_WARN("get next, right bound filter, region_id: {}, record: {}", _region,
                //    record->debug_string());
                return -1;
            }
            rocksdb::Slice iter_value;
            if (_idx_type == proto::I_UNIQ || _use_ttl) {
            ...
            if (!_fits_region(iter_key, iter_value)) {
                ...
                _valid = _valid && _iter->Valid();
                //TLOG_WARN("get next, fits region filter, region_id: {}, record: {}",
                //    _region, record->debug_string());
                continue;
            }
            //TLOG_WARN("get next, in range, region_id: {}, ke: {}",
            //    _region, _iter->key().ToString(true));
            TableKey key(iter_key, true);
            ...
            if (record != nullptr) {
                if (0 != (*record)->decode_key(*_index_info, key, pos)) {
                    TLOG_WARN("decode secondary record failed: {}", _index_info->id);
                    _valid = false;
                    return -1;
                }
            } else {
            ...
            } else if (_idx_type == proto::I_KEY) {
                if (record != nullptr) {
                    if (0 != (*record)->decode_primary_key(*_index_info, key, pos)) {
                        TLOG_WARN("decode primary record failed: {}, {}, {}",
