...
void TransactionPool::read_only_txn_process(int64_t region_id,
    ...
    DB_DEBUG("dml type: %s region_id: %ld, txn_id: %lu optimize_1pc:%d", proto::OpType_Name(op_type).c_str(),
...
void TransactionPool::get_prepared_txn_info(std::unordered_map<uint64_t, proto::TransactionInfo>& prepared_txn, bool for_num_rows) {
    _txn_map.traverse(
