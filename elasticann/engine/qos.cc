...
// 归还令牌
    ...
    void QosReject::wheather_need_reject() {
        ...
        TLOG_WARN("count: {}, window_count: {}, need_reject: {}, match_reject_condition: {}, time: {}",
        ...
        if (_has_reject) {
            ...
            if (match_reject_condition && _reject_begin_time.get_time() > FLAGS_qos_reject_interval_s * 1000 * 1000LL) {
                if (StoreQos::get_instance()->mark_need_reject_sql() >= 0) {
                    ...
                    TLOG_WARN("mark need reject again count: {}, window_count: {}", count, window_count);
                }
            } else if (_reject_begin_time.get_time() > FLAGS_qos_reject_timeout_s * 1000 * 1000LL) {
                // 超时清理拒绝标记
                StoreQos::get_instance()->clear_need_reject();
                _has_reject = false;
                TLOG_WARN("clear need reject, count: {}, window_count: {}", count, window_count);
            }
        } else if (match_reject_condition) {
            if (StoreQos::get_instance()->mark_need_reject_sql() >= 0) {
                _reject_begin_time.reset();
                _has_reject = true;
                TLOG_WARN("mark need reject count: {}, window_count: {}", count, window_count);
            }
        }
    }
    ...
    void StoreQos::token_bucket_modify() {
        ...
        TLOG_WARN("max_token: {}, global_extended_token: {}, total_consume_token: {}, compression_ratio: {}",
...

        {
            ...
            for (auto iter: *ptr) {
                ...
                if (consume_token != 0) {
                    TLOG_WARN("sign: {}, committed rate: {}", iter.first, consume_token);
                }

            }

            global_extended_token += total_consume_token;
            TLOG_WARN("global extended bucket rate: {}", global_extended_token);
            // 修改全局超额令牌桶速率
    ...
    int StoreQos::mark_need_reject_sql() {
        ...
        if (ptr->empty()) {
            TLOG_WARN("sign sqlqos map is empty");
            return -1;
        }
        ...
        for (auto it = sql_with_multi_index.rbegin(); it != sql_with_multi_index.rend(); it++) {
            ...
            if (index_id <= 0) {
                TLOG_WARN(
                        "conditions not met. sign: {}, log_term [get: {}, scan: {}], short_term [get: {}, scan: {}]",
            ...
            if (sql_it != ptr->end()) {
                TLOG_WARN(
                        "sql use multi index need reject sign: {}, log_term [get: {}, scan: {}], short_term [get: {}, scan: {}]",
                        it->first, it->second.get_long_term, it->second.scan_long_term, it->second.get_short_term,
                        it->second.scan_short_term);
