...
int RuntimeState::init(const proto::StoreReq& req,
    ...
    if (req.txn_infos_size() > 0) {
        const proto::TransactionInfo& txn_info = req.txn_infos(0);
...
int64_t RuntimeState::calc_single_store_concurrency(proto::OpType op_type) {
    // comment设置优先
    ...
    if (!FLAGS_limit_unappropriate_sql || op_type != proto::OP_SELECT) {
...
int RuntimeState::memory_limit_exceeded(int64_t rows_to_check, int64_t bytes) {
    ...
    if (_mem_tracker == nullptr) {
        ...
        if (_mem_tracker == nullptr) {
            _mem_tracker = EA::MemTrackerPool::get_instance()->get_mem_tracker(_log_id);
        }
    }
