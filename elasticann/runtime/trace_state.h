...
namespace EA {
    ...
    public:
        TraceLocalNode(const char *desc,
                       proto::TraceNode *trace_node, std::vector<TraceTimeCost> *trace_cost_vec, int trace_type,
        ...
        static int64_t get_scan_rows(proto::TraceNode *trace_node) {
            int64_t rows = 0;
            if (trace_node == nullptr) {
                return 0;
            }
            if (trace_node->has_node_type() && trace_node->node_type() == proto::SCAN_NODE) {
