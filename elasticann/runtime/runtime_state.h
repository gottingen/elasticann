...
public:
    ...
    // baikalStore init
    int init(const proto::StoreReq& req,
    ...
    //int init(const proto::CachePlan& commit_plan);
    ...
    proto::TupleDescriptor* get_tuple_desc(int tuple_id) {
        if (tuple_id >= (int32_t)_tuple_descs.size()) {
            return nullptr;
        }
        return &_tuple_descs[tuple_id];
    }
    std::vector<proto::TupleDescriptor>* mutable_tuple_descs() {
        return &_tuple_descs;
    }
    ...
    const std::vector<proto::TupleDescriptor>& tuple_descs() {
        return _tuple_descs;
    }
    ...
    int64_t calc_single_store_concurrency(proto::OpType op_type);
...
public:
    ...
    proto::StoreRes*    response = nullptr;
...
private:
    ...
    std::vector<proto::TupleDescriptor> _tuple_descs;
