...
namespace EA {
    ...
    void ExprOptimize::analyze_union(QueryContext *ctx, PacketNode *packet_node) {
        ...
        PacketNode *select_packet_node = static_cast<PacketNode *>(select_plan->get_node(proto::PACKET_NODE));
        ...
        proto::TupleDescriptor *tuple_desc = ctx->get_tuple_desc(union_node->union_tuple_id());
    ...
    int ExprOptimize::analyze_derived_table(QueryContext *ctx, PacketNode *packet_node) {
        ...
        FilterNode *having_node = static_cast<FilterNode *>(plan->get_node(proto::HAVING_FILTER_NODE));
        AggNode *agg_node = static_cast<AggNode *>(plan->get_node(proto::AGG_NODE));
        AggNode *merge_agg_node = static_cast<AggNode *>(plan->get_node(proto::MERGE_AGG_NODE));
        ...
        for (auto &iter: ctx->derived_table_ctx_mapping) {
            ...
            proto::TupleDescriptor *tuple_desc = ctx->get_tuple_desc(tuple_id);
            for (auto &iter: slot_column_map) {
                ...
                if (inter_column_id >= (int) select_projections.size()) {
                    TLOG_WARN("plan illegal");
                    return -1;
                }
                proto::PrimitiveType type = select_projections[inter_column_id]->col_type();
