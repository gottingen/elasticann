...
#include "elasticann/sqlparser/expr.h"
...
namespace EA {
    int PlanRouter::analyze(QueryContext *ctx) {
        ...
        if (packet_node != nullptr && packet_node->op_type() == proto::OP_LOAD) {
            return 0;
        }
        //TLOG_INFO("need_seperate:{}", plan->need_seperate());
        std::vector<ExecNode *> scan_nodes;
        std::vector<ExecNode *> dual_scan_nodes;
        plan->get_node(proto::SCAN_NODE, scan_nodes);
        plan->get_node(proto::DUAL_SCAN_NODE, dual_scan_nodes);
        InsertNode *insert_node = static_cast<InsertNode *>(plan->get_node(proto::INSERT_NODE));
        TruncateNode *truncate_node = static_cast<TruncateNode *>(plan->get_node(proto::TRUNCATE_NODE));
        KillNode *kill_node = static_cast<KillNode *>(plan->get_node(proto::KILL_NODE));
        TransactionNode *txn_node = static_cast<TransactionNode *>(plan->get_node(proto::TRANSACTION_NODE));
    ...
    int PlanRouter::insert_node_analyze(T *node, QueryContext *ctx) {
        ...
        if (index_ptr == nullptr) {
            TLOG_WARN("invalid index info: {}", table_id);
            return ret;
        }
            ...
            if (iter != ctx->table_partition_names.end()) {
                ...
                for (auto id: record_partition_ids) {
                    if (specified_partition_ids.count(id) == 0) {
                        ...
                        TLOG_WARN("partition set match failed");
                        return -1;
                    }
                }
            }
    ...
    int PlanRouter::scan_node_analyze(RocksdbScanNode *scan_node, QueryContext *ctx, bool has_join,
        ...
        if (ctx->debug_region_id != -1) {
            ...
            if (scan_node->main_scan_index() == nullptr) {
                TLOG_WARN("main scan index is nullprt");
                return -1;
            }
            ...
            TLOG_WARN("get region_info in debug mode failed, {}", ctx->debug_region_id);
            return -1;
        }
        ...
        auto get_tuple_desc = [ctx](int32_t tuple_id) ->
                proto::TupleDescriptor * { return ctx->get_tuple_desc(tuple_id); };
    ...
    int PlanRouter::scan_plan_router(RocksdbScanNode *scan_node,
        ...
        for (auto &scan_index_info: scan_node->scan_indexs()) {
            ...
            if (index_ptr == nullptr) {
                TLOG_WARN("invalid index info: {}", scan_index_info.router_index_id);
                return -1;
            }
        ...
        proto::TupleDescriptor *tuple_desc = get_tuple_desc(scan_node->tuple_id());
        int32_t max_slot_id = tuple_desc->slots_size();
        auto pri_info = schema_factory->get_index_info_ptr(main_table_id);
        if (pri_info == nullptr) {
            TLOG_WARN("pri index info not found main_table_id: {}", main_table_id);
            return -1;
        }
    ...
    int PlanRouter::truncate_node_analyze(TruncateNode *trunc_node, QueryContext *ctx) {
        ...
        for (auto index_id: table_info.indices) {
            ...
            if (index_ptr == nullptr) {
                TLOG_WARN("get index ptr fail, index_id: {}", index_id);
                return -1;
            }
            std::map<int64_t, proto::RegionInfo> index_region_infos;
    ...
    int PlanRouter::kill_node_analyze(KillNode *kill_node, QueryContext *ctx) {
        ...
        if (ctx->kill_ctx == nullptr) {
            TLOG_ERROR("ctx->kill_ctx is null");
            return -1;
        }
...
//            ctx->return_empty = true;
            ...
            TLOG_ERROR("ctx->kill_ctx->root is null");
            return -1;
        }
        InsertNode *insert_node = static_cast<InsertNode *>(plan->get_node(proto::INSERT_NODE));
        std::vector<ExecNode *> scan_nodes;
    ...
    int PlanRouter::transaction_node_analyze(TransactionNode *txn_node, QueryContext *ctx) {
        if (txn_node->txn_cmd() == proto::TXN_BEGIN) {
    ...
    int PartitionAnalyze::analyze(QueryContext *ctx) {
        ...
        ExecNode *truncate_node = plan->get_node(proto::TRUNCATE_NODE);
