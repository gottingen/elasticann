...
namespace EA {
    ...
    public:
        ...
        TransactionNode *create_txn_node(proto::TxnCmdType cmd_type, int64_t txn_lock_timeout = -1);
    ...
    private:
        ...
        template<typename T>
        int separate_single_txn(QueryContext *ctx, T *node, proto::OpType op_type);
        ...
        int create_lock_node(int64_t table_id, proto::LockCmdType lock_type, NodeMode mode, ExecNode *manager_node);

        //生成指定索引的node, update时适用
        int create_lock_node(int64_t table_id, proto::LockCmdType lock_type, NodeMode mode,
