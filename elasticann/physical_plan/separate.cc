...
int Separate::analyze(QueryContext* ctx) {
    ...
    default: {
        DB_FATAL("invalid op_type, op_type: %s", proto::OpType_Name(op_type).c_str());
        return -1;
    }
    }
    return ret;
...
int Separate::separate_simple_select(QueryContext* ctx, ExecNode* plan) {
    ...
    std::vector<ExecNode*> scan_nodes;
    plan->get_node(proto::SCAN_NODE, scan_nodes);
    SelectManagerNode* manager_node_inter = static_cast<SelectManagerNode*>(plan->get_node(proto::SELECT_MANAGER_NODE));
    // 复用prepare的计划
...
int Separate::separate_global_update(
    ...
    FilterNode* where_filter_node = static_cast<FilterNode*>(delete_manager_node->get_node(proto::WHERE_FILTER_NODE));
    if (where_filter_node != nullptr) {
        for (auto conjunct : *(where_filter_node->mutable_conjuncts())) {
            pri_node->add_conjunct(conjunct);
        }
    }
    FilterNode* table_filter_node = static_cast<FilterNode*>(delete_manager_node->get_node(proto::TABLE_FILTER_NODE));
...
int Separate::separate_single_txn(QueryContext* ctx, T* node, proto::OpType op_type) {
    ...
    std::unique_ptr<TransactionNode> store_begin_node(create_txn_node(proto::TXN_BEGIN_STORE, ctx->user_info->txn_lock_timeout));
