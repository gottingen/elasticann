...
#include "elasticann/exec/update_manager_node.h"
#include "elasticann/exec/delete_manager_node.h"
#include "elasticann/exec/common_manager_node.h"
...
namespace EA {
    int Separate::analyze(QueryContext *ctx) {
        ...
        //TLOG_WARN("need_seperate: {}", plan->need_seperate());
        if (!plan->need_seperate() && !ctx->has_derived_table) {
            return 0;
        }
        int ret = 0;
        proto::OpType op_type = packet_node->op_type();
        switch (op_type) {
            ...
            default: {
                TLOG_ERROR("invalid op_type, op_type: {}", proto::OpType_Name(op_type).c_str());
                return -1;
            }
        }
        return ret;
    ...
    int Separate::separate_union(QueryContext *ctx) {
        ExecNode *plan = ctx->root;
        UnionNode *union_node = static_cast<UnionNode *>(plan->get_node(proto::UNION_NODE));
        for (size_t i = 0; i < ctx->sub_query_plans.size(); i++) {
            auto select_ctx = ctx->sub_query_plans[i];
            ExecNode *select_plan = select_ctx->root;
            PacketNode *packet_node = static_cast<PacketNode *>(select_plan->get_node(proto::PACKET_NODE));
            ...
            if (state->init(select_ctx.get(), nullptr) < 0) {
                TLOG_WARN("init runtime_state failed");
                return -1;
            }
    ...
    int Separate::create_full_export_node(ExecNode *plan) {
        ...
        plan->get_node(proto::SCAN_NODE, scan_nodes);
        PacketNode *packet_node = static_cast<PacketNode *>(plan->get_node(proto::PACKET_NODE));
        ...
        if (export_node == nullptr) {
            TLOG_WARN("new export node fail");
            return -1;
        }
        proto::PlanNode pb_fetch_node;
        pb_fetch_node.set_node_type(proto::FULL_EXPORT_NODE);
    ...
    int Separate::separate_select(QueryContext *ctx) {
        ...
        plan->get_node(proto::JOIN_NODE, join_nodes);
...
//普通的select请求，考虑agg_node, sort_node, limit_node下推问题
    int Separate::separate_simple_select(QueryContext *ctx, ExecNode *plan) {
        ...
        PacketNode *packet_node = static_cast<PacketNode *>(plan->get_node(proto::PACKET_NODE));
        LimitNode *limit_node = static_cast<LimitNode *>(plan->get_node(proto::LIMIT_NODE));
        AggNode *agg_node = static_cast<AggNode *>(plan->get_node(proto::AGG_NODE));
        SortNode *sort_node = static_cast<SortNode *>(plan->get_node(proto::SORT_NODE));
        std::vector<ExecNode *> scan_nodes;
        plan->get_node(proto::SCAN_NODE, scan_nodes);
        SelectManagerNode *manager_node_inter = static_cast<SelectManagerNode *>(plan->get_node(
                proto::SELECT_MANAGER_NODE));
        // 复用prepare的计划
        ...
        if (ctx->sub_query_plans.size() == 1) {
            ...
            PacketNode *packet_node = static_cast<PacketNode *>(sub_query_plan->get_node(proto::PACKET_NODE));
            manager_node->steal_projections(packet_node->mutable_projections());
            manager_node->set_sub_query_node(packet_node->children(0));
            packet_node->clear_children();
            FilterNode *filter_node = static_cast<FilterNode *>(plan->get_node(proto::WHERE_FILTER_NODE));
            ...
            TLOG_WARN("illegal plan, has multiple sub query ctx");
            for (auto iter: ctx->sub_query_plans) {
                TLOG_WARN("sql: {}", iter->sql.c_str());
            }
            return -1;
        }
        ...
        if (agg_node != nullptr) {
            ...
            proto::PlanNode pb_node;
            agg_node->transfer_pb(0, &pb_node);
            pb_node.set_node_type(proto::MERGE_AGG_NODE);
            pb_node.set_limit(-1);
    ...
    int Separate::separate_join(QueryContext *ctx, const std::vector<ExecNode *> &join_nodes) {
        for (auto &join: join_nodes) {
            std::vector<ExecNode *> scan_nodes;
            join->join_get_scan_nodes(proto::SCAN_NODE, scan_nodes);
            std::vector<ExecNode *> dual_scan_nodes;
            join->join_get_scan_nodes(proto::DUAL_SCAN_NODE, dual_scan_nodes);
            for (auto &scan_node_ptr: scan_nodes) {
                ...
                if (manager_node_parent == nullptr) {
                    TLOG_WARN("fether node children is null");
                    return -1;
                }
                while (manager_node_parent->node_type() == proto::TABLE_FILTER_NODE ||
                       manager_node_parent->node_type() == proto::WHERE_FILTER_NODE) {
                ...
                // 复用prepare的计划
                if (manager_node_parent->node_type() == proto::SELECT_MANAGER_NODE) {
                    std::map<int64_t, proto::RegionInfo> region_infos =
            ...
            for (auto &scan_node_ptr: dual_scan_nodes) {
                ...
                if (iter == ctx->derived_table_ctx_mapping.end()) {
                    TLOG_WARN("illegal plan table_id: {} _tuple_id: {}", dual->table_id(), dual->tuple_id());
                    return -1;
                }
                int32_t tuple_id = iter->first;
            ...
            if ((join_node->join_type() != proto::LEFT_JOIN
                 && join_node->join_type() != proto::RIGHT_JOIN)
            ...
            while (parent->node_type() != proto::JOIN_NODE &&
                   parent != ctx->root) {
                if (parent->node_type() == proto::LIMIT_NODE) {
                    limit_node = static_cast<LimitNode *>(parent);
                }
                if (parent->node_type() == proto::AGG_NODE) {
                    agg_node = static_cast<AggNode *>(parent);
                }
                if (parent->node_type() == proto::SORT_NODE) {
            ...
            } else if (join_node->join_type() == proto::RIGHT_JOIN) {
            ...
            while (node->children_size() > 0) {
                ...
                if (node->node_type() == proto::SELECT_MANAGER_NODE) {
                    break;
                }
            ...
            } else if (node->node_type() == proto::JOIN_NODE) {
    ...
    int Separate::separate_load(QueryContext *ctx) {
        ...
        pb_manager_node.set_node_type(proto::INSERT_MANAGER_NODE);
        ...
        if (!need_separate_plan(ctx, main_table_id)) {
            manager_node->set_op_type(proto::OP_INSERT);
            ...
            if (ret < 0) {
                TLOG_WARN("separte global insert failed table_id: {}", main_table_id);
                return -1;
            }
        }
        ...
        if (need_separate_single_txn(ctx, main_table_id)) {
            separate_single_txn(ctx, parent, proto::OP_INSERT);
        }
        return 0;
    }
    ...
    int Separate::separate_insert(QueryContext *ctx) {
        ...
        packet_node->add_child(manager_node.release());
        if (need_separate_single_txn(ctx, main_table_id)) {
            separate_single_txn(ctx, packet_node, proto::OP_INSERT);
        }
        return 0;
    }
...
// insert_node中的属性完全转义到manager_node中，析构insert_node
    int Separate::separate_global_insert(InsertManagerNode *manager_node, InsertNode *insert_node) {
        ...
        if (manager_node->need_ignore()) {
            create_lock_node(table_id, proto::LOCK_GET, Separate::BOTH, manager_node);
            create_lock_node(table_id, proto::LOCK_NO, Separate::BOTH, manager_node);
        } else if (manager_node->is_replace()) {
            create_lock_node(table_id, proto::LOCK_GET, Separate::BOTH, manager_node);
            create_lock_node(table_id, proto::LOCK_GET_ONLY_PRIMARY, Separate::PRIMARY, manager_node);
            create_lock_node(table_id, proto::LOCK_DML, Separate::BOTH, manager_node);
    ...
    int Separate::create_lock_node(
        ...
        for (auto index_id: table_info->indices) {
            ...
            if (index_info->index_hint_status == proto::IHS_VIRTUAL) {
                TLOG_INFO("index info is virtual, skip.");
                continue;
            }
            if (index_info->index_hint_status == proto::IHS_DISABLE
                && index_info->state == proto::IS_DELETE_LOCAL) {
                continue;
            }
            if (index_info->is_global) {
                if (index_info->state == proto::IS_NONE) {
                    TLOG_INFO("index info is NONE, skip.");
                    continue;
                }
                if (index_info->type == proto::I_UNIQ) {
                    global_unique_indexs.emplace_back(index_id);
                } else if (lock_type != proto::LOCK_GET) {
                    //LOGK_GET只需要关注全局唯一索引
    ...
    int Separate::create_lock_node(
        ...
        if (mode == Separate::BOTH || mode == Separate::PRIMARY) {
            ...
            plan_node.set_node_type(proto::LOCK_PRIMARY_NODE);
    ...
    int Separate::separate_update(QueryContext *ctx) {
        ...
        ExecNode *plan = ctx->root;
        PacketNode *packet_node = static_cast<PacketNode *>(plan->get_node(proto::PACKET_NODE));
        UpdateNode *update_node = static_cast<UpdateNode *>(plan->get_node(proto::UPDATE_NODE));
        std::vector<ExecNode *> scan_nodes;
        plan->get_node(proto::SCAN_NODE, scan_nodes);
        proto::PlanNode pb_manager_node;
        pb_manager_node.set_node_type(proto::UPDATE_MANAGER_NODE);
        pb_manager_node.set_limit(-1);
        ...
        if (!need_separate_plan(ctx, main_table_id)) {
            ...
            manager_node->set_op_type(proto::OP_UPDATE);
            manager_node->set_region_infos(region_infos);
            manager_node->add_child(packet_node->children(0));
        } else {
            ret = separate_global_update(manager_node.get(), update_node, scan_nodes[0]);
            if (ret < 0) {
                TLOG_WARN("separte global update failed table_id:{}", main_table_id);
                return -1;
            }
        }
        packet_node->clear_children();
        packet_node->add_child(manager_node.release());
        if (need_separate_single_txn(ctx, main_table_id)) {
            separate_single_txn(ctx, packet_node, proto::OP_UPDATE);
        }
        return 0;
    }
    ...
    int Separate::separate_delete(QueryContext *ctx) {
        ExecNode *plan = ctx->root;
        PacketNode *packet_node = static_cast<PacketNode *>(plan->get_node(proto::PACKET_NODE));
        DeleteNode *delete_node = static_cast<DeleteNode *>(plan->get_node(proto::DELETE_NODE));
        std::vector<ExecNode *> scan_nodes;
        plan->get_node(proto::SCAN_NODE, scan_nodes);
        int64_t main_table_id = delete_node->table_id();
        proto::PlanNode pb_manager_node;
        pb_manager_node.set_node_type(proto::DELETE_MANAGER_NODE);
        pb_manager_node.set_limit(-1);
        ...
        if (!need_separate_plan(ctx, main_table_id)) {
            auto region_infos = static_cast<RocksdbScanNode *>(scan_nodes[0])->region_infos();
            manager_node->set_op_type(proto::OP_DELETE);
            manager_node->set_region_infos(region_infos);
            manager_node->add_child(packet_node->children(0));
        } else {
            int ret = separate_global_delete(manager_node.get(), delete_node, scan_nodes[0]);
            if (ret < 0) {
                TLOG_WARN("separte global delete failed table_id:{}", main_table_id);
                return -1;
            }
        }
        packet_node->clear_children();
        packet_node->add_child(manager_node.release());
        if (need_separate_single_txn(ctx, main_table_id)) {
            separate_single_txn(ctx, packet_node, proto::OP_DELETE);
        }
        return 0;
    }
    ...
    int Separate::separate_global_update(
        ...
        proto::PlanNode pb_delete_manager_node;
        pb_delete_manager_node.set_node_type(proto::DELETE_MANAGER_NODE);
        ...
        FilterNode *where_filter_node = static_cast<FilterNode *>(delete_manager_node->get_node(
                proto::WHERE_FILTER_NODE));
        if (where_filter_node != nullptr) {
            for (auto conjunct: *(where_filter_node->mutable_conjuncts())) {
                pri_node->add_conjunct(conjunct);
            }
        }
        FilterNode *table_filter_node = static_cast<FilterNode *>(delete_manager_node->get_node(
                proto::TABLE_FILTER_NODE));
        ...
        //生成basic_insert
        proto::PlanNode pb_insert_manager_node;
        pb_insert_manager_node.set_node_type(proto::INSERT_MANAGER_NODE);
        ...
        create_lock_node(main_table_id, proto::LOCK_DML, Separate::BOTH,
                         manager_node->global_affected_index_ids(),
                         manager_node->local_affected_index_ids(),
 ...
 */
    int Separate::separate_global_delete(
        ...
        create_lock_node(main_table_id, proto::LOCK_GET_DML, Separate::PRIMARY, manager_node);
        create_lock_node(main_table_id, proto::LOCK_DML, Separate::GLOBAL, manager_node);
        ...
        FilterNode *where_filter_node = static_cast<FilterNode *>(manager_node->get_node(proto::WHERE_FILTER_NODE));
        if (where_filter_node != nullptr) {
            for (auto conjunct: *(where_filter_node->mutable_conjuncts())) {
                pri_node->add_conjunct(conjunct);
            }
        }
        FilterNode *table_filter_node = static_cast<FilterNode *>(manager_node->get_node(proto::TABLE_FILTER_NODE));
        if (table_filter_node != nullptr) {
            for (auto conjunct: *(table_filter_node->mutable_conjuncts())) {
                pri_node->add_conjunct(conjunct);
            }
    ...
    template<typename T>
    int Separate::separate_single_txn(QueryContext *ctx, T *node, proto::OpType op_type) {
        ...
        std::unique_ptr<TransactionNode> store_begin_node(
                create_txn_node(proto::TXN_BEGIN_STORE, ctx->user_info->txn_lock_timeout));
        if (store_begin_node.get() == nullptr) {
            TLOG_WARN("create store_begin_node failed");
            return -1;
        }
        // create store prepare node
        std::unique_ptr<TransactionNode> store_prepare_node(create_txn_node(proto::TXN_PREPARE));
        if (store_prepare_node.get() == nullptr) {
            TLOG_WARN("create store_prepare_node failed");
            return -1;
        }
        // create store commit node
        std::unique_ptr<TransactionNode> store_commit_node(create_txn_node(proto::TXN_COMMIT_STORE));
        ...
        std::unique_ptr<TransactionNode> store_rollback_node(create_txn_node(proto::TXN_ROLLBACK_STORE));
    ...
    int Separate::separate_truncate(QueryContext *ctx) {
        ...
        proto::PlanNode pb_manager_node;
        pb_manager_node.set_node_type(proto::TRUNCATE_MANAGER_NODE);
        pb_manager_node.set_limit(-1);
        ...
        manager_node->init(pb_manager_node);
        manager_node->set_op_type(proto::OP_TRUNCATE_TABLE);
    ...
    int Separate::separate_kill(QueryContext *ctx) {
        ...
        proto::PlanNode pb_manager_node;
        pb_manager_node.set_node_type(proto::KILL_MANAGER_NODE);
        pb_manager_node.set_limit(-1);
        std::unique_ptr<CommonManagerNode> manager_node(new(std::nothrow) CommonManagerNode);
        if (manager_node.get() == nullptr) {
            TLOG_WARN("create manager_node failed");
        ...
        manager_node->init(pb_manager_node);
        manager_node->set_op_type(proto::OP_KILL);
        manager_node->set_region_infos(packet_node->children(0)->region_infos());
        manager_node->add_child(packet_node->children(0));
        packet_node->clear_children();
        packet_node->add_child(manager_node.release());
    ...
    int Separate::separate_commit(QueryContext *ctx) {
        ...
        CommitManagerNode *commit_node =
                static_cast<CommitManagerNode *>(plan->get_node(proto::COMMIT_MANAGER_NODE));
        std::unique_ptr<TransactionNode> store_prepare_node(create_txn_node(proto::TXN_PREPARE));
        if (store_prepare_node.get() == nullptr) {
            TLOG_WARN("create store_prepare_node failed");
            return -1;
        ...
        if (commit_node->txn_cmd() == proto::TXN_COMMIT_BEGIN) {
            // create store begin node
            std::unique_ptr<TransactionNode> store_begin_node(
                    create_txn_node(proto::TXN_BEGIN_STORE, ctx->user_info->txn_lock_timeout));
            if (store_begin_node.get() == nullptr) {
    ...
    int Separate::separate_rollback(QueryContext *ctx) {
        ...
        RollbackManagerNode *rollback_node =
                static_cast<RollbackManagerNode *>(plan->get_node(proto::ROLLBACK_MANAGER_NODE));
        ...
        if (rollback_node->txn_cmd() == proto::TXN_ROLLBACK_BEGIN) {
            // create store begin node
            std::unique_ptr<TransactionNode> store_begin_node(
                    create_txn_node(proto::TXN_BEGIN_STORE, ctx->user_info->txn_lock_timeout));
            if (store_begin_node.get() == nullptr) {
    ...
    int Separate::separate_begin(QueryContext *ctx) {
        ...
        BeginManagerNode *begin_node =
                static_cast<BeginManagerNode *>(plan->get_node(proto::BEGIN_MANAGER_NODE));
        // create store begin node
        std::unique_ptr<TransactionNode> store_begin_node(
                create_txn_node(proto::TXN_BEGIN_STORE, ctx->user_info->txn_lock_timeout));
        if (store_begin_node.get() == nullptr) {
    ...
    TransactionNode *Separate::create_txn_node(proto::TxnCmdType cmd_type, int64_t txn_lock_timeout) {
        // create fetcher node
        proto::PlanNode pb_plan_node;
        // pb_plan_node.set_txn_id(txn_id);
        pb_plan_node.set_node_type(proto::TRANSACTION_NODE);
        pb_plan_node.set_limit(-1);
    ...
    SelectManagerNode *Separate::create_select_manager_node() {
        proto::PlanNode pb_manager_node;
        pb_manager_node.set_node_type(proto::SELECT_MANAGER_NODE);
        pb_manager_node.set_limit(-1);
