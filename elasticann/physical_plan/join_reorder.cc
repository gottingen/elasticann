...
namespace EA {
    int JoinReorder::analyze(QueryContext *ctx) {
        JoinNode *join = static_cast<JoinNode *>(ctx->root->get_node(proto::JOIN_NODE));
        if (join == nullptr) {
            return 0;
        }
        ...
        ScanNode *first_node = static_cast<ScanNode *>(
                tuple_join_child_map[tuple_order[0]]->get_node(proto::SCAN_NODE));
        ...
        for (auto &pair: tuple_join_child_map) {
            ...
            ScanNode *scan_node = static_cast<ScanNode *>(
                    pair.second->get_node(proto::SCAN_NODE));
        ...
        while (tuple_equals_map.size() > 0) {
            ...
            if (select_tuple == -1) {
                // no equal join
                TLOG_WARN("has no equal condition in join");
                return 0;
            }
        ...
        for (size_t i = 1; i < tuple_reorder.size(); i++) {
            proto::PlanNode pb;
            pb.set_node_type(proto::JOIN_NODE);
            pb.set_limit(-1);
            pb.set_is_explain(ctx->is_explain);
            pb.set_num_children(2);
            proto::JoinNode *pb_join = pb.mutable_derive_node()->mutable_join_node();
            pb_join->set_join_type(proto::INNER_JOIN);
            for (size_t j = 0; j < i; j++) {
        ...
        if (!conditions.empty()) {
            TLOG_ERROR("join reorder predicate_pushdown fail, size:{}", conditions.size());
            return -1;
        }
        TLOG_WARN("join has reordered");
        //proto::Plan plan;
        //ExecNode::create_pb_plan(&plan, ctx->root);
        //TLOG_INFO("before: {}", plan.DebugString().c_str());
        join->get_parent()->replace_child(join, last_node);
        join->reorder_clear();
        delete join;
        //proto::Plan plan2;
        //ExecNode::create_pb_plan(&plan2, ctx->root);
        //TLOG_INFO("after: {}", plan2.DebugString().c_str());
        return 0;
    }
