...
namespace EA {
    ...
    int IndexSelector::analyze(QueryContext *ctx) {
        ...
        root->get_node(proto::SCAN_NODE, scan_nodes);
        if (scan_nodes.size() == 0) {
            return 0;
        }
        LimitNode *limit_node = static_cast<LimitNode *>(root->get_node(proto::LIMIT_NODE));
        AggNode *agg_node = static_cast<AggNode *>(root->get_node(proto::AGG_NODE));
        SortNode *sort_node = static_cast<SortNode *>(root->get_node(proto::SORT_NODE));
        JoinNode *join_node = static_cast<JoinNode *>(root->get_node(proto::JOIN_NODE));
        // TODO sort可以增加topN
        if (limit_node != nullptr && sort_node != nullptr) {
            sort_node->set_limit(limit_node->other_limit());
        }
        for (auto &scan_node_ptr: scan_nodes) {
            if (static_cast<ScanNode *>(scan_node_ptr)->engine() == proto::INFORMATION_SCHEMA) {
                continue;
            }
...
// https://dev.mysql.com/doc/refman/5.6/en/comparison-operators.html#operator_less-than-or-equal
    ...
    void IndexSelector::hit_field_or_like_range(ExprNode *expr, std::map<int32_t, FieldRange> &field_range_map,
        ...
        for (auto sub_expr: or_exprs) {
            if (sub_expr->node_type() != proto::LIKE_PREDICATE) {
                return;
            }
        ...
        index_ids.reserve(2);
        {
            ...
            for (auto &sub_expr: or_exprs) {
                ...
                new_fulltext_flag =
                        new_fulltext_flag && is_field_has_arrow_reverse_index(table_id, field_id, &index_id);
        ...
        auto &inner_node = std::get<FulltextInfoNode::FulltextChildType>(fulltext_index_node->info);
        ...
        for (auto sub_expr: or_exprs) {
            ...
            if (new_fulltext_flag) {
                ...
                auto &or_node = std::get<FulltextInfoNode::FulltextChildType>(back_node->info);
                ...
                or_node.children.back()->type = proto::FNT_TERM;
            }
    ...
    void IndexSelector::hit_match_against_field_range(ExprNode *expr,
        ...
        if (is_field_has_arrow_reverse_index(table_id, field_id, &index_id)) {
            ...
            inner_node.children.back()->type = proto::FNT_TERM;
        }
    ...
    void IndexSelector::hit_field_range(ExprNode *expr,
                                        ...
                                        int64_t table_id, FulltextInfoNode *fulltext_index_node) {
        if (expr->node_type() == proto::OR_PREDICATE) {
        ...
        int32_t field_id = slot_ref->field_id();
        proto::PrimitiveType col_type = expr->children(0)->col_type();
        for (uint32_t i = 1; i < expr->children_size(); i++) {
            if (!expr->children(i)->is_constant()) {
                return;
            }
        ...
        switch (expr->node_type()) {
            ...
            case proto::LIKE_PREDICATE: {
                ...
                ExprValue prefix_value(proto::STRING);
    ...
    int64_t IndexSelector::index_selector(const std::vector<proto::TupleDescriptor> &tuple_descs,
                                          ScanNode *scan_node,
        ...
        fulltext_index_tree.root->type = proto::FNT_AND;
        // expr => field_ids
    ...
    int IndexSelector::select_partition(SmartTable &table_info, ScanNode *scan_node,
        ...
        if (table_info->partition_ptr != nullptr) {
            ...
            if (partition_type == proto::PT_HASH) {
            ...
            } else if (partition_type == proto::PT_RANGE) {
                // todo
            }
        }
        return 0;
