...
namespace EA {
    void DictManager::create_dict(const ::EA::proto::OpsServiceRequest &request, braft::Closure *done) {
        if (!request.has_request_dict()) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INPUT_PARAM_ERROR, "empty dict request");
            return;
        }
                                     ...
                                     create_request.version().patch());
        {
            ...
            if ((tit != _tombstone_dicts.end()) && (tit->second.find(version) != tit->second.end())) {
                ...
                FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INPUT_PARAM_ERROR, "dict already removed");
                return;
            }
        }
        ...
        if (it->second.find(version) != it->second.end()) {
            ...
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INPUT_PARAM_ERROR, "dict already exist");
            return;
        }
        if (!it->second.empty() && it->second.rbegin()->first >= version) {
            ...
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INPUT_PARAM_ERROR,
                                                 "Version numbers must increase monotonically");
            return;
        }
        ...
        if (!st.ok()) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::PARSE_TO_PB_FAIL, std::string(st.message()));
            return;
        }
        if (!entity.SerializeToString(&rocks_value)) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::PARSE_TO_PB_FAIL, "serializeToArray fail");
            return;
        }

        int ret = ServiceRocksdb::get_instance()->put_meta_info(rocks_key, rocks_value);
        if (ret < 0) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INTERNAL_ERROR, "write db fail");
            return;
        }
        it->second[version] = entity;
        TLOG_INFO("dict :{} version: {} create", name, version.to_string());
        FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::SUCCESS, "success");
    }

    void DictManager::upload_dict(const ::EA::proto::OpsServiceRequest &request, braft::Closure *done) {
        /// check valid
        if (!request.has_request_dict()) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INPUT_PARAM_ERROR, "miss field dict request_dict");
            return;
        }
        auto &upload_request = request.request_dict();
        if (!request.request_dict().has_offset()) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INPUT_PARAM_ERROR, "miss field dict offset");
            return;
        }

        if (!request.request_dict().has_content()) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INPUT_PARAM_ERROR, "miss field dict content");
            return;
        }
        if (request.request_dict().content().empty()) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INPUT_PARAM_ERROR, "empty dict content");
            return;
        }

        if (!upload_request.dict().has_version()) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INPUT_PARAM_ERROR, "miss field dict version");
            return;
        }
        ...
        auto it = _dicts.find(name);
        if (it == _dicts.end()) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::PARSE_TO_PB_FAIL, "dict not exist");
            return;
        }
        ...
        if (pit == it->second.end()) {
            ...
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INPUT_PARAM_ERROR, "dict not exist");
        }
        ...
        if (fd < 0) {
            ...
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INTERNAL_ERROR, "open dict error");
        }
        ssize_t nw = full_pwrite(fd, upload_request.content().data(), upload_request.content().size(),
                                 upload_request.offset());
        if (nw < 0) {
            TLOG_WARN("upload dict :{} version: {} open file error", name, version.to_string());
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INTERNAL_ERROR, "open file error");
        }
        ...
        if (pit->second.finish()) {
            ...
            if (!cksm.ok()) {
                ...
                FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INTERNAL_ERROR, "check md5 fail");
            }
            if (cksm.value() != pit->second.cksm()) {
                ...
                FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INTERNAL_ERROR, "md5 not match");
            }
        }
        ::close(fd);
        /// persist
    ...
    void DictManager::remove_dict(const ::EA::proto::OpsServiceRequest &request, braft::Closure *done) {
        ...
        int ret = ServiceRocksdb::get_instance()->put_meta_info(rocks_key, rocks_value);
        if (ret < 0) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INTERNAL_ERROR, "delete from db fail");
            return;
        }
    ...
    void DictManager::restore_dict(const ::EA::proto::OpsServiceRequest &request, braft::Closure *done) {
        ...
        int ret = ServiceRocksdb::get_instance()->put_meta_info(rocks_key, rocks_value);
        if (ret < 0) {
            FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::INTERNAL_ERROR, "write from db fail");
            return;
        }
    ...
    int DictManager::load_snapshot() {
        ...
        std::string files_prefix = ServiceConstants::FILE_IDENTIFY;
        ...
        iter->Seek(files_prefix);
        for (; iter->Valid(); iter->Next()) {
            if (load_dict_snapshot(iter->value().ToString()) != 0) {
                return -1;
            }
        }
    ...
    int DictManager::load_dict_snapshot(const std::string &value) {
        ...
        if (dict_pb.tombstone()) {
            if (_tombstone_dicts.find(dict_pb.name()) == _tombstone_dicts.end()) {
                _tombstone_dicts[dict_pb.name()] = std::map<turbo::ModuleVersion, EA::proto::DictEntity>();
            }
            auto it = _tombstone_dicts.find(dict_pb.name());
            turbo::ModuleVersion version(dict_pb.version().major(), dict_pb.version().minor(),
                                         dict_pb.version().patch());
            it->second[version] = dict_pb;
        } else {
            if (_dicts.find(dict_pb.name()) == _dicts.end()) {
                _dicts[dict_pb.name()] = std::map<turbo::ModuleVersion, EA::proto::DictEntity>();
            }
            auto it = _dicts.find(dict_pb.name());
            turbo::ModuleVersion version(dict_pb.version().major(), dict_pb.version().minor(),
                                         dict_pb.version().patch());
            it->second[version] = dict_pb;
        }
    ...
    std::string DictManager::make_dict_key(const std::string &name, const turbo::ModuleVersion &version) {
        return ServiceConstants::FILE_IDENTIFY + name + version.to_string();
    }
