...
//
#include "elasticann/ops/dict/dict_server.h"
...
namespace EA {
    ...
    void DictServer::dict_query(::google::protobuf::RpcController *controller,
        ...
        switch (op_type) {
            case EA::proto::QUERY_DOWNLOAD_DICT:{
                QueryFileManager::get_instance()->download_dict(request, response);
                break;
            }
            case EA::proto::QUERY_INFO_DICT:{
                QueryFileManager::get_instance()->dict_info(request, response);
                break;
            }
            case EA::proto::QUERY_TOMBSTONE_DICT_INFO:{
                QueryFileManager::get_instance()->tombstone_dict_info(request, response);
                break;
            }
            case EA::proto::QUERY_LIST_DICT:{
                QueryFileManager::get_instance()->list_dict(request, response);
                break;
            }
            case EA::proto::QUERY_LIST_DICT_VERSION:{
                QueryFileManager::get_instance()->list_dict_version(request, response);
                break;
            }
            case EA::proto::QUERY_TOMBSTONE_LIST_DICT:{
                QueryFileManager::get_instance()->tombstone_list_dict(request, response);
                break;
            }
            case EA::proto::QUERY_TOMBSTONE_LIST_DICT_VERSION:{
                QueryFileManager::get_instance()->tombstone_list_dict_version(request, response);
                break;
            }
            default:{
                response->set_errcode(proto::INPUT_PARAM_ERROR);
    ...
    int DictServer::init(const std::vector<braft::PeerId> &peers) {
        auto ret = ServiceRocksdb::get_instance()->init(FLAGS_dict_db_path);
        if (ret < 0) {
        ...
        /// clean read links
        QueryFileManager::get_instance()->init();
        return 0;
    }
