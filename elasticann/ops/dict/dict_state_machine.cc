...
namespace EA {
    ...
    int DictStateMachine::init(const std::vector<braft::PeerId> &peers) {
        ...
        options.log_uri = FLAGS_dict_log_uri + "0";
    ...
    void DictStateMachine::save_snapshot(braft::Closure *done, rocksdb::Iterator *iter, braft::SnapshotWriter *writer) {
        brpc::ClosureGuard done_guard(done);
        ...
        for (; iter.valid(); iter.next()) {
            ...
            switch (request.op_type()) {
                ...
                default: {
                    ...
                    FILE_SERVICE_SET_DONE_AND_RESPONSE(done, proto::UNSUPPORT_REQ_TYPE, "unsupport request type");
                }
            }
    ...
    int DictStateMachine::send_set_peer_request(bool remove_peer, const std::string &change_peer) {
        DictServerInteract dict_server_interact;
