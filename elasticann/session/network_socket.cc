...
namespace EA {
    ...
    NetworkSocket::NetworkSocket() {
        ...
        proto::ExprNode str_node;
        str_node.set_node_type(proto::STRING_LITERAL);
        str_node.set_col_type(proto::STRING);
    ...
    bool NetworkSocket::transaction_has_write() {
        ...
        for (auto &pair: cache_plans) {
            proto::OpType type = pair.second.op_type;
            if (type == proto::OP_INSERT || type == proto::OP_DELETE ||
                type == proto::OP_UPDATE || type == proto::OP_SELECT_FOR_UPDATE) {
                return true;
            }
        }
        return false;
