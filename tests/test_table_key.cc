...
namespace EA {
    ...
    DOCTEST_TEST_CASE("test_append_i8, case_all") {
        MutTableKey mut_key;
        for (int16_t idx = SCHAR_MIN; idx <= SCHAR_MAX; idx++) {
            mut_key.append_i8((int8_t) idx);
        }
        DOCTEST_CHECK_EQ(256 * sizeof(int8_t), mut_key.size());
        TableKey key(mut_key);
        int pos = 0;
        for (int16_t idx = SCHAR_MIN; idx <= SCHAR_MAX; idx++) {
            DOCTEST_CHECK_EQ((int8_t) idx, key.extract_i8(pos));
            pos += sizeof(int8_t);
        }
    }

    DOCTEST_TEST_CASE("test_append_i16, case_all") {
        MutTableKey mut_key;
        for (int32_t idx = SHRT_MIN; idx <= SHRT_MAX; idx++) {
            mut_key.append_i16((int16_t) idx);
        }
        DOCTEST_CHECK_EQ((SHRT_MAX - SHRT_MIN + 1) * sizeof(int16_t), mut_key.size());
        TableKey key(mut_key);
        int pos = 0;
        for (int32_t idx = SHRT_MIN; idx <= SHRT_MAX; idx++) {
            DOCTEST_CHECK_EQ((int16_t) idx, key.extract_i16(pos));
            pos += sizeof(int16_t);
        }
    }

    DOCTEST_TEST_CASE("test_append_i32, case_all") {
        MutTableKey mut_key;
        int count = 0;
        ...
        DOCTEST_CHECK_EQ(count * sizeof(int32_t), mut_key.size());
        TableKey key(mut_key);
        int pos = 0;
        for (int64_t idx = INT_MIN; idx <= INT_MAX; idx += (INT_MAX / 100000)) {
            DOCTEST_CHECK_EQ((int32_t) idx, key.extract_i32(pos));
            pos += sizeof(int32_t);
        }
    }

    DOCTEST_TEST_CASE("test_append_i64, case_all") {
        MutTableKey mut_key;
        int count = 0;
        ...
        DOCTEST_CHECK_EQ(count * sizeof(int64_t), mut_key.size());
        TableKey key(mut_key);
        int pos = 0;
        for (int64_t idx = -9223372036854775808UL; idx < 9223372036854775807UL - gap; idx += gap) {
            DOCTEST_CHECK_EQ(idx, key.extract_i64(pos));
            pos += sizeof(int64_t);
        }
    }

    DOCTEST_TEST_CASE("test_append_float, case_all") {
        MutTableKey mut_key;
        int count = 0;
        ...
        DOCTEST_CHECK_EQ(count * sizeof(float), mut_key.size());
        TableKey key(mut_key);
        int pos = 0;
        for (uint32_t idx = 0; idx < 10000; ++idx) {
    ...
    DOCTEST_TEST_CASE("test_append_double, case_all") {
        MutTableKey mut_key;
        int count = 0;
        srand((unsigned) time(NULL));
        ...
        DOCTEST_CHECK_EQ(count * sizeof(double), mut_key.size());
        TableKey key(mut_key);
        int pos = 0;
        for (uint32_t idx = 0; idx < 100000; ++idx) {
            double diff = key.extract_double(pos) - double_vals[idx];
            DOCTEST_CHECK_EQ(true, diff < 1e-9);
            pos += sizeof(double);
        }
    }

    DOCTEST_TEST_CASE("test_sort_float, case_all") {
        srand((unsigned) time(NULL));
        std::vector<float> float_vals;
        ...
        for (uint32_t idx = 0; idx < float_vals.size(); ++idx) {
            ...
            DOCTEST_CHECK_EQ(key.extract_float(0), float_vals[idx]);
        }
    }

    DOCTEST_TEST_CASE("test_sort_double, case_all") {
        srand((unsigned) time(NULL));
        std::vector<double> double_vals;
        std::vector<std::string> str_vals;
        ...
        for (uint32_t idx = 0; idx < double_vals.size(); ++idx) {
            ...
            DOCTEST_CHECK_EQ(key.extract_double(0), double_vals[idx]);
        }
    }
