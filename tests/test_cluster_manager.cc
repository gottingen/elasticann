...
public:
    ClusterManagerTest() {
        ...
        _state_machine = new EA::MetaStateMachine(peer_id);
...
protected:
    ...
    EA::MetaStateMachine *_state_machine;
};
...
DOCTEST_TEST_CASE_FIXTURE(ClusterManagerTest, "test_add_and_drop") {
    ...
    DOCTEST_REQUIRE_EQ(3, _cluster_manager->_logical_physical_map.size());
    for (auto &_logical_physical: _cluster_manager->_logical_physical_map) {
        if (_logical_physical.first == EA::FLAGS_default_logical_room) {
            DOCTEST_REQUIRE_EQ(1, _logical_physical.second.size());
        } else {
            DOCTEST_REQUIRE_EQ(0, _logical_physical.second.size());
        }
    }
    //增加一个逻辑机房
    ...
    _cluster_manager->add_logical(request_logical, NULL);
    DOCTEST_REQUIRE_EQ(4, _cluster_manager->_logical_physical_map.size());
    for (auto &_logical_physical: _cluster_manager->_logical_physical_map) {
        if (_logical_physical.first == EA::FLAGS_default_logical_room) {
            DOCTEST_REQUIRE_EQ(1, _logical_physical.second.size());
        } else {
    ...
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_physical_info.size());
    DOCTEST_REQUIRE_EQ(0, _cluster_manager->_instance_physical_map.size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_physical_instance_map.size());
    ...
    _cluster_manager->add_physical(request_physical, NULL);
    DOCTEST_REQUIRE_EQ(4, _cluster_manager->_logical_physical_map.size());
    DOCTEST_REQUIRE_EQ(3, _cluster_manager->_physical_info.size());
    ...
    DOCTEST_REQUIRE_EQ(0, _cluster_manager->_instance_physical_map.size());
    DOCTEST_REQUIRE_EQ(6, _cluster_manager->_physical_instance_map.size());
    DOCTEST_REQUIRE_EQ(0, _cluster_manager->_instance_info.size());
    ...
    DOCTEST_REQUIRE_EQ(6, _cluster_manager->_physical_info.size());
    DOCTEST_REQUIRE_EQ(4, _cluster_manager->_logical_physical_map.size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_instance_physical_map.size());
    ...
    DOCTEST_REQUIRE_EQ(6, _cluster_manager->_physical_info.size());
    DOCTEST_REQUIRE_EQ(5, _cluster_manager->_logical_physical_map.size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_instance_physical_map.size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_physical_instance_map["py2"].size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_instance_info.size());
    _cluster_manager->load_snapshot();
    ...
    DOCTEST_REQUIRE_EQ(5, _cluster_manager->_physical_info.size());
    DOCTEST_REQUIRE_EQ(4, _cluster_manager->_logical_physical_map.size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_instance_physical_map.size());
    DOCTEST_REQUIRE_EQ(2, _cluster_manager->_logical_physical_map["lg1"].size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_logical_physical_map["lg4"].size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_logical_physical_map["lg2"].size());
    _cluster_manager->load_snapshot();
    DOCTEST_REQUIRE_EQ(5, _cluster_manager->_physical_info.size());
    DOCTEST_REQUIRE_EQ(4, _cluster_manager->_logical_physical_map.size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_instance_physical_map.size());
    ...
    DOCTEST_REQUIRE_EQ(3, _cluster_manager->_logical_physical_map.size());
    DOCTEST_REQUIRE_EQ(0, _cluster_manager->_instance_physical_map.size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_logical_physical_map["lg1"].size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_logical_physical_map["lg4"].size());
    _cluster_manager->load_snapshot();
    DOCTEST_REQUIRE_EQ(3, _cluster_manager->_physical_info.size());
    DOCTEST_REQUIRE_EQ(3, _cluster_manager->_logical_physical_map.size());
    ...
    DOCTEST_REQUIRE_EQ(4, _cluster_manager->_physical_info.size());
    DOCTEST_REQUIRE_EQ(3, _cluster_manager->_logical_physical_map.size());
    DOCTEST_REQUIRE_EQ(4, _cluster_manager->_instance_physical_map.size());
    DOCTEST_REQUIRE_EQ(2, _cluster_manager->_logical_physical_map["lg1"].size());
    DOCTEST_REQUIRE_EQ(1, _cluster_manager->_logical_physical_map["lg4"].size());
    _cluster_manager->load_snapshot();
    DOCTEST_REQUIRE_EQ(4, _cluster_manager->_physical_info.size());
    ...
    DOCTEST_REQUIRE_EQ(0, ret);
    DOCTEST_REQUIRE(select1 != select2);
    //轮询再选一次
    ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""}, exclude_stores, selected_instance);
    DOCTEST_REQUIRE_EQ(0, ret);
    DOCTEST_REQUIRE(select1 == selected_instance);

    //无resource_tag, 有exclude_store
    exclude_stores.insert("127.0.0.1:8010");
    ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""}, exclude_stores, selected_instance);
    DOCTEST_REQUIRE_EQ(0, ret);
    DOCTEST_REQUIRE("127.0.0.2:8010" == selected_instance);
    ...
    ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""}, exclude_stores, selected_instance);
    DOCTEST_REQUIRE_EQ(0, ret);
    DOCTEST_REQUIRE("127.0.0.1:8010" == selected_instance);
 ...
 */
DOCTEST_TEST_CASE_FIXTURE(ClusterManagerTest, "test_load_balance_by_network_segment_rolling") {
...
// resource tag只有1个实例, 不开网段balance，正常rolling
    resource_tag = "tag";
    _cluster_manager->
            get_network_segment_count(resource_tag, network_segment_size, prefix
    );
    DOCTEST_REQUIRE_EQ(1, network_segment_size);
    DOCTEST_REQUIRE_EQ(32, prefix);
    for (
            int i = 0;
            i < 40; ++i) {
    ...
    DOCTEST_REQUIRE_EQ(pick_times["127.1.1.1:8100"], 40);
// 开启网段rolling
    _state_machine->
...
// exclude包含唯一instance, 则pick失败
    DOCTEST_REQUIRE_EQ(pick_times["127.1.1.1:8100"], 80);
...
// resource tag下只有4个实例, 即使开了按照网段load balance, 也退化成按照ip来, 不开网段balance正常rolling
    ...
    DOCTEST_REQUIRE_EQ(4, network_segment_size);
    DOCTEST_REQUIRE_EQ(32, prefix);
    pick_times.
    ...
    for (
        auto &pair
            : pick_times) {
        DOCTEST_REQUIRE_EQ(pair.second, 10);
    }
// 开启了网段load balance
    _state_machine->
            set_network_segment_balance(resource_tag,
...
// 一个region有三个peer，进行load balance, 必定会选另一个网段下的instance
    ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""},
                                                    ...
                                                    selected_instance);
    DOCTEST_REQUIRE_EQ(0, ret);
    DOCTEST_REQUIRE_EQ(selected_instance, "127.0.1.4:8010");
...
// resource tag下有20个实例, 网段分布均匀, 不开网段balance, 正常rolling
    ...
    DOCTEST_REQUIRE_EQ(10, network_segment_size);
    DOCTEST_REQUIRE_EQ(16, prefix);
    pick_times.
    ...
    for (
        auto &pair
            : pick_times) {
        DOCTEST_REQUIRE_EQ(pair.second, 2);
    }
// 开启了网段balance
    _state_machine->
            set_network_segment_balance(resource_tag,
    ...
    for (
        auto &pair
            : pick_times) {
        DOCTEST_REQUIRE_EQ(pair.second, 4);
    }
...
// 完全随机的情况下，应该每个instance被pick500次，最后每个instance的pick_time相对均匀
    for (
        ...
        for (
            ...
            peer_list.
                    insert(resource_ips[resource_tag][random_index]);
        }
 ...
 * 网段信息的动态维护
 */
DOCTEST_TEST_CASE_FIXTURE(ClusterManagerTest, "test_load_balance_by_network_segment_add_drop_instance") {
    std::string selected_instance, resource_tag;
    std::unordered_map<std::string, int> pick_times;
    int ret = 0;
    size_t network_segment_size, prefix;
...
// rolling 30次，唯一的instance被pick 30次
    for (
        ...
        ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""}, {}, selected_instance);
        DOCTEST_REQUIRE_EQ(0, ret);
        DOCTEST_REQUIRE_EQ("10.0.0.0:8010", selected_instance);
    }
...
// 有两个网段，prefix=32
    DOCTEST_REQUIRE_EQ(2, network_segment_size);
    DOCTEST_REQUIRE_EQ(32, prefix);
// rolling 30次，每个instance被pick 15次
    ...
    DOCTEST_REQUIRE_EQ(15, pick_times["10.0.0.0:8010"]);
    DOCTEST_REQUIRE_EQ(15, pick_times["10.222.0.1:8010"]);
...
// 现在有三个网段，且prefix=32
    DOCTEST_REQUIRE_EQ(3, network_segment_size);
    DOCTEST_REQUIRE_EQ(32, prefix);
// rolling 30次，每个instance被pick 10次
    ...
    DOCTEST_REQUIRE_EQ(10, pick_times["10.0.0.0:8010"]);
    DOCTEST_REQUIRE_EQ(10, pick_times["10.222.0.1:8010"]);
...
// rolling 30次，剩下的两个instance每个被pick 15次
    ...
    for (
        ...
        pick_times[selected_instance]++;
        DOCTEST_REQUIRE_EQ(0, ret);
        DOCTEST_REQUIRE_NE("10.0.0.0:8010", selected_instance);
    }
    DOCTEST_REQUIRE_EQ(15, pick_times["10.222.0.1:8010"]);
...
// 对2个resource tag，都rolling 100次，正常
    for (
        ...
        ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""}, {}, selected_instance);
        DOCTEST_REQUIRE_EQ(0, ret);
        DOCTEST_REQUIRE_EQ("10.222.0.1:8010", selected_instance);
        ret = _cluster_manager->select_instance_rolling({"add_drop_tag2", "", ""}, {}, selected_instance);
        DOCTEST_REQUIRE_EQ(0, ret);
        DOCTEST_REQUIRE_EQ("10.223.0.1:8010", selected_instance);
    }
}
 ...
 */
DOCTEST_TEST_CASE_FIXTURE(ClusterManagerTest, "test_load_balance_by_network_segment_with_gflag") {
    std::string selected_instance, resource_tag;
    std::unordered_map<std::string, int> pick_times;
    int ret = 0;
    size_t network_segment_size, prefix;
...
// rolling 40次，每个instance被pick 10次。
    ...
    DOCTEST_REQUIRE_EQ(10, pick_times["11.0.0.1:8010"]);
    DOCTEST_REQUIRE_EQ(10, pick_times["11.0.0.2:8010"]);
    DOCTEST_REQUIRE_EQ(10, pick_times["11.0.0.3:8010"]);
...
// 3，4被exclude，则在bj-network1进行pick，其中1，2每个被pick 10次
    ...
    DOCTEST_REQUIRE_EQ(20, pick_times["11.0.0.1:8010"]);
    DOCTEST_REQUIRE_EQ(20, pick_times["11.0.0.2:8010"]);
// bj-network1，4被exclude，则只能pick 3
    ...
    for (
        ...
        ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""}, {"11.0.0.1:8010", "11.0.0.4:8010"},
                                                        selected_instance);
        DOCTEST_REQUIRE_EQ(0, ret);
        DOCTEST_REQUIRE_EQ("11.0.0.3:8010", selected_instance);
    }
...
// 现在1，2不属于同一个网段bj-network1, 模拟1，4被exclude，则2，3每个被pick 20次
    ...
    DOCTEST_REQUIRE_EQ(20, pick_times["11.0.0.2:8010"]);
    DOCTEST_REQUIRE_EQ(20, pick_times["11.0.0.3:8010"]);
}

/*
 * 测试添加集群
 */
DOCTEST_TEST_CASE_FIXTURE(ClusterManagerTest, "test_add_cluster") {
    std::string selected_instance, resource_tag;
    std::unordered_map<std::string, int> pick_times;
    int ret = 0;
    size_t network_segment_size, prefix;
...
// 初始化&add instance, 开启全局的network balance
    ...
    DOCTEST_REQUIRE_EQ(13, network_segment_size);
    DOCTEST_REQUIRE_EQ(21, prefix);
    for (
            int i = 0;
            i < 1000; ++i) {
        ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""}, {}, selected_instance);
    ...
    for (
        ...
        DOCTEST_REQUIRE_EQ(5, pick_times[ip]);
        DOCTEST_REQUIRE_EQ(5, pick_times[ip2]);
    }
}