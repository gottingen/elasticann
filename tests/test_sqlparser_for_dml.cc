...
DOCTEST_TEST_CASE("test_parser, case_insert) {
    {
        ...
        DOCTEST_REQUIRE_EQ(insert_stmt->priority, parser::PE_LOW_PRIORITY);
        ASSERT_TRUE(insert_stmt->table_name != nullptr);
        DOCTEST_REQUIRE_EQ(std::string(insert_stmt->table_name->db.value), "db");
        DOCTEST_REQUIRE_EQ(std::string(insert_stmt->table_name->table.value), "table_a");
        DOCTEST_REQUIRE_EQ(0, insert_stmt->columns.size());
        DOCTEST_REQUIRE_EQ(3, insert_stmt->lists.size());
        parser::RowExpr* row_expr0 = insert_stmt->lists[0];
        DOCTEST_REQUIRE_EQ(2, row_expr0->children.size());
        parser::RowExpr* row_expr1 = insert_stmt->lists[1];
        DOCTEST_REQUIRE_EQ(2, row_expr1->children.size());
        parser::RowExpr* row_expr2 = insert_stmt->lists[2];
        DOCTEST_REQUIRE_EQ(2, row_expr2->children.size());
        DOCTEST_REQUIRE_EQ(2, insert_stmt->on_duplicate.size());
    ...
    }
    {
        ...
        DOCTEST_REQUIRE_EQ(insert_stmt->priority, parser::PE_DELAYED_PRIORITY);
        ASSERT_TRUE(insert_stmt->table_name != nullptr);
        ASSERT_TRUE(insert_stmt->table_name->db.empty());
        DOCTEST_REQUIRE_EQ(std::string(insert_stmt->table_name->table.value), "table_a");
        DOCTEST_REQUIRE_EQ(2, insert_stmt->columns.size());
    ...
    }
    {
        ...
        ASSERT_TRUE(insert_stmt->table_name->db.empty());
        DOCTEST_REQUIRE_EQ(std::string(insert_stmt->table_name->table.value), "table_a");
        DOCTEST_REQUIRE_EQ(1, insert_stmt->columns.size());
        ASSERT_TRUE(std::string(insert_stmt->columns[0]->db.value) == "db");
        ASSERT_TRUE(std::string(insert_stmt->columns[0]->table.value) == "table_a");
        ASSERT_TRUE(std::string(insert_stmt->columns[0]->name.value) == "field_a");

        DOCTEST_REQUIRE_EQ(1, insert_stmt->lists.size());
        parser::RowExpr* row_expr0 = insert_stmt->lists[0];
        DOCTEST_REQUIRE_EQ(2, row_expr0->children.size());
    ...
    }
    {
        ...
        ASSERT_FALSE(insert_stmt->is_replace);
        ASSERT_FALSE(insert_stmt->is_ignore);
        DOCTEST_REQUIRE_EQ(insert_stmt->priority, parser::PE_HIGH_PRIORITY);
        ASSERT_TRUE(insert_stmt->table_name != nullptr);
        ASSERT_TRUE(insert_stmt->table_name->db.empty());
        DOCTEST_REQUIRE_EQ(std::string(insert_stmt->table_name->table.value), "table_a");
        DOCTEST_REQUIRE_EQ(0, insert_stmt->columns.size());

        DOCTEST_REQUIRE_EQ(2, insert_stmt->lists.size());
        parser::RowExpr* row_expr0 = insert_stmt->lists[0];
        DOCTEST_REQUIRE_EQ(2, row_expr0->children.size());
        parser::RowExpr* row_expr1 = insert_stmt->lists[1];
        DOCTEST_REQUIRE_EQ(2, row_expr1->children.size());
    ...
    }
    {
        ...
        DOCTEST_REQUIRE_EQ(2, insert_stmt->lists.size());
        parser::RowExpr* row_expr0 = insert_stmt->lists[0];
        DOCTEST_REQUIRE_EQ(1, row_expr0->children.size());
        parser::RowExpr* row_expr1 = insert_stmt->lists[1];
        DOCTEST_REQUIRE_EQ(1, row_expr1->children.size());
    ...
    }
    {
        ...
        DOCTEST_REQUIRE_EQ(0, insert_stmt->on_duplicate.size());
    }
    {
        parser::SqlParser parser;
    ...
    }
    {
        ...
        DOCTEST_REQUIRE_EQ(1, insert_stmt->on_duplicate.size());
    }
    {
        parser::SqlParser parser;
        ...
        ASSERT_TRUE(insert_stmt->table_name != nullptr);
        ASSERT_TRUE(std::string(insert_stmt->table_name->db.value) == "db");
        DOCTEST_REQUIRE_EQ(std::string(insert_stmt->table_name->table.value), "t1");
        DOCTEST_REQUIRE_EQ(0, insert_stmt->columns.size());
        ...
        DOCTEST_REQUIRE_EQ(3, select_stmt->fields.size());
    }
    {
        parser::SqlParser parser;
        std::string sql_insert9 = "insert into db.t1 (select f1,f2,f3 from db.t2);";
    ...
    }
    {
        ...
        ASSERT_TRUE(insert_stmt->table_name != nullptr);
        ASSERT_TRUE(std::string(insert_stmt->table_name->db.value) == "db");
        DOCTEST_REQUIRE_EQ(std::string(insert_stmt->table_name->table.value), "student1");
        DOCTEST_REQUIRE_EQ(9, insert_stmt->columns.size());
        ASSERT_TRUE(insert_stmt->subquery_stmt != nullptr);
        ASSERT_TRUE(insert_stmt->subquery_stmt->node_type == NT_SELECT);
        parser::SelectStmt* select_stmt = (parser::SelectStmt*)insert_stmt->subquery_stmt;
        DOCTEST_REQUIRE_EQ(9, select_stmt->fields.size());
        ASSERT_TRUE(select_stmt->is_in_braces == true);
    }
}
DOCTEST_TEST_CASE("test_parser, case_replace) {
    {
        parser::SqlParser parser;
        //select distict
...
DOCTEST_TEST_CASE("test_parser, case_update) {
    {
        parser::SqlParser parser;
        //select distict
        std::string sql_replace0 = "replace LOW_PRIORITY into db.table_a(field_a, field_b) values (1, 'test'),"
...
DOCTEST_TEST_CASE("test_parser, case_delete) {
    {
        parser::SqlParser parser;
        //select distict
        ...
        DOCTEST_REQUIRE_EQ(delete_stmt->priority, parser::PE_LOW_PRIORITY);
        ...
        DOCTEST_REQUIRE_EQ(std::string(from_table->db.value), "db");
        DOCTEST_REQUIRE_EQ(std::string(from_table->table.value), "table_a");
    ...
    }
    {
        ...
        DOCTEST_REQUIRE_EQ(delete_stmt->priority, parser::PE_NO_PRIORITY);
    ...
    }
    {
        ...
        DOCTEST_REQUIRE_EQ(2, delete_stmt->delete_table_list.size());
        parser::TableName* table_name0 = delete_stmt->delete_table_list[0];
        DOCTEST_REQUIRE_EQ(std::string(table_name0->db.value), "db");
        DOCTEST_REQUIRE_EQ(std::string(table_name0->table.value), "table_a");
        parser::TableName* table_name1 = delete_stmt->delete_table_list[1];
        DOCTEST_REQUIRE_EQ(std::string(table_name1->db.value), "db");
        DOCTEST_REQUIRE_EQ(std::string(table_name1->table.value), "table_b");
    }
    {
        parser::SqlParser parser;
        //select distict
    ...
    }
    {
        ...
        DOCTEST_REQUIRE_EQ(update_stmt->priority, parser::PE_LOW_PRIORITY);
